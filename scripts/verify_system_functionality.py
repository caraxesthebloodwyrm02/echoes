...
def verify_feature_exists(module_path: str, feature_name: str, description: str = "") -> Dict[str, Any]:
    ...
    try:
        ...
        if len(module_path_parts) == 1:
            ...
            for part in module_path_parts[1:]:
                ...
                if module is None:
                    return {"exists": False, "error": f"Could not import {part} from {module_path}"}
            return {"exists": True, "module": module_path, "feature": feature_name}
...
def check_class_methods(class_obj, expected_methods: List[str]) -> Dict[str, Any]:
    ...
    methods = [m for m in dir(class_obj) if not m.startswith('__') or (m.startswith('__') and not m.endswith('__'))]
    found = []
    missing = []
...
def main():
    ...
    try:
        ...
        # Check key methods
        expected_methods = [
            "chat", "add_knowledge", "execute_action", "gather_knowledge",
            "search_knowledge", "get_stats", "update_quantum_state",
            "measure_quantum_state", "get_conversation_history", "clear_history"
        ]
        method_check = check_class_methods(EchoesAssistantV2, expected_methods)
        
        if method_check["missing_count"] == 0:
            results["passed"].append(f"All expected methods present ({method_check['found_count']}/{len(expected_methods)})")
        else:
            results["warnings"].append(f"Missing methods: {method_check['missing']}")
            results["passed"].append(f"Found {method_check['found_count']}/{len(expected_methods)} expected methods")
        
        print(f"   [OK] EchoesAssistantV2 class exists")
        print(f"   [OK] Found {method_check['total_methods']} methods total")
        print(f"   [OK] {method_check['found_count']}/{len(expected_methods)} expected methods present")
    ...
    print("-" * 80)
    try:
        from echoes.core.rag_v2 import create_rag_system, OPENAI_RAG_AVAILABLE, RAG_AVAILABLE
    ...
    try:
        ...
        results["passed"].append("ModelRouter, ModelMetrics, ModelResponseCache importable")
    ...
    try:
        ...
        assistant1 = EchoesAssistantV2(enable_tools=False, enable_rag=False, enable_streaming=False)
        ...
        with open('assistant_v2_core.py', 'r', encoding='utf-8') as f:
            ...
            if responses_api_calls > 0:
                results["passed"].append(f"Responses API integration present ({responses_api_calls} calls)")
                print(f"   [OK] Responses API calls found: {responses_api_calls}")
            if chat_api_calls > 0:
                results["passed"].append(f"Chat Completions API integration present ({chat_api_calls} calls)")
    ...
    try:
        ...
        with open('assistant_v2_core.py', 'r', encoding='utf-8') as f:
            ...
            if 'def stream_response()' in content or 'yield' in content:
                ...
                results["warnings"].append("Streaming implementation needs verification")
    ...
    try:
        with open('assistant_v2_core.py', 'r', encoding='utf-8') as f:
            ...
            if api_error_handling > 0:
                results["passed"].append(f"API error handling present ({api_error_handling} instances)")
                print(f"   [OK] API error handling: {api_error_handling} instances")
            
            if general_error_handling > 0:
                results["passed"].append(f"General error handling present ({general_error_handling} instances)")
                print(f"   [OK] General error handling: {general_error_handling} instances")
                
            if fallback_patterns > 0:
                results["passed"].append(f"Fallback mechanisms present ({fallback_patterns} references)")
    ...
    # Overall assessment
    pass_rate = len(results['passed']) / (len(results['passed']) + len(results['failed']) + len(results['warnings'])) * 100
    ...
    if len(results['failed']) == 0:
        ...
        if len(results['warnings']) == 0:
            ...
            print(f"[WARN] {len(results['warnings'])} warnings found - review recommended")
    else:
