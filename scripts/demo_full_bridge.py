    ...
    try:
        ...
        for claim in claims:
            ...
            if truth_result['success']:
                print(f"   ✅ {claim[:50]}... → {truth_result['verdict']} ({truth_result['confidence']:.2f})")
            else:
        ...
        if bridge_result['success']:
            ...
            if isinstance(bridge_result.get('patterns'), dict) and bridge_result['patterns'].get('success'):
            ...
            if isinstance(bridge_result.get('truth'), dict) and bridge_result['truth'].get('success'):
        ...
        if glimpse_result['success'] and glimpse_result['aligned']:
            ...
            if commit_result['success']:
                print(f"   ✅ Glimpse committed to session {commit_result['session_id']}")
        ...
        print(f"   Glimpse System: {'✅ Active' if status['status']['glimpse_active'] else '❌ Inactive'}")
        print(f"   External Contact: {'✅ Connected' if status['status']['external_contact_enabled'] else '❌ Disconnected'}")
