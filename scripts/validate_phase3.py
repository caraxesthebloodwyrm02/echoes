...
def test_environment_variable_parsing():
    ...
    for true_value in ['1', 'true', 'yes', 'TRUE', 'YES']:
        ...
        use_responses_api = os.getenv("USE_RESPONSES_API", "false").lower() in ("1", "true", "yes")
        assert use_responses_api == True, f"Expected True for {true_value}, got {use_responses_api}"
    ...
    for false_value in ['0', 'false', 'no', 'FALSE', 'NO', 'anything_else']:
        ...
        use_responses_api = os.getenv("USE_RESPONSES_API", "false").lower() in ("1", "true", "yes")
        assert use_responses_api == False, f"Expected False for {false_value}, got {use_responses_api}"
...
def test_api_parameter_mapping():
    ...
    assert "max_output_tokens" in responses_params, "max_output_tokens parameter missing"
    assert responses_params["input"] == messages, "input mapping incorrect"
    assert responses_params["max_output_tokens"] == max_tokens, "max_output_tokens mapping incorrect"
...
def test_response_format_handling():
    ...
    assert assistant_response == "Hello world!", f"Expected 'Hello world!', got '{assistant_response}'"
    ...
    assert chat_content == "Hello from chat completions!", f"Unexpected content: {chat_content}"
