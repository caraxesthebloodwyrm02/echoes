...
def soft_knee_compress(signal, threshold_db=-6, ratio=4, knee_width=3):
    ...
    gain_reduction[over_threshold] = compressor_curve(signal_db[over_threshold] - threshold_db) * 20 / np.log(10)
...
for run in range(1, runs + 1):
    ...
    if len(vocal_resampled) < len(vocal):
        vocal_resampled = np.pad(vocal_resampled, (0, len(vocal) - len(vocal_resampled)))
    ...
    static = np.clip(static / np.max(static) if np.max(static) > 0 else static, -0.1, 0.1)
    reverbed += static
