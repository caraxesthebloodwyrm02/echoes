...
class FileOrganizer:
    def __init__(self, root_dir: str):
        ...
        self.target_dirs = {
            'glimpse': {
                ...
                'keywords': ['glimpse', 'nexus', 'smartvision', 'analysis', 'insight', 'diagnostic'],
    ...
    def get_file_keywords(self, filepath: Path) -> Set[str]:
        ...
        try:
            if filepath.suffix.lower() in {'.py', '.md', '.txt', '.yaml', '.yml', '.json'}:
                with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    for word in re.findall(r'\b\w{4,}\b', content):
                        if any(kw in word for kw in ['glimpse', 'test', 'doc', 'script', 'config']):
                            keywords.add(word)
    ...
    def get_best_match_dir(self, filepath: Path) -> Optional[Path]:
        ...
        for name, config in self.target_dirs.items():
            ...
            score = sum(1 for kw in keywords if any(k in kw for k in config['keywords']))
    ...
    def organize_files(self, dry_run: bool = True) -> Dict[str, List[str]]:
        ...
        for filepath in self.root_dir.glob('*'):
            ...
            if any(str(filepath).startswith(str(config['dir'])) for config in self.target_dirs.values()):
                continue
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Organize files based on content and naming patterns.')
