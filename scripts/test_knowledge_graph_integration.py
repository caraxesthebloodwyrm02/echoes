...
async def test_knowledge_graph_integration():
    ...
    result2 = assistant.add_knowledge_node(
        ...
        properties={"category": "AI", "complexity": "high", "applications": ["chatbots", "translation"]}
    )
    ...
    result3 = assistant.add_knowledge_node(
        ...
        properties={"status": "active", "version": "2.0", "features": ["RAG", "Glimpse", "Knowledge Graph"]}
    )
    ...
    result7 = assistant.add_memory_fragment(
        ...
        context={"event": "AI Conference 2024", "location": "San Francisco", "date": "2024-10-15"},
        importance=0.9
    )
    ...
    if search_result['success']:
        ...
        for node in search_result['results']:
            print(f"      📍 {node['label']} ({node['type']}): {node['description'][:50]}...")
            if node['related_nodes']:
                print(f"         ↳ Related: {', '.join([r['label'] for r in node['related_nodes']])}")
    ...
    memories_result = assistant.retrieve_relevant_memories("Sarah Chen conference", limit=3)
    if memories_result['success']:
        ...
        for memory in memories_result['memories']:
            print(f"      💭 {memory['content'][:60]}... (importance: {memory['importance']})")
    ...
    if communication_result['success']:
        ...
        print(f"      - Concepts: {communication_result['context_used']['related_concepts']}")
    ...
    print(f"      🧠 Knowledge Graph: {'Enabled' if full_stats.get('knowledge_graph_stats') else 'Disabled'}")
    print(f"      🔍 RAG System: {'Enabled' if full_stats.get('rag_enabled') else 'Disabled'}")
    print(f"      👁️ Glimpse: {'Enabled' if full_stats.get('glimpse_enabled') else 'Disabled'}")
