...
async def test_glimpse_integration():
    ...
    evidence = ["Financial report shows 25% growth", "CEO announcement confirmed results"]
    ...
    if analysis_result['success']:
        ...
        if isinstance(analysis_result['patterns'], dict) and analysis_result['patterns'].get('success'):
            print(f"   ✓ Patterns analyzed: {len(analysis_result['patterns'].get('patterns', []))}")
        if isinstance(analysis_result['truth'], dict) and analysis_result['truth'].get('success'):
            print(f"   ✓ Truth verified: {analysis_result['truth'].get('verdict', 'UNKNOWN')}")
    else:
