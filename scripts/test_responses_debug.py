...
def debug_responses_api():
    ...
    try:
        result = assistant.chat("Hello! Say 'API test successful' if you can read this.", stream=False)
    ...
    try:
        result = assistant2.chat("What is 2 + 2? Use the calculator tool.", stream=False)
        print(f"   Result length: {len(result) if result else 0}")
        print(f"   Result: {repr(result)}")
        print(f"   Type: {type(result)}")
    except Exception as e:
    ...
    if assistant2.tool_registry:
        ...
        if tools:
            ...
            if converted_tools:
                print(f"   Converted tool schema: {json.dumps(converted_tools[0], indent=2)}")
