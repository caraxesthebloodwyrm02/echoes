...
def authenticate_sources(output_file='source_authentication_step2.json'):
    ...
    trusted_sources = {
        ...
        'openai_official_api': {
            ...
            'expected_response_codes': [200, 401, 429]  # 401/429 are expected for auth/rate limit
        }
    }
    ...
    packages_to_check = ['pydantic', 'fastapi', 'openai', 'sentence-transformers', 'numpy']
    ...
    for package in packages_to_check:
        try:
            ...
            if result.returncode == 0:
                ...
                auth_data['anomalies'].append(f'Package {package} import failed: {result.stderr}')
                print(f'   ❌ {package}: Import failed')

        except Exception as e:
            auth_data['validation_results'][f'{package}_version'] = f'ERROR: {str(e)}'
            auth_data['anomalies'].append(f'Package {package} validation failed: {str(e)}')
    ...
    try:
        ...
        git_check = subprocess.run(['git', 'rev-parse', '--git-dir'],
                                 capture_output=True, cwd='e:/Projects/Echoes', timeout=10)

        if git_check.returncode == 0:
            # Get current commit hash
            commit_result = subprocess.run(['git', 'rev-parse', 'HEAD'],
                                         capture_output=True, text=True, cwd='e:/Projects/Echoes', timeout=10)
            ...
            if commit_result.returncode == 0:
                ...
                if status_result.returncode == 0 and status_result.stdout.strip():
                    ...
                    auth_data['validation_results']['uncommitted_changes'] = uncommitted_files
                    auth_data['anomalies'].append(f'{uncommitted_files} uncommitted changes detected')
    ...
    successful_validations = sum(1 for v in auth_data['validation_results'].values()
                               if not str(v).startswith(('ERROR', 'NOT_FOUND', 'VERSION_MISMATCH', 'IMPORT_FAILED')))

    authenticity_score = (successful_validations / total_validations) * 100 if total_validations > 0 else 0
    ...
    if anomalies == 0 and authenticity_score >= 75:
        print('✅ STEP 2 COMPLETE: Source authentication successful, proceeding to Step 3')
        return True
    else:
        print(f'⚠️  STEP 2 ISSUES: {anomalies} anomalies detected, authenticity at {authenticity_score:.1f}%')
        return False
