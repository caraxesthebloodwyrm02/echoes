...
def reconstruct_components(extraction_dir='extracted_components_step3', output_file='component_reconstruction_step5.json'):
    ...
    if not os.path.exists(extraction_dir):
        reconstruction_data['anomalies'].append(f'Extraction directory not found: {extraction_dir}')
    ...
    reconstruction_specs = {
        'api/main.py': {
            ...
            'validation_checks': ['syntax_check', 'import_verification', 'functionality_test'],
        ...
        'app/agents/agent.py': {
            ...
            'validation_checks': ['syntax_check', 'agent_initialization', 'message_processing'],
            'dependencies': ['openai', 'asyncio', 'logging']
        },
    ...
    for component_name, spec in reconstruction_specs.items():
        ...
        if not os.path.exists(source_path):
            ...
            try:
                ...
                validation_results = run_validation_checks(target_path, spec['validation_checks'])

                reconstruction_result['validations_passed'] = sum(1 for v in validation_results.values() if v.get('passed', False))
                reconstruction_result['validation_details'] = validation_results

                if reconstruction_result['validations_passed'] == reconstruction_result['total_validations']:
                    reconstruction_result['status'] = 'success'
                    successful_reconstructions += 1
                    print(f'   ✅ Validated: {component_name} ({reconstruction_result["validations_passed"]}/{reconstruction_result["total_validations"]} checks passed)')
                else:
                    ...
                    reconstruction_data['anomalies'].append(f'Validation failed for {component_name}: {failed_checks}')
                    print(f'   ⚠️  Validation issues: {component_name} ({reconstruction_result["validations_passed"]}/{reconstruction_result["total_validations"]} checks passed)')

            except Exception as e:
                reconstruction_result['status'] = 'deployment_failed'
                reconstruction_result['error'] = str(e)
                reconstruction_data['anomalies'].append(f'Deployment failed for {component_name}: {str(e)}')
                print(f'   ❌ Deployment failed: {component_name} - {str(e)}')

        reconstruction_data['reconstructed_components'][component_name] = reconstruction_result

    # Generate reconstruction summary
    reconstruction_success_rate = (successful_reconstructions / total_components) * 100 if total_components > 0 else 0
    ...
    reconstruction_data['summary'] = {
        ...
        'reconstruction_methods_used': list(set(spec['reconstruction_method'] for spec in reconstruction_specs.values())),
    ...
    if reconstruction_success_rate >= 80 and len(reconstruction_data['anomalies']) == 0:
        print('✅ STEP 5 COMPLETE: Component reconstruction successful, proceeding to Step 6')
        return True
    else:
        print(f'⚠️  STEP 5 ISSUES: Reconstruction rate at {reconstruction_success_rate:.1f}%, {len(reconstruction_data["anomalies"])} anomalies')
        return False
