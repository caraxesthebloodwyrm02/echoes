...
class CognitiveAccountingSystem:
    ...
    def __init__(self, storage_path: str = "enhanced_accounting"):
        ...
        self.values_accounting = {
            ...
            "trust": {
                ...
                "metrics": ["payout_timeliness", "value_accuracy", "dispute_resolution"],
                "weight": 0.20
            },
            "creativity": {
                "description": "Valuing and rewarding creative contributions",
                "metrics": ["creativity_bonus_paid", "innovation_rewards", "creative_attribution"],
                "weight": 0.25
            },
            "delightful_humor": {
                "description": "Promoting positive and joyful interactions",
                "metrics": ["engagement_bonus", "positive_interaction_rate", "humor_rewards"],
                "weight": 0.15
            },
            "freedom_of_thought": {
                "description": "Protecting cognitive liberty and privacy",
                "metrics": ["privacy_compliance", "thought_protection", "cognitive_rights"],
                "weight": 0.15
            }
        }
    ...
    def create_user_account(self, user_id: str, consent_type: ConsentType = ConsentType.PERSONAL_DEVELOPMENT) -> UserAccount:
    ...
    def record_cognitive_transaction(self,
                                   user_id: str,
                                   session_id: str,
                                   effort_metrics: CognitiveEffortMetrics,
                                   value_type: ValueType = ValueType.COGNITIVE_JOULES) -> TransactionRecord:
        ...
        base_value = Decimal(str(effort_metrics.joules_of_work)) * self.accounting_policies["base_rate_per_joule"]
        ...
        if value_type == ValueType.CREATIVE_INSIGHTS:
            ...
            bonus_multiplier *= self.accounting_policies["collaboration_bonus_multiplier"]
    ...
    def generate_user_statement(self, user_id: str, period_start: str, period_end: str) -> Dict[str, Any]:
        ...
        period_transactions = [
            ...
            if (tx.user_id == user_id and 
                period_start <= tx.timestamp <= period_end)
        ]
        ...
        period_fees = sum(tx.gross_value * tx.platform_cut for tx in period_transactions)
        ...
        return {
            ...
            "summary": {
                ...
                "average_value_per_transaction": float(period_net / len(period_transactions))
            },
    ...
    def generate_financial_statement(self, period_type: AccountingPeriod, period_start: str, period_end: str) -> FinancialStatement:
        ...
        statement = FinancialStatement(
            ...
            average_joules_per_user=total_joules / unique_users if unique_users > 0 else 0.0,
            average_value_per_user=total_net / unique_users if unique_users > 0 else Decimal('0.00'),
    ...
    def calculate_payout_eligibility(self, user_id: str) -> Dict[str, Any]:
        ...
        if user_account.total_net_value < self.accounting_policies["minimum_payout_threshold"]:
            return {
                "eligible": False,
                ...
                "threshold_needed": float(self.accounting_policies["minimum_payout_threshold"] - user_account.total_net_value)
            }
        ...
        return {
            ...
            "last_transaction": max(user_account.transactions) if user_account.transactions else None
        }
    
    def _calculate_values_alignment_score(self, transactions: List[TransactionRecord]) -> Dict[str, Any]:
        ...
        for value_name, value_config in self.values_accounting.items():
            ...
            if value_name == "integrity":
                ...
                compliant_tx = len([tx for tx in transactions if tx.consent_compliance.get('compliant', False)])
                ...
                creative_tx = len([tx for tx in transactions if tx.value_type in [ValueType.CREATIVE_INSIGHTS, ValueType.INNOVATION_POTENTIAL]])
        ...
        return {
            ...
            "compliance_status": "Excellent" if total_score >= 90 else "Good" if total_score >= 75 else "Needs Improvement"
        }
            ...
            if accounts_file.exists():
                with open(accounts_file, 'r', encoding='utf-8') as f:
                    ...
                    for user_id, account_dict in accounts_data.items():
                        ...
                        for field in ['total_gross_value', 'total_net_value', 'total_tax_paid', 'total_platform_fees']:
                            if field in account_dict:
                                account_dict[field] = Decimal(str(account_dict[field]))
                        for key, value in account_dict.get('value_breakdown', {}).items():
            ...
            if statements_file.exists():
                with open(statements_file, 'r', encoding='utf-8') as f:
                    ...
                    for stmt_id, stmt_dict in statements_data.items():
                        # Convert Decimal fields and enums
                        for field in ['total_gross_value', 'total_net_value', 'total_tax_collected', 'total_platform_revenue', 'average_value_per_user']:
                            if field in stmt_dict:
                                stmt_dict[field] = Decimal(str(stmt_dict[field]))
                        for key, value in stmt_dict.get('value_distribution', {}).items():
                            stmt_dict['value_distribution'][key] = Decimal(str(value))
                        stmt_dict['period_type'] = AccountingPeriod(stmt_dict['period_type'])
                        
                        self.financial_statements[stmt_id] = FinancialStatement(**stmt_dict)
            
            print(f"✓ Enhanced accounting loaded: {len(self.user_accounts)} users, {len(self.transactions)} transactions")
    ...
    def _save_data(self):
        ...
        try:
            # Save user accounts
            accounts_file = self.storage_path / "user_accounts.json"
            accounts_data = {user_id: asdict(account) for user_id, account in self.user_accounts.items()}
            ...
            statements_file = self.storage_path / "financial_statements.json"
            statements_data = {stmt_id: asdict(stmt) for stmt_id, stmt in self.financial_statements.items()}
