...
class KnowledgeGraph:
    ...
    def _load_knowledge(self):
        ...
        try:
            ...
            print(f"✓ Knowledge graph loaded: {len(self.nodes)} nodes, {len(self.relations)} relations, {len(self.memories)} memories")
    ...
    def get_related_nodes(self, node_id: str, relation_type: Optional[str] = None, max_depth: int = 2) -> List[KnowledgeNode]:
    ...
    def retrieve_memories(self, query: str, context: Optional[Dict] = None, limit: int = 5) -> List[MemoryFragment]:
        ...
        for memory in self.memories:
            ...
            days_old = (datetime.now(timezone.utc) - datetime.fromisoformat(memory.timestamp.replace('Z', '+00:00'))).days
    ...
    def update_conversation_context(self, message: str, response: str, entities: List[str] = None):
    ...
    def get_communication_context(self, query: str) -> Dict[str, Any]:
        ...
        for entity in entities:
            related = self.get_related_nodes(entity.id, relation_type="related_to", max_depth=1)
            related_concepts.extend([node.label for node in related[:3]])
        
        return {
            "entities": entity_names,
            "memories": [{"content": mem.content, "importance": mem.importance} for mem in memories],
    ...
    def learn_from_conversation(self, message: str, response: str, confidence: float = 0.8):
        """Learn and extract knowledge from conversations"""
        entities, concepts = self.extract_entities_and_concepts(message + " " + response)
