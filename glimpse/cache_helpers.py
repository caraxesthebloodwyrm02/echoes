...
class PromptCache:
    ...
    @staticmethod
    def _hash_prompt(messages: list[dict], model: str, temperature: float, max_tokens: Optional[int]) -> str:
    ...
    async def get(self, messages: list[dict], model: str, temperature: float, max_tokens: Optional[int]) -> Optional[Any]:
    ...
    async def set(self, messages: list[dict], model: str, temperature: float, max_tokens: Optional[int], response: Any) -> None:
...
def cached_openai_call(cache: Optional[PromptCache] = None, skip_cache: bool = False):
    ...
    """
    def decorator(func):
        async def wrapper(messages: list[dict], model: str, temperature: float, 
            ...
            if skip_cache or temperature > 0:
                logger.debug("cache_bypass", extra={
