...
class UserEngagementTracker:
    ...
    def track_interaction(self, user_responded: bool = True):
        ...
        self.engagement_score = min(1.0, self.engagement_score + 0.05 if user_responded else self.engagement_score - 0.02)
    ...
    def should_ask_curiosity(self) -> bool:
        ...
        return (self.engagement_score > 0.3 and 
                time_since_last > 300 and 
                self.curiosity_response_rate > 0.2 and
...
class EnhancedClarifierEngine:
    ...
    def detect_critical_ambiguity(self, input_text: str, goal: str, constraints: str) -> List[Clarifier]:
        ...
        if (any(word in text_lower for word in ["delete", "remove", "cancel", "terminate"]) and
            "critical_action" not in constraints.lower()):
            clarifiers.append(self.critical_clarifiers["critical_audience"])
        
        # Maximum 1 critical clarifier to avoid blocking
        return clarifiers[:1]
    
    def generate_curiosity_question(self, context: Dict[str, Any]) -> Optional[Clarifier]:
        """
