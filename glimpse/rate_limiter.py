...
class AdaptiveRateLimiter:
    ...
    async def acquire(
        ...
        async with self.lock:
            while True:
                ...
                content_time_needed = content_tokens_needed / self.tokens_per_second_tpm if content_tokens_needed > 0 else 0
    ...
    async def record_rate_limit(self, endpoint: str = "default"):
        ...
        async with self.lock:
            ...
            self.current_rpm = max(
                self.min_rpm, 
                min(self.current_rpm * 0.8, self.current_rpm - 100)
            )
            self.current_tpm = max(
                self.min_tpm,
                min(self.current_tpm * 0.8, self.current_tpm - 10000)
            )
            self.tokens_per_second = self.current_rpm / 60.0
            self.bucket_capacity = (self.current_rpm / 60.0) * self.burst_multiplier
            self.tokens_per_second_tpm = self.current_tpm / 60.0
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requests_in_last_minute": len([t for t in self.stats.request_timestamps 
                                          if t > now - 60]),
            "endpoints": {
                endpoint: {
                    "total_requests": stats.total_requests,
                    "success_rate": stats.get_success_rate(),
                    "requests_in_last_minute": len([t for t in stats.request_timestamps 
                                                  if t > now - 60])
                }
