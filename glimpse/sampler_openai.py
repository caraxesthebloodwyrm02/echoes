"""
OpenAI-backed sampler for Glimpse using the async wrapper.
Replaces or supplements default_sampler with real API calls.
"""
import asyncio
import logging
from typing import Optional, Tuple, Awaitable
from glimpse.engine import Draft, GlimpseResult, Sampler
from .openai_wrapper import get_default_client
...
async def _openai_chat_completion(messages: list[dict], model: str, temperature: float, max_tokens: Optional[int], **kwargs) -> dict:
    """Cached wrapper around the actual OpenAI chat completion."""
    client = get_default_client()
    return await client.chat_completion(messages, model, temperature, max_tokens, **kwargs)
