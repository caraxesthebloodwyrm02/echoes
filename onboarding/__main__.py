    ...
    try:
        ...
        if result["onboarding_status"] == "complete":
            ...
            if result.get("ecosystem_connection", {}).get("status") == "connected":
                ...
                print(
                    f"   • Ecosystem Version: {result['ecosystem_connection']['ecosystem_version']}"
                )
                print(
                    f"   • Connection Type: {result['ecosystem_connection']['connection_type']}"
                )
            ...
            if result.get("api_integration", {}).get("status") == "connected":
                ...
                print(
                    f"   • Test Response: {result['api_integration']['test_response']}"
                )
        ...
        print(
            f"   • Ecosystem: {'Connected' if status['ecosystem_connected'] else 'Disconnected'}"
        )
