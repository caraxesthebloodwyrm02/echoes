...
class WeeklyPlan:
    ...
    def add_focus_area(self, area: str, key_outcomes: List[str], 
                      status: PlanStatus = PlanStatus.PENDING) -> None:
    ...
    def mark_complete(self, item_id: int, notes: str = "") -> None:
        ...
        for item in self.plan["focus_areas"]:
            if item["id"] == item_id:
                ...
                self.plan["focus_areas"] = [i for i in self.plan["focus_areas"] 
                                          if i["id"] != item_id]
    ...
    def weekly_review(self) -> str:
        ...
        pending = len([i for i in self.plan["focus_areas"] 
                      if i["status"] != PlanStatus.COMPLETED])
