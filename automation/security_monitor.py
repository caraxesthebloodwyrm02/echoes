...
def check_bandit_scan():
    ...
    success, stdout, stderr = run_command(
        ['python', '-m', 'bandit', '-r', 'app/', 'packages/', '-f', 'json', '-o', 'bandit_report.json']
    )
    ...
    try:
        ...
        medium_severity_count = results.get('metrics', {}).get('_totals', {}).get('SEVERITY.MEDIUM', 0)
        ...
        print(f'📊 Bandit Results: {high_severity_count} HIGH, {medium_severity_count} MEDIUM, {low_severity_count} LOW issues')
...
def generate_security_report():
    ...
    print(f'📊 Security Score: {report["security_score"]:.1f}% ({passed_checks}/{total_checks} checks passed)')
