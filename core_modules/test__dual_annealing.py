...
class TestDualAnnealing:
    ...
    def test_max_fun_no_ls(self):
        ret = dual_annealing(self.func, self.ld_bounds, no_local_search=True, maxfun=500, rng=self.seed)
        assert ret.nfev <= 500
        assert not ret.success
    ...
    def test_fun_args_no_ls(self):
        ret = dual_annealing(self.func, self.ld_bounds, args=((3.14159,)), no_local_search=True, rng=self.seed)
    ...
    def test_multi_ls_minimizer(self, method, atol):
        ret = dual_annealing(self.func, self.ld_bounds, minimizer_kwargs=dict(method=method), rng=self.seed)
    ...
    def test_callable_jac_hess_with_args_gh11052(self):
        ...
        def hess(x, power):
            ...
            return np.diag(power * np.exp(x**power) * x ** (power - 2) * (power * x**power + power - 1))

        def hessp(x, p, power):
            return hess(x, power) @ p

        res1 = dual_annealing(f, args=(2,), bounds=[[0, 1], [0, 1]], rng=rng, minimizer_kwargs=dict(method="L-BFGS-B"))
        res2 = dual_annealing(
            f, args=(2,), bounds=[[0, 1], [0, 1]], rng=rng, minimizer_kwargs=dict(method="L-BFGS-B", jac=jac)
        )
        res3 = dual_annealing(
            f,
            args=(2,),
