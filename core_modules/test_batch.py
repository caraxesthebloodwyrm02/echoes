...
class TestBatch:
    ...
    def batch_test(
        self, fun, arrays, *, core_dim=2, n_out=1, kwargs=None, dtype=None, broadcast=True, check_kwargs=True
    ):
    ...
    @pytest.mark.parametrize("fun_n_out", [(linalg.cholesky, 1), (linalg.ldl, 3), (linalg.cho_factor, 2)])
    @pytest.mark.parametrize("dtype", floating)
    ...
    @pytest.mark.parametrize(
        "fun_n_nout", [(linalg.eigh, 1), (linalg.eigh, 2), (linalg.eigvalsh, 1), (linalg.eigvals, 1)]
    )
    @pytest.mark.parametrize("dtype", floating)
    ...
    def test_expm_frechet(self, compute_expm, dtype):
        ...
        self.batch_test(linalg.expm_frechet, (A, E), n_out=n_out, kwargs=dict(compute_expm=compute_expm))
    ...
    @pytest.mark.parametrize("fun_n_out", [(linalg.eigh_tridiagonal, 2), (linalg.eigvalsh_tridiagonal, 1)])
    ...
    def test_clarkson_woodruff_transform(self, dtype):
        rng = np.random.default_rng(8342310302941288912051)
        A = get_random((5, 3, 4, 6), dtype=dtype, rng=rng)
        self.batch_test(linalg.clarkson_woodruff_transform, A, kwargs=dict(sketch_size=3, rng=311224))
