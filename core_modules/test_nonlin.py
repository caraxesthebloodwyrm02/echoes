...
class TestNonlin:
    ...
    def test_warnings_invalid_inner_param(self):
        ...
        nonlin.newton_krylov(F, F.xin, method="minres", inner_maxiter=100, inner_callback=lambda _: ...)
...
class TestLinear:
    ...
    def _check_autojac(self, A, b):
        ...
        sol = nonlin.nonlin_solve(func, np.zeros(b.shape[0]), jac, maxiter=2, f_tol=1e-6, line_search=None, verbose=0)
        np.testing.assert_allclose(A @ sol, b, atol=1e-6)
        # test jac input as array -- not a function
        sol = nonlin.nonlin_solve(func, np.zeros(b.shape[0]), A, maxiter=2, f_tol=1e-6, line_search=None, verbose=0)
        np.testing.assert_allclose(A @ sol, b, atol=1e-6)
