...
def shgo(
    ...
    *,
    ...
    if len(shc.LMC.xl_maps) == 0:
        ...
        shc.fail_routine(mes="Failed to find a feasible minimizer point. " f"Lowest sampling point = {shc.f_lowest}")
...
class SHGO:
    ...
    def finite_homology_growth(self):
        ...
        if self.hgrd <= self.minhgrd:
            self.stop_global = True
        if self.disp:
            logging.info(f"Current homology growth = {self.hgrd} " f" (minimum growth = {self.minhgrd})")
        return self.stop_global
