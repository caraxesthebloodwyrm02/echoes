    ...
    def get_texts_widths_heights_descents(self, renderer):
        ...
        for _loc, _angle, label in self._locs_angles_labels:
            ...
            whd = mtext._get_text_metrics_with_cache(
                renderer, clean_line, self._fontproperties, ismath=ismath, dpi=self.get_figure(root=True).dpi
            )
            whd_list.append(whd)
        return whd_list
...
class AxisArtist(martist.Artist):
    ...
    def __init__(self, axes, helper, offset=None, axis_direction="bottom", **kwargs):
        ...
        self.offset_transform = ScaledTranslation(
            *offset, Affine2D().scale(1 / 72) + self.axes.get_figure(root=False).dpi_scale_trans  # points to inches.
        )
