...
class MultiUFunc:
        ...
        if not isinstance(ufunc_or_ufuncs, np.ufunc):
            ...
            for ufunc in ufuncs_iter:
                if not isinstance(ufunc, np.ufunc):
                    raise ValueError("All ufuncs must have type `numpy.ufunc`." f" Received {ufunc_or_ufuncs}")
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self._resolve_out_shapes is not None:
            ...
            ufunc_out_shapes = self._resolve_out_shapes(*args[: -ufunc.nin], *ufunc_arg_shapes, ufunc.nout, **kwargs)
...
def _(diff_n):
    ...
    if not 0 <= diff_n <= 2:
        raise ValueError("diff_n is currently only implemented for orders 0, 1, and 2," f" received: {diff_n}.")
    return diff_n
...
def _(n, m, z_shape, branch_cut_shape, nout, **kwargs):
    ...
    return ((n + 1, 2 * abs(m) + 1) + np.broadcast_shapes(z_shape, branch_cut_shape) + (diff_n + 1,),)
...
def _(diff_n):
    diff_n = _nonneg_int_or_fail(diff_n, "diff_n", strict=False)
    if not 0 <= diff_n <= 2:
        raise ValueError("diff_n is currently only implemented for orders 2," f" received: {diff_n}.")
    return diff_n
