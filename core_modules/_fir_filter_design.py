...
def kaiserord(ripple, width):
    ...
    if A < 8:
        ...
        raise ValueError("Requested maximum ripple attenuation " f"{A:f} is too small for the Kaiser formula.")
    beta = kaiser_beta(A)
...
def firwin(numtaps, cutoff, *, width=None, window="hamming", pass_zero=True, scale=True, fs=None):
    ...
    if pass_zero in ("bandstop", "lowpass"):
        if pass_zero == "lowpass":
            if xp_size(cutoff) != 1:
                raise ValueError("cutoff must have one element if " f'pass_zero=="lowpass", got {cutoff.shape}')
        elif xp_size(cutoff) <= 1:
            raise ValueError("cutoff must have at least two elements if " f'pass_zero=="bandstop", got {cutoff.shape}')
        pass_zero = True
    elif pass_zero in ("bandpass", "highpass"):
        if pass_zero == "highpass":
            if xp_size(cutoff) != 1:
                raise ValueError("cutoff must have one element if " f'pass_zero=="highpass", got {cutoff.shape}')
        elif xp_size(cutoff) <= 1:
            raise ValueError("cutoff must have at least two elements if " f'pass_zero=="bandpass", got {cutoff.shape}')
        pass_zero = False
    elif not (pass_zero is True or pass_zero is False):
        raise ValueError(
            f"Parameter {pass_zero=} not in (True, False, 'bandpass', " + "'lowpass', 'highpass', 'bandstop')"
        )
...
def firwin2(numtaps, freq, gain, *, nfreqs=None, window="hamming", antisymmetric=False, fs=None):
    ...
    if nfreqs is not None and numtaps >= nfreqs:
        raise ValueError(
            f"ntaps must be less than nfreqs, but firwin2 was called with " f"ntaps={numtaps} and nfreqs={nfreqs}"
        )
    ...
    if xp.any(d == 0):
        ...
        if xp.any(d <= 0):
            raise ValueError(
                "freq cannot contain numbers that are too close " "(within eps * (fs/2): " f"{eps}) to a repeated value"
            )
...
def remez(numtaps, bands, desired, *, weight=None, type="bandpass", maxiter=25, grid_density=16, fs=None):
    """
...
def firls(numtaps, bands, desired, *, weight=None, fs=None):
    ...
    if bands.size != desired.size:
        raise ValueError(
            f"desired must have one entry per frequency, got {desired.size} " f"gains for {bands.size} frequencies."
        )
    desired.shape = (-1, 2)
    ...
    if len(weight) != len(desired):
        raise ValueError("weight must be the same size as the number of " f"band pairs ({len(bands)}).")
    if (weight < 0).any():
...
def minimum_phase(
    h, method: Literal["homomorphic", "hilbert"] = "homomorphic", n_fft: int | None = None, *, half: bool = True
):
    ...
    if not xp.any(xp.flip(h[-n_half:]) - h[:n_half] <= 1e-8 + 1e-6 * abs(h[:n_half])):
        warnings.warn("h does not appear to by symmetric, conversion may fail", RuntimeWarning, stacklevel=2)
