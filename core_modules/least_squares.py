...
def call_minpack(fun, x0, jac, ftol, xtol, gtol, max_nfev, x_scale, jac_method=None):
    ...
    x, info, status = _minpack._lmder(
        fun, jac, xp.astype(x0, x0.dtype), (), full_output, col_deriv, ftol, xtol, gtol, max_nfev, factor, diag
    )
...
def check_tolerance(ftol, xtol, gtol, method):
    ...
    if method == "lm" and (ftol < EPS or xtol < EPS or gtol < EPS):
        raise ValueError("All tolerances must be higher than machine epsilon " f"({EPS:.2e}) for method 'lm'.")
    elif ftol < EPS and xtol < EPS and gtol < EPS:
        raise ValueError("At least one of the tolerances must be higher than " f"machine epsilon ({EPS:.2e}).")
...
def least_squares(
    ...
    if J0.shape != (m, n):
        raise ValueError(f"The return value of `jac` has wrong shape: expected {(m, n)}, " f"actual {J0.shape}.")
    ...
    if method == "lm":
        ...
        result = call_minpack(vector_fun.fun, x0, vector_fun.jac, ftol, xtol, gtol, max_nfev, x_scale, jac_method=jac)
        ...
        if tr_solver == "lsmr" and "regularize" in tr_options:
            warn("The keyword 'regularize' in `tr_options` is not relevant " "for 'dogbox' method.", stacklevel=2)
