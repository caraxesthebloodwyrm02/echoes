...
class RendererSVG(RendererBase):
    ...
    def _draw_gouraud_triangle(self, transformed_points, colors):
        ...
        writer.element(
            "path", attrib={"d": dpath, "fill": f"url(#GR{self._n_gradients:x}_0)", "shape-rendering": "crispEdges"}
        )
        ...
        if not ismath:
            ...
            for style, chars in spans.items():
                ...
                for x, y, t in chars:  # Output one tspan for each character
                    writer.element("tspan", chr(t), x=_short_float_fmt(x), y=_short_float_fmt(y), style=style)
