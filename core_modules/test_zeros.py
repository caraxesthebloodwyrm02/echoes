...
class TestBracketMethods(TestScalarRootFinders):
    ...
    def test_bracket_is_array(self, method, function):
        ...
        a, b = 0.5, sqrt(3)
        r = root_scalar(
            function, method=method.__name__, bracket=np.array([a, b]), x0=a, xtol=self.xtol, rtol=self.rtol
        )
        assert r.converged
        assert_allclose(r.root, 1.0, atol=self.xtol, rtol=self.rtol)
        assert r.method == method.__name__
...
def test_gh9551_raise_error_if_disp_true():
    ...
    with pytest.raises(
        RuntimeError,
        match=r"^Derivative was zero\. Failed to converge after \d+ iterations, " r"value is [+-]?\d*\.\d+\.$",
    ):
