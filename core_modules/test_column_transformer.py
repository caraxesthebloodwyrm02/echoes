...
def test_verbose_feature_names_out_callable_or_str(transformers, remainder, verbose_feature_names_out, expected_names):
    """Check feature_names_out for verbose_feature_names_out=True (default)"""
    pd = pytest.importorskip("pandas")
    df = pd.DataFrame([[1, 2, 3, 4]], columns=["a", "b", "c", "d"])
    ct = ColumnTransformer(
...
def test_remainder_request_always_present():
    ...
    ct = ColumnTransformer(
        ...
        remainder=ConsumingTransformer().set_fit_request(metadata=True).set_transform_request(metadata=True),
    )
...
def test_unused_transformer_request_present():
    ...
    ct = ColumnTransformer(
        [
            (
                "trans",
                ConsumingTransformer().set_fit_request(metadata=True).set_transform_request(metadata=True),
                lambda X: [],
            )
        ]
    )
