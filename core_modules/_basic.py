...
from numpy import pi, asarray, floor, isscalar, sqrt, where, sin, place, issubdtype, extract, inexact, nan, zeros, sinc
...
def _factorialx_approx_core(n, k, extend):
    ...
    if not isinstance(n, np.ndarray):
        with warnings.catch_warnings():
            ...
            warnings.simplefilter("ignore", RuntimeWarning)
            return np.power(k, (n - n_mod_k) / k) * gamma(n / k + 1) / gamma(n_mod_k / k + 1) * max(n_mod_k, 1)
...
def _factorialx_wrapper(fname, n, k, exact, extend):
    ...
    if extend not in ("zero", "complex"):
        raise ValueError(f"argument `extend` must be either 'zero' or 'complex', received: {extend}")
    ...
    if fname == "factorial2":
        ...
        msg_needs_complex += (
            " Additionally, it will perturb the values of the" " multifactorial at most positive integers `n`."
        )
        # check type of k
