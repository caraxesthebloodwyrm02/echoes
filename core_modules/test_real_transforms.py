...
class _TestDCTBase:
    ...
    def test_definition(self, dct_lock):
        for i in FFTWDATA_SIZES:
            ...
            assert_array_almost_equal(y / np.max(y), yr / np.max(y), decimal=self.dec, err_msg=f"Size {i} failed")
...
class _TestIDCTBase:
    ...
    def test_definition(self, idct_lock):
        for i in FFTWDATA_SIZES:
            ...
            # difference is due to fftw using a better algorithm w.r.t error
            # propagation compared to the ones from fftpack.
            assert_array_almost_equal(x / np.max(x), xr / np.max(x), decimal=self.dec, err_msg=f"Size {i} failed")
...
class TestOverwrite:
    ...
    def _check(self, x, routine, type, fftsize, axis, norm, overwrite_x, **kw):
        ...
        sig = f"{routine.__name__}({x.dtype}{x.shape!r}, {fftsize!r}, " f"axis={axis!r}, overwrite_x={overwrite_x!r})"
        if not overwrite_x:
