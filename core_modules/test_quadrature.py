...
class TestQMCQuad:
    ...
    def basic_test(self, n_points=2**8, n_estimates=8, signs=None):
        ...
        qrng = stats.qmc.Sobol(ndim, seed=rng)
        logres = qmc_quad(
            lambda *args: np.log(func(*args)), a, b, n_points=n_points, n_estimates=n_estimates, log=True, qrng=qrng
        )
...
class TestCumulativeSimpson:
    ...
    def test_cumulative_simpson_against_simpson_with_default_dx(self, y):
        ...
        np.testing.assert_allclose(res[..., 1:], ref[..., 1:] + theoretical_difference[..., 1:], atol=1e-16)
