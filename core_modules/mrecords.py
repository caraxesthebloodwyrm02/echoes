    ...
    def __array_finalize__(self, obj):
        ...
        if _mask is None:
            ...
            if objmask is ma.nomask:
                ...
                _mask = np.array([tuple([m] * len(mdescr)) for m in objmask], dtype=mdescr).view(np.recarray)
...
def fromtextfile(
    ...
    _datalist = [
        ma.masked_array(a, mask=m, dtype=t, fill_value=f) for (a, m, t, f) in zip(_variables.T, _mask, vartypes, mfillv)
    ]
