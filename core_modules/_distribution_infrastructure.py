    ...
    def draw(self, sizes=None, rng=None, proportions=None, region="domain"):
        ...
        for size, param in zip(sizes, self.parameters.values()):
            parameter_values[param.name] = param.draw(
                size, rng=rng, proportions=proportions, parameter_values=parameter_values, region=region
            )
...
def _set_invalid_nan(f):
    ...
    def filtered(self, x, *args, **kwargs):
        ...
        mask_low = x < low if (method_name in replace_strict and left_inc) or keep_low_endpoint else x <= low
        ...
        if discrete and method_name in replace_non_integral:
            ...
            any_non_integral = mask_non_integral if mask_non_integral.shape == () else np.any(mask_non_integral)
        ...
        if any_endpoint:
            ...
            replace_low_endpoint = b[mask_low_endpoint] if method_name.endswith("ccdf") else a[mask_low_endpoint]
            replace_high_endpoint = a[mask_high_endpoint] if method_name.endswith("ccdf") else b[mask_high_endpoint]
...
class UnivariateDistribution(_ProbabilityDistribution):
    ...
    def _broadcast(self, parameters):
        ...
        if len(parameter_shapes) == 1:
            return (parameters, parameter_vals[0].shape, parameter_vals[0].size, parameter_vals[0].ndim)
    ...
    def tol(self, tol):
        ...
        tol = np.asarray(tol)
        if tol.shape != () or not tol > 0 or not np.issubdtype(tol.dtype, np.floating):  # catches NaNs
            message = f"Attribute `tol` of `{self.__class__.__name__}` must " "be a positive float, if specified."
            raise ValueError(message)
        self._tol = tol[()]
    ...
    def validation_policy(self, validation_policy):
        validation_policy = str(validation_policy).lower() if validation_policy is not None else None
    ...
    def _validate_order_kind(self, order, kind, kinds):
        ...
        message = f"Argument `order` of `{self.__class__.__name__}.moment` " "must be a finite, positive integer."
        try:
        ...
        message = f"Argument `kind` of `{self.__class__.__name__}.moment` " f"must be one of {set(kinds)}."
    ...
    def _num_parameters(cls, i_parameterization=0):
        ...
        # parameterization.
        return 0 if not cls._num_parameterizations() else len(cls._parameterizations[i_parameterization])
    ...
    def _mode_optimization(self, xatol=None, **params):
        ...
        f, args = _kwargs2args(lambda x, **params: -self._pxf_dispatch(x, **params), args=(), kwargs=params)
    ...
    def kurtosis(self, *, method=None, convention="non-excess"):
        ...
        message = f"Parameter `convention` of `{self.__class__.__name__}.kurtosis` " f"must be one of {conventions}."
    ...
    def _logcdf2_logexp_safe(self, x, y, **params):
        ...
        mask = np.isinf(out.real)
        if np.any(mask):
            params_mask = {key: np.broadcast_to(val, mask.shape)[mask] for key, val in params.items()}
            out = np.asarray(out)
    ...
    def moment(self, order=1, kind="raw", *, method=None):
        kinds = {"raw": self._moment_raw, "central": self._moment_central, "standardized": self._moment_standardized}
    ...
    def plot(self, x="x", y=None, *, t=None, ax=None):
        ...
        message = "To use `plot`, distribution parameters must be " "scalars or arrays with one or fewer dimensions."
        if ndim > 1:
            raise ValueError(message)
        ...
        if len(self._parameters):
            ...
            for param_vals in zip(*param_arrays):
                assignments = [f"${parameters[name].symbol}$ = {val:.4g}" for name, val in zip(param_names, param_vals)]
...
class DiscreteDistribution(UnivariateDistribution):
    ...
    def _cdf2(self, x, y, *, method):
        raise NotImplementedError(
            "Two argument cdf functions are currently only supported for " "continuous distributions."
        )
...
def _make_distribution_rv_generic(dist):
    ...
    def _overrides(method_name):
        return getattr(dist.__class__, method_name, None) is not getattr(old_class, method_name, None)
...
class ShiftedScaledDistribution(TransformedDistribution):
    ...
    _scale_param = _RealParameter("scale", symbol=r"\sigma", domain=_scale_domain, typical=(0.1, 10))
    ...
    def _input_validation(self, components, weights):
        ...
        for var in components:
            ...
            if not isinstance(var, ContinuousDistribution):
                message = "Each element of `components` must be an instance of " "`ContinuousDistribution`."
                raise ValueError(message)
            if not var._shape == ():
...
class MonotonicTransformedDistribution(TransformedDistribution):
    ...
    def __init__(
        self, X, /, *args, g, h, dh, logdh=None, increasing=True, repr_pattern=None, str_pattern=None, **kwargs
    ):
