...
class TestComparisonsMixed1(TestComparisons):
    ...
    def setup_method(self):
        ...
        self.B = np.array([["efg", "efg", "123  "], ["051", "efgg", "tuv"]], np.str_).view(np.char.chararray)
...
class TestComparisonsMixed2(TestComparisons):
    ...
    def setup_method(self):
        TestComparisons.setup_method(self)
        self.A = np.array([["abc", "abcc", "123"], ["789", "abc", "xyz"]], np.str_).view(np.char.chararray)


class TestInformation:
    def setup_method(self):
        self.A = np.array([[" abc ", ""], ["12345", "MixedCase"], ["123 \t 345 \0 ", "UPPER"]]).view(np.char.chararray)
        self.B = np.array([[" \u03a3 ", ""], ["12345", "MixedCase"], ["123 \t 345 \0 ", "UPPER"]]).view(
            np.char.chararray
        )
        # Array with longer strings, > MEMCHR_CUT_OFF in code.
        self.C = np.array(["ABCDEFGHIJKLMNOPQRSTUVWXYZ", "01234567890123456789012345"]).view(np.char.chararray)
...
class TestMethods:
    ...
    def test_replace_count_and_size(self):
        ...
        assert_array_equal(
            r5, np.array(["01234ABCDE6789" * i for i in range(3)] + ["01234ABCDE6789" + "0123456789" * 2])
        )
...
class TestOperations:
    ...
    @pytest.mark.parametrize("data", [["plate", "   ", "shrimp"], [b"retro", b"  ", b"encabulator"]])
...
class TestMethodsScalarValues:
    def test_mod(self):
        A = np.array([[" abc ", ""], ["12345", "MixedCase"], ["123 \t 345 \0 ", "UPPER"]], dtype="S")
        tgt = [[b"123 abc ", b"123"], [b"12312345", b"123MixedCase"], [b"123123 \t 345 \0 ", b"123UPPER"]]
    ...
    def test_expandtabs(self):
        ...
        assert_equal(np.char.expandtabs(s, tabsize=2), "  one level of indentation\n    two levels of indentation")
