...
parametrize_rgi_interp_methods = pytest.mark.parametrize("method", RegularGridInterpolator._ALL_METHODS)
...
class TestRegularGridInterpolator:
    ...
    def test_length_one_axis(self):
        ...
        xp_assert_close(interp(np.array([[1, 1], [1, 5], [1, 10]])), np.asarray([2.0, 6, 11]), atol=1e-14)
    ...
    def test_concurrency(self):
        points, values = self._get_sample_4d()
        sample = np.array([[0.1, 0.1, 1.0, 0.9], [0.2, 0.1, 0.45, 0.8], [0.5, 0.5, 0.5, 0.5], [0.3, 0.1, 0.2, 0.4]])
        interp = RegularGridInterpolator(points, values, method="slinear")
