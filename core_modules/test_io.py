...
class LoadTxtBase:
    ...
    def test_converters_decode(self):
        ...
        c.write(b"\xcf\x96")
        c.seek(0)
        x = self.loadfunc(c, dtype=np.str_, encoding="bytes", converters={0: lambda x: x.decode("UTF-8")})
...
class TestLoadTxt(LoadTxtBase):
    ...
    def test_usecols(self):
        ...
        bogus_idx = 1.5
        assert_raises_regex(
            TypeError, f"^usecols must be.*{type(bogus_idx).__name__}", np.loadtxt, c, usecols=bogus_idx
        )

        assert_raises_regex(
            TypeError, f"^usecols must be.*{type(bogus_idx).__name__}", np.loadtxt, c, usecols=[0, bogus_idx, 0]
        )
...
class TestFromTxt(LoadTxtBase):
    ...
    def test_skip_footer(self):
        ...
        ctrl = np.array([(f"{i:f}", f"{i:f}", f"{i:f}") for i in range(41)], dtype=[(_, float) for _ in "ABC"])
...
M   33  21.99
    ...
    def test_autonames_and_usecols(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            warnings.filterwarnings("always", "", VisibleDeprecationWarning)
            test = np.genfromtxt(data, usecols=("A", "C", "D"), names=True, dtype=None, encoding="bytes")
            assert_(w[0].category is VisibleDeprecationWarning)
    ...
    def test_invalid_converter(self):
        ...
        kwargs = {"converters": {2: strip_per, 3: strip_rand}, "delimiter": ",", "dtype": None, "encoding": "bytes"}
    ...
    def test_utf8_userconverters_with_explicit_dtype(self):
        ...
        with temppath() as path:
            ...
            test = np.genfromtxt(
                path, delimiter=",", names=None, dtype=float, usecols=(2, 3), converters={2: str}, encoding="UTF-8"
            )
    ...
    def test_inconsistent_dtype(self):
        ...
        kwargs = {"delimiter": ",", "converters": converters, "dtype": [(_, int) for _ in "abcde"], "encoding": "bytes"}
    ...
    def test_comments_is_none(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            warnings.filterwarnings("always", "", VisibleDeprecationWarning)
            test = np.genfromtxt(
                TextIO("test1,testNonetherestofthedata"), dtype=None, comments=None, delimiter=",", encoding="bytes"
            )
            assert_(w[0].category is VisibleDeprecationWarning)
    ...
    def test_latin1(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            warnings.filterwarnings("always", "", VisibleDeprecationWarning)
            test = np.genfromtxt(TextIO(s), dtype=None, comments=None, delimiter=",", encoding="bytes")
            assert_(w[0].category is VisibleDeprecationWarning)
        ...
        with warnings.catch_warnings(record=True) as w:
            warnings.filterwarnings("always", "", VisibleDeprecationWarning)
            test = np.genfromtxt(
                TextIO(b"0,testNonethe" + latin1), dtype=None, comments=None, delimiter=",", encoding="bytes"
            )
            assert_(w[0].category is VisibleDeprecationWarning)
    ...
    def test_utf8_file_nodtype_unicode(self):
        ...
        with temppath() as path:
            ...
            with warnings.catch_warnings(record=True) as w:
                warnings.filterwarnings("always", "", VisibleDeprecationWarning)
                test = np.genfromtxt(path, dtype=None, comments=None, delimiter=",", encoding="bytes")
    ...
    def test_recfromcsv(self):
        #
        data = TextIO("A,B\n0,1\n2,3")
        kwargs = {"missing_values": "N/A", "names": True, "case_sensitive": True, "encoding": "bytes"}
