...
def test_internal_cpp_api() -> None:
    ...
    with pytest.raises(ValueError, match=r"z must be a 1D array with the same length as the x and y arrays"):
...
def test_tricontourf_path():
    ...
    paths = cs.get_paths()
    assert len(paths) == 1
    expected_vertices = [[0, 0], [4, 0], [4, 4], [0, 4], [0, 0], [1, 1], [1, 3], [3, 3], [3, 1], [1, 1]]
    assert_array_almost_equal(paths[0].vertices, expected_vertices)
