...
def bisplrep(
    ...
    lwrk1 = _int_overflow(
        u * v * (2 + b1 + b2) + 2 * (u + v + km * (m + ne) + ne - kx - ky) + b2 + 1, OverflowError, msg=msg
    )
...
def splder(tck, n=1):
    ...
    if n > k:
        raise ValueError(f"Order of derivative (n = {n!r}) must be <= " f"order of spline (k = {tck[2]!r})")
    ...
    with np.errstate(invalid="raise", divide="raise"):
        try:
            ...
            raise ValueError("The spline has internal repeated knots " f"and is not differentiable {n} times") from e
