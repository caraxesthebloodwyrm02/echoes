...
def _monte_carlo_test_iv(data, rvs, statistic, vectorized, n_resamples, batch, alternative, axis):
    ...
    if vectorized is None:
        try:
            ...
            message = f"Signature inspection of {statistic=} failed; " "pass `vectorize` explicitly."
            raise ValueError(message) from e
    ...
    return (data_iv, rvs, statistic_vectorized, vectorized, n_resamples_int, batch_iv, alternative, axis_int, dtype, xp)
...
def monte_carlo_test(
    data, rvs, statistic, *, vectorized=None, n_resamples=9999, batch=None, alternative="two-sided", axis=0
):
    ...
    for k in range(0, n_resamples, batch_nominal):
        batch_actual = min(batch_nominal, n_resamples - k)
        resamples = [
            rvs_i(size=(batch_actual, n_observations_i)) for rvs_i, n_observations_i in zip(rvs, n_observations)
        ]
    ...
    eps = 0 if not xp.isdtype(observed.dtype, ("real floating")) else xp.finfo(observed.dtype).eps * 100
...
def _power_iv(rvs, test, n_observations, significance, vectorized, n_resamples, batch, kwargs):
    ...
    if not np.issubdtype(significance.dtype, np.floating) or np.min(significance) < 0 or np.max(significance) > 1:
...
def power(test, rvs, n_observations, *, significance=0.01, vectorized=None, n_resamples=10000, batch=None, kwargs=None):
    ...
    *alternative* hypothesis and calculate the power.
    ...
    for nobs_i, args_i in zip(nobs, args):
        ...
        for k in range(0, n_resamples, batch_nominal):
            batch_actual = min(batch_nominal, n_resamples - k)
            resamples = [rvs_j(size=(batch_actual, nobs_ij), **kwargs_i) for rvs_j, nobs_ij in zip(rvs, nobs_i)]
...
def _permutation_test_iv(data, statistic, permutation_type, vectorized, n_resamples, batch, alternative, axis, rng):
    ...
    return (data_iv, statistic, permutation_type, vectorized, n_resamples_int, batch_iv, alternative, axis_int, rng)
...
def permutation_test(
    ...
    *difference* between two paired elements. Accordingly, if ``data`` contains
    ...
    (data, statistic, permutation_type, vectorized, n_resamples, batch, alternative, axis, rng) = args
