...
xslow_test_moments = {"studentized_range", "ksone", "vonmises", "vonmises_line", "recipinvgauss", "kstwo", "kappa4"}

slow_fit_mle = {"exponweib", "genexpon", "genhyperbolic", "johnsonsb", "kappa4", "powerlognorm", "tukeylambda"}
...
def test_cont_basic_fit_cases():
    ...
    assert len(xslow_fit_mle.union(xfail_fit_mle).union(skip_fit_mle)) == len(xslow_fit_mle) + len(xfail_fit_mle) + len(
        skip_fit_mle
    )
    assert len(xslow_fit_mm.union(xfail_fit_mm).union(skip_fit_mm)) == len(xslow_fit_mm) + len(xfail_fit_mm) + len(
        skip_fit_mm
    )
...
def check_ppf_isf(distfn, arg, msg):
    p = np.array([0.1, 0.9])
    npt.assert_almost_equal(
        distfn.isf(p, *arg), distfn.ppf(1 - p, *arg), decimal=DECIMAL, err_msg=msg + " - ppf-isf relationship"
    )
