...
class Mapper(
    ...
    def _configure_pks(self) -> None:
        ...
        for fc in set(self.tables).union([self.persist_selectable]):
            ...
            self._cols_by_table[fc] = util.ordered_column_set(fc.c).intersection(all_cols)  # type: ignore  # noqa: E501
    ...
    def _persistent_sortkey_fn(self):
        ...
        if set(key_fns).difference([None]):
            ...
            def key(state):
                return tuple(key_fn(val) if key_fn is not None else val for key_fn, val in zip(key_fns, state.key[1]))
