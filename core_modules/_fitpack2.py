...
class UnivariateSpline:
    ...
    **NaN handling**: If the input arrays contain ``nan`` values, the result
    ...
    def set_smoothing_factor(self, s):
        ...
        data = self._data
        if data[6] == -1:
            warnings.warn("smoothing factor unchanged for" "LSQ spline with fixed knots", stacklevel=2)
            return
...
class RectBivariateSpline(BivariateSpline):
    ...
    def __init__(self, x, y, z, bbox=[None] * 4, kx=3, ky=3, s=0, maxit=20):
        ...
        xb, xe, yb, ye = bbox
        with FITPACK_LOCK:
            nx, tx, ny, ty, c, fp, ier = dfitpack.regrid_smth(x, y, z, xb, xe, yb, ye, kx, ky, s, maxit)
