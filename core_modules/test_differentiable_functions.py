...
class TestScalarFunction(TestCase):
    ...
    def test_workers(self):
        ...
        with MapWrapper(2) as mapper:
            approx = ScalarFunction(ex.fun, x0, (), "2-point", ex.hess, None, (-np.inf, np.inf), workers=mapper)
            ...
            ex = ExScalarFunction()
            ex2 = ExScalarFunction()
            approx = ScalarFunction(ex.fun, x0, (), "3-point", ex.hess, None, (-np.inf, np.inf), workers=mapper)
            approx_series = ScalarFunction(
                ex2.fun,
                x0,
                (),
            ...
            approx = ScalarFunction(ex.fun, x0, (), ex.grad, "3-point", None, (-np.inf, np.inf), workers=mapper)
            approx_series = ScalarFunction(
                ex2.fun,
                x0,
                (),
...
class ExVectorialFunction:
    ...
    def fun(self, x):
        self.nfev += 1
        return np.array(
            [2 * (x[0] ** 2 + x[1] ** 2 - 1) - x[0], 4 * (x[0] ** 3 + x[1] ** 2 - 4) - 3 * x[0]], dtype=x.dtype
        )
...
class TestVectorialFunction(TestCase):
    ...
    def test_workers(self):
        ...
        with MapWrapper(2) as mapper:
            ...
            approx_series = VectorFunction(ex2.fun, x0, "2-point", ex2.hess, None, None, (-np.inf, np.inf), False)
            ...
            approx_series = VectorFunction(ex2.fun, x0, "3-point", ex2.hess, None, None, (-np.inf, np.inf), False)
            assert_allclose(approx.jac(x0), ex.jac(x0))
            assert_allclose(approx_series.jac(x0), ex.jac(x0))
            assert_allclose(approx_series.hess(x0, v), ex.hess(x0, v))
            assert_allclose(approx.hess(x0, v), ex.hess(x0, v))
            ...
            ex.nfev = ex.njev = ex.nhev = 0
            ex2.nfev = ex2.njev = ex2.nhev = 0
            approx = VectorFunction(ex.fun, x0, ex.jac, "2-point", None, None, (-np.inf, np.inf), False, workers=mapper)
            approx_series = VectorFunction(ex2.fun, x0, ex2.jac, "2-point", None, None, (-np.inf, np.inf), False)
            assert_allclose(approx.jac(x0), ex.jac(x0))
            assert_allclose(approx_series.jac(x0), ex.jac(x0))
    ...
    def test_sparse_analytic_jac(self):
        ...
        vf1 = VectorFunction(ex.fun, x0, ex.jac, ex.hess, None, None, (-np.inf, np.inf), sparse_jacobian=None)
        # jac(x) returns sparse jacobian, but sparse_jacobian=False requests dense
        vf2 = VectorFunction(
            ex.fun, x0, sparse_adapter(ex.jac), ex.hess, None, None, (-np.inf, np.inf), sparse_jacobian=False
        )
    ...
    def test_sparse_numerical_jac(self):
        ...
        vf1 = VectorFunction(ex.fun, x0, "2-point", ex.hess, None, None, (-np.inf, np.inf), sparse_jacobian=None)
        # use sparse numerical difference, but ask it to be converted to dense
        finite_diff_jac_sparsity = csr_array(np.ones((N, N)))
        vf2 = VectorFunction(
            ex.fun, x0, "2-point", ex.hess, None, finite_diff_jac_sparsity, (-np.inf, np.inf), sparse_jacobian=False
        )
...
@pytest.mark.skipif(platform.python_implementation() == "PyPy", reason="assert_deallocate not available on PyPy")
def test_ScalarFunctionNoReferenceCycle():
    """Regression test for gh-20768."""
    ex = ExScalarFunction()
    x0 = np.zeros(3)
    with assert_deallocated(lambda: ScalarFunction(ex.fun, x0, (), ex.grad, ex.hess, None, (-np.inf, np.inf))):
        pass
...
def test_VectorFunctionNoReferenceCycle():
    ...
    ex = ExVectorialFunction()
    x0 = [1.0, 0.0]
    with assert_deallocated(lambda: VectorFunction(ex.fun, x0, ex.jac, ex.hess, None, None, (-np.inf, np.inf), None)):
        pass
