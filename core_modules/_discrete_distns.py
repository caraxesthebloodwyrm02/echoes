    ...
    def _stats(self, n, a, b, moments="mv"):
        ...
        def kurtosis(n, a, b):
            ...
            term_2 = (
                ...
                + 3.0 * n**2.0 * ((a + 5.0) * b**2.0 + (a + 5.0) * (a - 1.0) * b + 2.0 * (a - 1.0) ** 2)
                + 3 * (a - 1.0) * n * ((a + 5.0) * b**2.0 + (a + 5.0) * (a - 1.0) * b + 2.0 * (a - 1.0) ** 2.0)
            )
...
class zipf_gen(rv_discrete):
    ...
    def _munp(self, n, a):
        return xpx.apply_where(
            a > n + 1, (a, n), lambda a, n: special.zeta(a - n, 1) / special.zeta(a, 1), fill_value=np.inf
        )
...
class skellam_gen(rv_discrete):
    ...
    def _cdf(self, x, mu1, mu2):
        ...
        with np.errstate(over="ignore"):  # see gh-17432
            px = np.where(
                x < 0, scu._ncx2_cdf(2 * mu2, -2 * x, 2 * mu1), 1 - scu._ncx2_cdf(2 * mu1, 2 * (x + 1), 2 * mu2)
            )
        return px
...
class yulesimon_gen(rv_discrete):
    ...
    def _stats(self, alpha):
        ...
        g2 = np.where(
            alpha > 4, alpha + 3 + ((11 * alpha**3 - 49 * alpha - 22) / (alpha * (alpha - 4) * (alpha - 3))), np.inf
        )
