...
def test_kde_1d():
    ...
    assert_allclose(gkde(xx), stats.norm.pdf(xx[:, None], loc=loc, scale=scale).sum(axis=-1) / gkde.n, rtol=5e-14)
...
def test_kde_1d_weighted():
    ...
    pdf = stats.norm.pdf
    assert_allclose(gkde(xx), np.sum(pdf(xx[:, None], loc=loc, scale=scale) * gkde.weights, axis=-1), rtol=5e-14)
...
def test_kde_2d_weighted(n_basesample):
    ...
    arg = xx[:, None, :] - gkde.dataset.T
    pdf = stats.multivariate_normal.pdf
    assert_allclose(gkde(xx.T), np.sum(pdf(arg, cov=gkde.covariance) * gkde.weights, axis=-1), rtol=5e-14)
