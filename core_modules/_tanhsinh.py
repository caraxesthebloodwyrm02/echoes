...
def tanhsinh(
    f, a, b, *, args=(), log=False, maxlevel=None, minlevel=2, atol=None, rtol=None, preserve_shape=False, callback=None
):
    ...
    (f, a, b, log, maxfun, maxlevel, minlevel, atol, rtol, args, preserve_shape, callback, xp) = _tanhsinh_iv(
    ...
    def pre_func_eval(work):
        ...
        work.h = h0 / 2**work.n
        xjc, wj = _get_pairs(work.n, h0, dtype=work.dtype, inclusive=(work.n == minlevel), xp=xp, work=work)
    ...
    def post_func_eval(x, fj, work):
        ...
        if work.log:
            fj[work.abinf] += xp.log(1 + work.xj[work.abinf] ** 2) - 2 * xp.log(1 - work.xj[work.abinf] ** 2)
        ...
        if work.Sk.shape[-1]:
            Snm1 = work.Sk[:, -1]
            Sn = special.logsumexp(xp.stack([Snm1 - math.log(2), Sn]), axis=0) if log else Snm1 / 2 + Sn
...
def _euler_maclaurin_sum(fj, work, xp):
    ...
    Sn = special.logsumexp(fjwj + xp.log(work.h), axis=-1) if work.log else xp.sum(fjwj, axis=-1) * work.h
...
def _estimate_error(work, xp):
    ...
    if work.Sk.shape[-1] == 0:
        ...
        Snm1 = special.logsumexp(fjwj, **axis_kwargs) + xp.log(h) if work.log else xp.sum(fjwj, **axis_kwargs) * h
    ...
    if work.Sk.shape[-1] < 2:
        ...
        fjwj = xp.reshape(fjwj_rl[..., :n_x], (n_active, 2 * n_x))
        # Compute the Euler-Maclaurin sum at this level
        Snm2 = special.logsumexp(fjwj, **axis_kwargs) + xp.log(h) if work.log else xp.sum(fjwj, **axis_kwargs) * h
...
def _tanhsinh_iv(f, a, b, log, maxfun, maxlevel, minlevel, atol, rtol, args, preserve_shape, callback):
    ...
    return (f, a, b, log, maxfun, maxlevel, minlevel, atol, rtol, args, preserve_shape, callback, xp)
