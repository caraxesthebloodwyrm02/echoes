...
def null_space(A, rcond=None, *, overwrite_a=False, check_finite=True, lapack_driver="gesdd"):
    ...
    """
    u, s, vh = svd(
        A, full_matrices=True, overwrite_a=overwrite_a, check_finite=check_finite, lapack_driver=lapack_driver
    )
    M, N = u.shape[0], vh.shape[1]
    if rcond is None:
        rcond = np.finfo(s.dtype).eps * max(M, N)
...
def subspace_angles(A, B):
    ...
    if len(B) != len(QA):
        raise ValueError("A and B must have the same number of rows, got " f"{QA.shape[0]} and {B.shape[0]}")
    QB = orth(B)
    del B
