...
def minimize(
    ...
    **Custom minimizers**
    ...
    if meth in ("nelder-mead", "powell", "cobyla", "cobyqa") and bool(jac):
        warn(f"Method {method} does not use gradient information (jac).", RuntimeWarning, stacklevel=2)
    # - hess
    if (
        meth not in ("newton-cg", "dogleg", "trust-ncg", "trust-constr", "trust-krylov", "trust-exact", "_custom")
        and hess is not None
    ):
        warn(f"Method {method} does not use Hessian information (hess).", RuntimeWarning, stacklevel=2)
    # - hessp
    if meth not in ("newton-cg", "trust-ncg", "trust-constr", "trust-krylov", "_custom") and hessp is not None:
        warn(
            f"Method {method} does not use Hessian-vector product" " information (hessp).", RuntimeWarning, stacklevel=2
        )
    ...
    if (
        meth not in ("nelder-mead", "powell", "l-bfgs-b", "cobyla", "cobyqa", "slsqp", "tnc", "trust-constr", "_custom")
        and bounds is not None
    ):
        warn(f"Method {method} cannot handle bounds.", RuntimeWarning, stacklevel=2)
    # - return_all
    if meth in ("l-bfgs-b", "tnc", "cobyla", "cobyqa", "slsqp") and options.get("return_all", False):
        warn(f"Method {method} does not support the return_all option.", RuntimeWarning, stacklevel=2)
