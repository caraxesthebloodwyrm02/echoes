...
# Tests gh-8908
@image_comparison(
    ["plot_masked_units.png"], remove_text=True, style="mpl20", tol=0 if platform.machine() == "x86_64" else 0.02
)
    ...
    def __array__(self, dtype=None, copy=None):
        if dtype is not None and dtype != self._array.dtype:
            if copy is not None and not copy:
                raise ValueError(f"Converting array from {self._array.dtype} to " f"{dtype} requires a copy")
