...
class FigureBase(Artist):
    ...
    def set_figure(self, fig):
        ...
        if fig is self._root_figure:
            _api.warn_deprecated(
                "3.10", message=(f"{no_switch} From Matplotlib 3.12 this operation will raise " "an exception.")
            )
            return
    ...
    figure = property(
        ...
        set_figure,
        doc=("The root `Figure`.  To get the parent of a `SubFigure`, " "use the `get_figure` method."),
    )
...
default: %(va)s
    ...
    @_docstring.Substitution(x0=0.5, y0=0.01, name="super xlabel", ha="center", va="bottom", rc="label")
    @_docstring.copy(_suplabels)
    ...
    @_docstring.Substitution(x0=0.02, y0=0.5, name="super ylabel", ha="left", va="center", rc="label")
    @_docstring.copy(_suplabels)
...
'polar', 'rectilinear', str}, optional
    ...
    def colorbar(self, mappable, cax=None, ax=None, use_gridspec=True, **kwargs):
        ...
        if hasattr(mappable, "get_figure") and (mappable_host_fig := mappable.get_figure(root=True)) is not None:
...
class SubFigure(FigureBase):
    ...
    def draw(self, renderer):
        ...
        try:
            ...
            mimage._draw_list_compositing_images(renderer, self, artists, self.get_figure(root=True).suppressComposite)
    ...
    def figimage(
        ...
        **kwargs
        ...
        im = mimage.FigureImage(
            self, cmap=cmap, norm=norm, colorizer=colorizer, offsetx=xo, offsety=yo, origin=origin, **kwargs
        )
