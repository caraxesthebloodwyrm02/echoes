    ...
    if (output_shape is not None) and (shape(res) != output_shape):
        if output_shape[0] != 1:
            if len(output_shape) > 1:
                if output_shape[1] == 1:
                    return shape(res)
            msg = f"{checker}: there is a mismatch between the input and output " f"shape of the '{argname}' argument"
...
def _root_hybr(
    ...
    **unknown_options,
    ...
    errors = {
        ...
        3: f"xtol={xtol:f} is too small, no further improvement " "in the approximate\n solution is possible.",
...
def leastsq(
    ...
    errors = {
        ...
        1: ["Both actual and predicted relative reductions " f"in the sum of squares\n  are at most {ftol:f}", None],
        ...
        6: [f"ftol={ftol:f} is too small, no further reduction " "in the sum of squares\n  is possible.", ValueError],
        7: [
            f"xtol={xtol:f} is too small, no further improvement in " "the approximate\n  solution is possible.",
            ValueError,
        ],
        8: [
