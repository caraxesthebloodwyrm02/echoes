...
def transform_class_def(builder: IRBuilder, cdef: ClassDef) -> None:
    ...
    for stmt in cdef.defs.body:
        if isinstance(stmt, (FuncDef, Decorator, OverloadedFuncDef)) and stmt.name == GENERATOR_HELPER_NAME:
            builder.error(f'Method name "{stmt.name}" is reserved for mypyc internal use', stmt.line)
        ...
        elif isinstance(stmt, PassStmt) or (isinstance(stmt, ExpressionStmt) and isinstance(stmt.expr, EllipsisExpr)):
            continue
...
class ExtClassBuilder(ClassBuilder):
    ...
    def add_attr(self, lvalue: NameExpr, stmt: AssignmentStmt) -> None:
        ...
        self.builder.primitive_op(py_setattr_op, [typ, self.builder.load_str(lvalue.name), value], stmt.line)
...
def setup_non_ext_dict(builder: IRBuilder, cdef: ClassDef, metaclass: Value, bases: Value) -> Value:
    ...
    has_prepare = builder.primitive_op(py_hasattr_op, [metaclass, builder.load_str("__prepare__")], cdef.line)
...
def gen_glue_ne_method(builder: IRBuilder, cls: ClassIR, line: int) -> None:
    ...
    with builder.enter_method(cls, "__ne__", eq_sig.ret_type):
        ...
        if not strict_typing or can_return_not_implemented:
            ...
            retval = builder.coerce(builder.builder.unary_not(eqval, line, likely_bool=True), rettype, line)
