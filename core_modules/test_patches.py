...
@image_comparison(["wedge_range"], remove_text=True, tol=0 if platform.machine() == "x86_64" else 0.009)
...
@image_comparison(
    ["connection_patch.png"], style="mpl20", remove_text=True, tol=0 if platform.machine() == "x86_64" else 0.024
)
...
def test_connection_patch_pixel_points(fig_test, fig_ref):
    ...
    ax1, ax2 = fig_test.subplots(1, 2)
    con = mpatches.ConnectionPatch(
        xyA=xyA_pts, coordsA="axes points", axesA=ax1, xyB=xyB_pts, coordsB="figure points", arrowstyle="->", shrinkB=5
    )
    fig_test.add_artist(con)
    ...
    con = mpatches.ConnectionPatch(
        xyA=xyA_pix, coordsA="axes pixels", axesA=ax1, xyB=xyB_pix, coordsB="figure pixels", arrowstyle="->", shrinkB=5
    )
    fig_ref.add_artist(con)
...
def test_arrow_set_data():
    ...
    expected1 = np.array(
        [[1.9, 0.0], [2.1, -0.0], [2.1, 8.0], [2.3, 8.0], [2.0, 10.0], [1.7, 8.0], [1.9, 8.0], [1.9, 0.0]]
    )
    assert np.allclose(expected1, np.round(arrow.get_verts(), 2))

    expected2 = np.array(
        [[0.39, 0.04], [0.61, -0.04], [3.01, 6.36], [3.24, 6.27], [3.5, 8.0], [2.56, 6.53], [2.79, 6.44], [0.39, 0.04]]
    )
