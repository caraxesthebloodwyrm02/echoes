...
class TestTanhSinh:
    ...
    def test_accuracy(self, ref, case, xp):
        ...
        if distname in {"studentized_range", "levy_stable"} and not int(os.getenv("SCIPY_XSLOW", 0)):
    ...
    def test_preserve_shape(self, xp):
        ...
        def f(x, xp):
            return xp.stack([xp.stack([x, xp.sin(10 * x)]), xp.stack([xp.cos(30 * x), x * xp.sin(100 * x)])])
...
class TestNSum:
    ...
    def test_input_validation(self, xp):
        ...
        with np.errstate(all="ignore"):
            ...
            res = nsum(f, xp.asarray(1.0), xp.asarray(10.0), step=xp.asarray([xp.nan, -xp.inf, xp.inf, -1, 0]))
            assert xp.all((res.status == -1) & xp.isnan(res.sum) & xp.isnan(res.error) & ~res.success & res.nfev == 1)
    ...
    def test_integral(self, maxterms, xp):
        ...
        logres = nsum(logf, a, b_original, step=step, log=True, tolerances=dict(rtol=math.log(rtol)), maxterms=maxterms)
        xp_assert_close(xp.exp(logres.sum), res.sum)
    ...
    def test_infinite_bounds(self, log, xp):
        ...
        def f(x, a):
            return xp.log(xp.tanh(a / 2)) - a * xp.abs(x) if log else xp.tanh(a / 2) * xp.exp(-a * xp.abs(x))
