...
class multivariate_normal_gen(multi_rv_generic):
    ...
    def _process_parameters_psd(self, dim, mean, cov):
        ...
        if cov.ndim == 0:
            ...
            rows, cols = cov.shape
            if rows != cols:
                msg = "Array 'cov' must be square if it is two dimensional," f" but cov.shape = {str(cov.shape)}."
            else:
                msg = (
            ...
            raise ValueError(f"Array 'cov' must be at most two-dimensional, " f"but cov.ndim = {cov.ndim}")
...
class multivariate_normal_frozen(multi_rv_frozen):
    ...
    def cdf(self, x, *, lower_limit=None, rng=None):
        ...
        out = self._dist._cdf(
            x, self.mean, self.cov_object.covariance, self.maxpts, self.abseps, self.releps, lower_limit, rng
        )
        return _squeeze_output(out)
...
def _dirichlet_check_parameters(alpha):
    ...
    if np.min(alpha) <= 0:
        ...
        raise ValueError("Parameter vector 'a' must be one dimensional, " f"but a.shape = {alpha.shape}.")
    return alpha
...
def _dirichlet_check_input(alpha, x):
    ...
    if (np.abs(np.sum(x, 0) - 1.0) > 10e-10).any():
        raise ValueError(
            "The input vector 'x' must lie within the normal " f"simplex. but np.sum(x, 0) = {np.sum(x, 0)}."
        )
...
class wishart_gen(multi_rv_generic):
    ...
    def _process_parameters(self, df, scale):
        ...
        if scale.ndim == 0:
            ...
            raise ValueError(
                "Array 'scale' must be square if it is two dimensional," f" but scale.scale = {str(scale.shape)}."
            )
        elif scale.ndim > 2:
            raise ValueError(f"Array 'scale' must be at most two-dimensional, " f"but scale.ndim = {scale.ndim}")
    ...
    def _process_quantiles(self, x, dim):
        ...
        if x.ndim == 1:
            ...
            if not x.shape[0] == x.shape[1]:
                raise ValueError(
                    "Quantiles must be square if they are two dimensional," f" but x.shape = {str(x.shape)}."
                )
            x = x[:, :, np.newaxis]
        elif x.ndim == 3:
            if not x.shape[0] == x.shape[1]:
        ...
        if not x.shape[0:2] == (dim, dim):
            raise ValueError("Quantiles have incompatible dimensions: should" f" be {(dim, dim)}, got {x.shape[0:2]}.")
...
class multivariate_t_gen(multi_rv_generic):
    ...
    def cdf(
        self, x, loc=None, shape=1, df=1, allow_singular=False, *, maxpts=None, lower_limit=None, random_state=None
    ):
    ...
    def _process_parameters(self, loc, shape, df):
        ...
        if shape.ndim == 0:
            ...
            if rows != cols:
                msg = "Array 'cov' must be square if it is two dimensional," f" but cov.shape = {str(shape.shape)}."
            else:
                msg = "Dimension mismatch: array 'cov' is of shape %s," " but 'loc' is a vector of length %d."
                msg = msg % (str(shape.shape), len(loc))
            raise ValueError(msg)
        elif shape.ndim > 2:
            raise ValueError(f"Array 'cov' must be at most two-dimensional, " f"but cov.ndim = {shape.ndim}")
