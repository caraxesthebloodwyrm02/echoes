...
@pytest.mark.skip_xp_backends("array_api_strict", reason="Currently uses fancy indexing assignment.")
class TestChandrupatlaMinimize:
    ...
    def test_flags(self, xp):
        ...
        ref_flags = xp.asarray([eim._ECONVERGED, eim._ESIGNERR, eim._ECONVERR, eim._EVALUEERR], dtype=xp.int32)
    ...
    def test_nit_expected(self, case, xp):
        ...
        res = _chandrupatla_minimize(func, *bracket, xatol=xatol, fatol=fatol, xrtol=xrtol, frtol=frtol)
    ...
    def test_dtype(self, loc, dtype, xp):
        ...
        loc = xp.asarray(loc, dtype=dtype)
        bracket = (xp.asarray(-3, dtype=dtype), xp.asarray(1, dtype=dtype), xp.asarray(5, dtype=dtype))
...
class TestFindRoot:
    ...
    def test_vectorization(self, shape, xp):
        ...
        assert xp.all((res.x[finite] == res.bracket[0][finite]) | (res.x[finite] == res.bracket[1][finite]))
        ...
        assert xp.all(
            xp.abs(res.f_x[finite]) == xp.minimum(xp.abs(res.f_bracket[0][finite]), xp.abs(res.f_bracket[1][finite]))
        )
    ...
    def test_flags(self, xp):
        ...
        def f(xs, js):
            ...
            funcs = [lambda x: x - 2.5, lambda x: x - 10, lambda x: (x - 0.1) ** 3, lambda x: xp.nan]
    ...
    def test_maxiter_callback(self, xp):
        ...
        def callback(res):
            ...
            if callback.iter == 0:
                ...
                changed = ((res.bracket[0] == callback.bracket[0]) & (res.bracket[1] != callback.bracket[1])) | (
                    (res.bracket[0] != callback.bracket[0]) & (res.bracket[1] == callback.bracket[1])
                )
    ...
    def test_nit_expected(self, case, xp):
        ...
        bracket = (xp.asarray(bracket[0], dtype=xp.float64), xp.asarray(bracket[1], dtype=xp.float64))
