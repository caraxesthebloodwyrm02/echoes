...
def test_float_types(tp):
    ...
    if tp(1e16).itemsize > 4:
        assert_equal(str(tp(1e16)), str(float("1e16")), err_msg=f"Failed str formatting for type {tp}")
    else:
        ref = "1e+16"
...
def test_complex_types(tp):
    ...
    if tp(1e16).itemsize > 8:
        assert_equal(str(tp(1e16)), str(complex(1e16)), err_msg=f"Failed str formatting for type {tp}")
    else:
        ref = "(1e+16+0j)"
        assert_equal(str(tp(1e16)), ref, err_msg=f"Failed str formatting for type {tp}")
...
def test_scalar_format():
    ...
    for fmat, val, valtype in tests:
        try:
            assert_equal(fmat.format(val), fmat.format(valtype(val)), f"failed with val {val}, type {valtype}")
