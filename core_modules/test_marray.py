...
@pytest.mark.parametrize("fun, kwargs", [(stats.gmean, {}), (stats.hmean, {}), (stats.pmean, {"p": 2})])
...
@pytest.mark.parametrize(
    "lambda_",
    ["pearson", "log-likelihood", "freeman-tukey", "mod-log-likelihood", "neyman", "cressie-read", "chisquare"],
)
...
def test_length_nonmasked_marray_iterable_axis_raises():
    ...
    with pytest.raises(NotImplementedError, match="`axis` must be an integer or None for use with `MArray`"):
