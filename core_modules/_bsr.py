    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False, blocksize=None, *, maxprint=None):
        ...
        if issparse(arg1):
            ...
            if isshape(arg1):
                ...
                self.indptr, self.indices, self.data, self._shape = (bsr.indptr, bsr.indices, bsr.data, bsr._shape)
                ...
                if blocksize is not None:
                    ...
                    if tuple(blocksize) != self.data.shape[1:]:
                        raise ValueError(f"mismatching blocksize={blocksize}" f" vs {self.data.shape[1:]}")
            else:
        ...
        if full_check:
            ...
            if self.nnz > 0:
                if self.indices.max() >= N // C:
                    raise ValueError(f"column index values must be < {N//C}" f" (now max {self.indices.max()})")
