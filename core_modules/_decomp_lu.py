...
lapack_cast_dict = {x: "".join([y for y in "fdFD" if np.can_cast(x, y)]) for x in np.typecodes["All"]}
    ...
    **(If `permute_l` is ``True``)**
    ...
    if a1.dtype.char not in "fdFD":
        ...
        if not dtype_char:  # No casting possible
            raise TypeError(f"The dtype {a1.dtype} cannot be cast " "to float(32, 64) or complex(64, 128).")
