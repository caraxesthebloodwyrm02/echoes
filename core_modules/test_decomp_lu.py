...
class TestLU:
    ...
    def test_old_lu_smoke_tests(self):
        ...
        assert_allclose(
            u, np.array([[9, 10, 12, 12], [0, 8 / 9, 15 / 9, 24 / 9], [0, 0, -0.5, 0]]), rtol=0.0, atol=1e-14
        )
        ...
        assert_allclose(l, np.array([[1, 0, 0], [1 / 9, 1, 0], [5 / 9, 0.5, 1]]))
        assert_allclose(
            u, np.array([[9, 10, 12, 12], [0, 8 / 9, 15 / 9, 24 / 9], [0, 0, -0.5, 0]]) * 1j, rtol=0.0, atol=1e-14
        )
    ...
    @pytest.mark.parametrize(
        "shape", [[2, 2], [2, 4], [4, 2], [20, 20], [20, 4], [4, 20], [3, 2, 9, 9], [2, 2, 17, 5], [2, 2, 11, 7]]
    )
