    ...
    if kwargs.get("improve_tcg", True) and boundary_reached and n_act < n:
        ...
        while n_act < n:
            ...
            all_reduct = sin_values * (
                grad_step * t_samples
                - grad_sd
                - sin_values * (0.5 * t_samples**2.0 * curv_step - 2.0 * t_samples * curv_step_sd + 0.5 * curv_sd)
            )
            if np.all(all_reduct <= 0.0):
                # No reduction in the objective function is obtained.
                break
        ...
        # Ensure that the alternative iteration improves the objective
        # function.
        if grad_orig @ step + 0.5 * step @ hess_prod(step) > grad_orig @ step_base + 0.5 * step_base @ hess_prod(
            step_base
        ):
            step = step_base
...
def qr_tangential_byrd_omojokun(aub, aeq, free_xl, free_xu, free_ub):
    ...
    n_act = np.count_nonzero(
        np.abs(np.diag(r)) >= 10.0 * EPS * n * np.linalg.norm(r[: np.min(r.shape), : np.min(r.shape)], axis=0)
    )
    return n_act, q
...
def qr_normal_byrd_omojokun(aub, free_xl, free_xu, free_slack, free_ub):
    ...
    n_act = np.count_nonzero(
        np.abs(np.diag(r))
        >= 10.0 * EPS * (n + m_linear_ub) * np.linalg.norm(r[: np.min(r.shape), : np.min(r.shape)], axis=0)
    )
    return n_act, q
