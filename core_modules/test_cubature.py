...
def genz_malik_1980_f_3_exact(a, b, alphas, xp):
    ...
    return (-1) ** ndim * 1 / xp.prod(alphas, axis=-1) * xp.prod(xp.exp(alphas * a) - xp.exp(alphas * b), axis=-1)
...
def genz_malik_1980_f_4_exact(a, b, alphas, xp):
    ...
    def F(x):
        ...
        return (
            (-1) ** ndim / xp.prod(alphas, axis=-1) / math.factorial(ndim) / (1 + xp.sum(alphas * x_reshaped, axis=-1))
        )
