    ...
    def _matvec(self, x):
        ...
        for i in range(N):
            ...
            if self.boundary_conditions in ("neumann", "dirichlet"):
                Y[(slice(None),) * i + (0,) + (slice(None),) * (N - i - 1)] -= np.roll(X, 1, axis=i)[
                    (slice(None),) * i + (0,) + (slice(None),) * (N - i - 1)
                ]
                Y[(slice(None),) * i + (-1,) + (slice(None),) * (N - i - 1)] -= np.roll(X, -1, axis=i)[
                    (slice(None),) * i + (-1,) + (slice(None),) * (N - i - 1)
                ]

                if self.boundary_conditions == "neumann":
                    Y[(slice(None),) * i + (0,) + (slice(None),) * (N - i - 1)] += np.roll(X, 0, axis=i)[
                        (slice(None),) * i + (0,) + (slice(None),) * (N - i - 1)
                    ]
                    Y[(slice(None),) * i + (-1,) + (slice(None),) * (N - i - 1)] += np.roll(X, 0, axis=i)[
                        (slice(None),) * i + (-1,) + (slice(None),) * (N - i - 1)
                    ]
...
class Sakurai(LinearOperator):
    ...
    def tosparse(self):
        ...
        return diags_array(
            [d[0], d[1], d[2], d[1], d[0]], offsets=[-2, -1, 0, 1, 2], shape=(self.n, self.n), dtype=d.dtype
        )
...
class MikotaK(LinearOperator):
    ...
    def tosparse(self):
        ...
        return diags_array(
            [self._diag1, self._diag0, self._diag1], offsets=[-1, 0, 1], shape=self.shape, dtype=self.dtype
        )
