...
class Line3D(lines.Line2D):
    ...
    def draw(self, renderer):
        ...
        xs, ys, zs, tis = proj3d._proj_transform_clip(xs3d, ys3d, zs3d, self.axes.M, self.axes._focal_length)
        self.set_data(xs, ys)
        super().draw(renderer)
        self.stale = False
...
class Collection3D(Collection):
    ...
    def do_3d_projection(self):
        ...
        self._paths = [
            mpath.Path(np.ma.column_stack([xs, ys]), cs) for (xs, ys, _), (_, cs) in zip(xyzs_list, self._3dverts_codes)
        ]
...
def collection_2d_to_3d(col, zs=0, zdir="z", axlim_clip=False):
    ...
    col._3dverts_codes = [
        (
            np.column_stack(juggle_axes(*np.column_stack([p.vertices, np.broadcast_to(z, len(p.vertices))]).T, zdir)),
            p.codes,
        )
...
class Poly3DCollection(PolyCollection):
    ...
    def __init__(self, verts, *args, zsort="average", shade=False, lightsource=None, axlim_clip=False, **kwargs):
        """
        Parameters
        ----------
