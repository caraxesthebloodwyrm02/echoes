...
def linux_commands_wildcard_injection(context, config):
    ...
    if context.call_function_name_qual in config["shell"] or (
        context.call_function_name_qual in config["subprocess"] and context.check_call_arg_value("shell", "True")
    ):
                ...
                for vulnerable_func in vulnerable_funcs:
                    if vulnerable_func in argument_string and "*" in argument_string:
                        return bandit.Issue(
                            ...
                            text="Possible wildcard injection in call: %s" % context.call_function_name_qual,
