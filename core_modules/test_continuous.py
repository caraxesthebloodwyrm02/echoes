...
class Test_RealInterval:
    ...
    def test_contains(self, shapes, inclusive_a, inclusive_b, data):
        ...
        d = b - a
        x = np.concatenate([np.linspace(a - d, a, 10), np.linspace(a, b, 10), np.linspace(b, b + d, 10)])
...
def draw_distribution_from_family(family, data, rng, proportions, min_side=0):
    ...
    if n_parameterizations > 0:
        ...
        shapes, result_shape = data.draw(npst.mutually_broadcastable_shapes(num_shapes=n_parameters, min_side=min_side))
    ...
    x = dist._variable.draw(
        x_shape, parameter_values=dist._parameters, proportions=proportions, rng=rng, region="typical"
    )
    ...
    y = dist._variable.draw(
        y_shape, parameter_values=dist._parameters, proportions=proportions, rng=rng, region="typical"
    )
...
def check_cdf2(dist, log, x, y, result_shape, methods):
    ...
    for method in methods:
        if isinstance(dist, DiscreteDistribution):
            message = "Two argument cdf functions are currently only supported for " "continuous distributions."
...
def check_moment_funcs(dist, result_shape):
    ...
    for i in range(2, 6):
        ref = dist.moment(i, "raw", method="quadrature")
        check(i, "raw", "transform", ref, success=has_formula(i, "central") or has_formula(i, "standardized"))
    ...
    for i in range(3, 6):
        ref = dist.moment(i, "central", method="quadrature")
        check(i, "central", "normalize", ref, success=has_formula(i, "standardized") and not np.any(variance == 0))
    ...
    for i in range(6):
        ...
        if not (isinstance(dist, Binomial) and np.any((dist.n == 0) | (dist.p == 0) | (dist.p == 1))):
...
def test_input_validation():
    ...
    message = "The `Test` distribution family does not accept parameters, " "but parameters `{'a'}` were provided."
    ...
    message = (
        "The provided parameters `{a}` do not match a supported " "parameterization of the `Test2` distribution family."
    )
    with pytest.raises(ValueError, match=message):
        Test2(a=1)
    ...
    message = (
        "The parameters `{c, d}` provided to the `Test2` " "distribution family cannot be broadcast to the same shape."
    )
    with pytest.raises(ValueError, match=message):
        Test2(c=[1, 2], d=[1, 2, 3])
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            for order in range(5):
                if distname not in skip_raw.get(order, {}):
                    assert_allclose(X.moment(order, kind="raw"), Y.moment(order), rtol=rtol, atol=atol)
            for order in range(3, 4):
                if distname not in skip_standardized:
                    assert_allclose(
                        X.moment(order, kind="standardized"), Y.stats("mvsk"[order - 1]), rtol=rtol, atol=atol
                    )
            if isinstance(dist, stats.rv_continuous):
                ...
                assert_allclose(
                    X.sample(shape=10, rng=seed), Y.rvs(size=10, random_state=np.random.default_rng(seed)), rtol=rtol
                )
...
class TestTransforms:
    ...
    def test_loc_scale(self, data, seed):
        ...
        tmp = draw_distribution_from_family(TransformedNormal, data, rng, proportions=(1, 0, 0, 0), min_side=1)
        dist, x, y, p, logp, result_shape, x_result_shape, xy_result_shape = tmp
        ...
        with np.errstate(invalid="ignore", divide="ignore"):
            ...
            for i in range(1, 5):
                ...
                assert_allclose(dist.moment(i, "standardized"), dist0.moment(i, "standardized") * np.sign(scale) ** i)
...
class TestFullCoverage:
    ...
    @pytest.mark.parametrize(("dtype_in", "dtype_out"), [(np.float16, np.float16), (np.int16, np.float64)])
    ...
    def test_ContinuousDistribution__repr__(self):
        ...
        assert repr(X) == (
            "Uniform(a=array([0., 0., 0., 0.], dtype=float32)," " b=array([1., 1., 1., 1.], dtype=float32))"
        )
...
class MixedDist(ContinuousDistribution):
    ...
    def _pdf_formula(self, x, *args, **kwargs):
        return 0.4 * 1 / (1.1 * np.sqrt(2 * np.pi)) * np.exp(-0.5 * ((x + 0.25) / 1.1) ** 2) + 0.6 * 1 / (
            0.9 * np.sqrt(2 * np.pi)
        ) * np.exp(-0.5 * ((x - 0.5) / 0.9) ** 2)
