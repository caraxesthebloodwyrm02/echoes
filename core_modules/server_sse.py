...
from automation.guardrails.middleware import GuardrailMiddleware
...
class ServerState:
    def __init__(self, base_path: Path, enable_guardrails: bool = True) -> None:
        ...
        self.guardrail_middleware = None

        # Register event callback to broadcast frames
        self.system.register_event_callback(self._on_event)

        # Initialize Guardrail Middleware if enabled
        if enable_guardrails:
            doc_path = base_path.parent / "docs" / "glimpse" / "DEPLOYMENT_AND_OPERATIONS.md"
            self.guardrail_middleware = GuardrailMiddleware(doc_path=str(doc_path))
...
class PreviewRequestHandler(BaseHTTPRequestHandler):
    ...
    def _handle_input(self):
        ...
        # Validate request against guardrails if middleware is active
        if self.state.guardrail_middleware:
            is_valid, message = self.state.guardrail_middleware.validate_request(data, self.headers)
            if not is_valid:
                if "Rate limit" in message:
                    self.send_error(429, message)
...
if __name__ == "__main__":
    # Example: start with guardrails enabled
    start_server(enable_guardrails=True)
    print("SSE server running at http://127.0.0.1:8765 with guardrails enabled.")
    try:
        while True:
            time.sleep(1)
