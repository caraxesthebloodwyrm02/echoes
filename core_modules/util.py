    ...
    for initial, suffixes in get_suffixes_from_common_prefixes(sorted(word_list)):
        ...
        if len(suffixes) > 1:
            if all(len(s) == 1 for s in suffixes):
                ret.append(f"{initial}[{''.join(_escape_regex_range_chars(s) for s in suffixes)}]{trailing}")
            else:
                if _level < max_level:
                    ...
                    if all(len(s) == 1 for s in suffixes):
                        ...
                        ret.append(f"{initial}({ncgroup}{'|'.join(re.escape(s) for s in suffixes)}){trailing}")
        else:
            if suffixes:
