    ...
    * Methods to generate various derivations of a "from" clause, including
    ...
    def _generate_fromclause_column_proxies(
        ...
        columns._populate_separate_keys(
            col._make_proxy(fromclause, primary_key=primary_key, foreign_keys=foreign_keys) for col in self.c
        )
...
class Join(roles.DMLTableRole, FromClause):
    ...
    def _populate_column_collection(
        ...
        _columns: List[KeyedColumnElement[Any]] = [c for c in self.left.c] + [c for c in self.right.c]

        primary_key.extend(sqlutil.reduce_columns((c for c in _columns if c.primary_key), self.onclause))
        columns._populate_separate_keys((col._tq_key_label, col) for col in _columns)  # type: ignore
        foreign_keys.update(itertools.chain(*[col.foreign_keys for col in _columns]))  # type: ignore  # noqa: E501
    ...
    def _populate_column_collection(
        ...
        for c in self._column_args:
            if c.table is not None and c.table is not self:
                _, c = c._make_proxy(self, primary_key=primary_key, foreign_keys=foreign_keys)
            else:
...
class CompoundSelect(HasCompileState, GenerativeSelect, TypedReturnsRows[_TP]):
    ...
    @classmethod
    def _create_union(cls, *selects: _SelectStatementForCompoundArgument[_TP]) -> CompoundSelect[_TP]:
        return CompoundSelect(_CompoundSelectKeyword.UNION, *selects)

    @classmethod
    def _create_union_all(cls, *selects: _SelectStatementForCompoundArgument[_TP]) -> CompoundSelect[_TP]:
        return CompoundSelect(_CompoundSelectKeyword.UNION_ALL, *selects)

    @classmethod
    def _create_except(cls, *selects: _SelectStatementForCompoundArgument[_TP]) -> CompoundSelect[_TP]:
        return CompoundSelect(_CompoundSelectKeyword.EXCEPT, *selects)

    @classmethod
    def _create_except_all(cls, *selects: _SelectStatementForCompoundArgument[_TP]) -> CompoundSelect[_TP]:
        return CompoundSelect(_CompoundSelectKeyword.EXCEPT_ALL, *selects)

    @classmethod
    def _create_intersect(cls, *selects: _SelectStatementForCompoundArgument[_TP]) -> CompoundSelect[_TP]:
        return CompoundSelect(_CompoundSelectKeyword.INTERSECT, *selects)

    @classmethod
    def _create_intersect_all(cls, *selects: _SelectStatementForCompoundArgument[_TP]) -> CompoundSelect[_TP]:
...
class SelectState(util.MemoizedSlots, CompileState):
    ...
    def _memoized_attr__label_resolve_dict(
        ...
        only_froms: Dict[str, ColumnElement[Any]] = {
            c.key: c for c in _select_iterables(self.froms) if c._allow_label_resolve  # type: ignore
        }
...
class Select(
    ...
    @overload
    def with_only_columns(self, __ent0: _TCCA[_T0], *, maintain_column_froms: bool = ...) -> Select[Tuple[_T0]]: ...
    ...
    def union(self, *other: _SelectStatementForCompoundArgument[_TP]) -> CompoundSelect[_TP]:
    ...
    def union_all(self, *other: _SelectStatementForCompoundArgument[_TP]) -> CompoundSelect[_TP]:
    ...
    def except_(self, *other: _SelectStatementForCompoundArgument[_TP]) -> CompoundSelect[_TP]:
    ...
    def except_all(self, *other: _SelectStatementForCompoundArgument[_TP]) -> CompoundSelect[_TP]:
    ...
    def intersect(self, *other: _SelectStatementForCompoundArgument[_TP]) -> CompoundSelect[_TP]:
    ...
    def intersect_all(self, *other: _SelectStatementForCompoundArgument[_TP]) -> CompoundSelect[_TP]:
...
class Exists(UnaryExpression[bool]):
    ...
    def _regroup(
        ...
        if not isinstance(element, Select):
            raise exc.InvalidRequestError("Can only apply this operation to a plain SELECT construct")
