...
class TestTrustRegionConstr:
    ...
    @pytest.mark.parametrize(
        "hess",
        ("prob.hess", "3-point", SR1(), BFGS(exception_strategy="damp_update"), BFGS(exception_strategy="skip_update")),
    )
    def test_list_of_problems(self, prob, grad, hess):
        ...
        sensitive = isinstance(prob, BoundedRosenbrock) and grad == "3-point" and isinstance(hess, BFGS)
        if sensitive:
...
def test_gh20665_too_many_constraints():
    ...
    with np.testing.suppress_warnings() as sup:
        ...
        minimize(
            rosen, x0, method="trust-constr", constraints=[g], options={"factorization_method": "SVDFactorization"}
        )
