...
def test_groupby_resample_with_list_of_keys():
    ...
    mi_exp = pd.MultiIndex.from_arrays([[0, 0, 1, 1], df["date"]._values[::2]], names=["group", "date"])
    expected = DataFrame(
        data={
            "val": [4.0, 3.5, 6.5, 3.0],
        },
...
def test_groupby_resample_kind(kind):
    ...
    df = DataFrame(
        {
            "datetime": pd.to_datetime(["20181101 1100", "20181101 1200", "20181102 1300", "20181102 1400"]),
            "group": ["A", "B", "A", "B"],
            "value": [1, 2, 3, 4],
        }
    )
