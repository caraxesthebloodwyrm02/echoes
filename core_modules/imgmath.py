...
def html_visit_math(self: HTML5Translator, node: nodes.math) -> None:
    ...
    if rendered_path is None:
        ...
        self.body.append(f'<span class="math">{self.encode(node.astext()).strip()}</span>')
    else:
        ...
        self.body.append(f'<img class="math" src="{img_src}"{get_tooltip(self, node)}{align}/>')
...
def html_visit_displaymath(self: HTML5Translator, node: nodes.math_block) -> None:
    ...
    if rendered_path is None:
        # something failed -- use text-only as a bad substitute
        self.body.append(f'<span class="math">{self.encode(node.astext()).strip()}</span></p>\n</div>')
    else:
        if self.builder.config.imgmath_embed:
            image_format = self.builder.config.imgmath_image_format.lower()
            img_src = render_maths_to_base64(image_format, rendered_path)
...
def setup(app: Sphinx) -> ExtensionMetadata:
    ...
    app.add_config_value("imgmath_dvisvgm_args", ["--no-fonts"], "html", types=frozenset({list, tuple}))
