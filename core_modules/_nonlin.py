...
class LowRankMatrix:
    ...
    def __array__(self, dtype=None, copy=None):
        if dtype is not None:
            warnings.warn(
                "LowRankMatrix is scipy-internal code, `dtype` " f"should only be None but was {dtype} (not handled)",
                stacklevel=3,
            )
        if copy is not None:
            warnings.warn(
                "LowRankMatrix is scipy-internal code, `copy` " f"should only be None but was {copy} (not handled)",
                stacklevel=3,
            )
        if self.collapsed is not None:
...
class KrylovJacobian(Jacobian):
    ...
    def __init__(self, rdiff=None, method="lgmres", inner_maxiter=20, inner_M=None, outer_k=10, **kw):
        ...
        valid_inner_params = [k for k in signature(self.method).parameters if k not in ("self", "args", "kwargs")]
