...
class TestAsLinearOperator:
    def setup_method(self):
        ...
        self.cases += [
            (interface.aslinearoperator(M).adjoint(), A.T.conj()) for M, A in make_cases(original.T, np.float64)
        ]
        ...
        self.cases += [
            (interface.aslinearoperator(M).adjoint(), A.T.conj()) for M, A in make_cases(original.T, np.complex128)
        ]
