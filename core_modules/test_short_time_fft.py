...
def test_closest_STFT_dual_window_STFT_roundtrip_complex(scaled):
    ...
    SFT = ShortTimeFFT(win, hop=hop, dual_win=dual, fs=1, fft_mode="twosided", scale_to=None, phase_shift=None)
...
def test_closest_STFT_dual_window_cola(win_name, nperseg, noverlap, scale_fac):
    ...
    xp_assert_close(
        d_s, desired_dual * scale_fac, atol=res * 10, rtol=rel_tol_d, err_msg="Calculated incorrect scaled window!"
    )

    # check unscaled window:
    d_u, u = closest_STFT_dual_window(win * scale_fac, nperseg - noverlap, desired_dual, scaled=False)
    ...
    xp_assert_close(d_u, desired_dual, atol=res * 10, rtol=rel_tol_d, err_msg="Calculated incorrect unscaled window!")
...
def test_invalid_initializer_parameters():
    ...
    with pytest.raises(ValueError, match=r"Parameter win must be 1d, " + r"but win.shape=\(2, 2\)!"):
    ...
    with pytest.raises(ValueError, match=r"dual_win.shape=\(5,\) must equal " + r"win.shape=\(4,\)!"):
...
def test_exceptions_properties_methods():
    ...
    with pytest.raises(
        ValueError,
        match="fft_mode='invalid_typ' not in " + r"\('twosided', 'centered', " + r"'onesided', 'onesided2X'\)!",
    ):
    ...
    with pytest.raises(ValueError, match=r"S.shape\[f_axis\]=2 must be equal" + " to self.f_pts=5.*"):
        SFT.istft(np.zeros((2, 2)))
    with pytest.raises(ValueError, match=r"S.shape\[t_axis\]=1 needs to have" + " at least 2 slices.*"):
    ...
    with pytest.raises(ValueError, match=r"\(k1=1\) - \(k0=0\) = 1 has to " + "be at least.* length 4!"):
        SFT.istft(np.zeros((SFT.f_pts, 3)), k0=0, k1=1)

    with pytest.raises(ValueError, match=r"Parameter axes_seq='invalid' " + r"not in \['tf', 'ft'\]!"):
        # noinspection PyTypeChecker
...
def test_from_window(win_params, Nx: int):
    ...
    SFT1 = ShortTimeFFT.from_window(
        win_params, fs, nperseg, noverlap, symmetric_win=True, fft_mode="twosided", scale_to="psd", phase_shift=1
    )
    # periodic window:
    SFT2 = ShortTimeFFT.from_window(
        win_params, fs, nperseg, noverlap, symmetric_win=False, fft_mode="twosided", scale_to="psd", phase_shift=1
    )
...
def test_from_win_equals_dual_unitary():
    ...
    SFT = ShortTimeFFT.from_win_equals_dual(des_win, hop, 1, fft_mode="twosided", scale_to="unitary")
    # Orthogonal signals:
    ...
    assert sum(x * y) == 0
    xp_assert_close(np.sum(Sxx), np.sum(x**2, dtype=Sxx.dtype), atol=atol, err_msg="Energies do not match!")
    xp_assert_close(np.sum(Sxy), 0.0j, atol=atol, err_msg="STFT scalar product of Sx and Sy not 0!")
    xp_assert_close(SFT.dual_win, SFT.win * SFT.m_num, atol=atol, err_msg="Wrong factor for dual_win/win!")
...
def test_fft_func_roundtrip(n: int):
    ...
    for f_typ, mfft, scaling, phase_shift in product(*pp.values()):
        ...
        xp_assert_close(
            x0.astype(x1.dtype),
            x1,
            err_msg="_fft_func() roundtrip failed for " + f"{f_typ=}, {mfft=}, {scaling=}, {phase_shift=}",
        )
...
def test_minimal_length_signal(m_num):
    ...
    xp_assert_close(x1, x, err_msg=f"Roundtrip minimal length signal ({n=})" + f" for {m_num} sample window failed!")
    with pytest.raises(ValueError, match=rf"len\(x\)={n-1} must be >= ceil.*"):
        SFT.stft(x[:-1])
    with pytest.raises(
        ValueError, match=rf"S.shape\[t_axis\]={Sx.shape[1]-1}" f" needs to have at least {Sx.shape[1]} slices"
    ):
        SFT.istft(Sx[:, :-1], k1=n)
...
def test_tutorial_stft_legacy_stft():
    ...
    xp_assert_close((abs(Sz1[:, 1]).min(), abs(Sz1[:, 1]).max()), (6.925060911593139e-07, 8.00271269218721e-07))
...
def test_tutorial_stft_legacy_spectrogram():
    ...
    f2_u, t2, Sz2_u = spectrogram(
        z, fs, win, nperseg, noverlap, detrend=None, return_onesided=False, scaling="spectrum", mode="complex"
    )
...
def test_roundtrip_multidimensional(fft_mode: FFT_MODE_TYPE):
    ...
    for i, j in product(range(x.shape[0]), range(x.shape[2])):
        ...
        xp_assert_close(
            ...
            err_msg="Multidim. roundtrip for component " + f"x[{i}, :, {j}] and {fft_mode=} failed!",
        )
...
def test_roundtrip_two_dimensional(phase_shift: int | None):
    ...
    """
    n = 21
    SFT = ShortTimeFFT.from_window("hann", fs=1, nperseg=13, noverlap=7, mfft=16, phase_shift=phase_shift)
    x = np.arange(2 * n, dtype=float).reshape(2, n)
    Sx = SFT.stft(x)
    y = SFT.istft(Sx, k1=n)
    xp_assert_close(y, x, atol=2 * np.finfo(SFT.win.dtype).resolution, err_msg="2-dim. roundtrip failed!")
...
def test_roundtrip_windows(window, n: int, nperseg: int, noverlap: int):
    ...
    x32_1_r = x32_1.real
    xp_assert_close(x32, x32_1_r.astype(np.float32), err_msg="Roundtrip for 32 Bit float values failed")
    xp_assert_close(x32.imag, np.zeros_like(x32.imag), err_msg="Roundtrip for 32 Bit float values failed")
...
def test_roundtrip_complex_window(signal_type):
    ...
    Sz = SFT.stft(z)
    z1 = SFT.istft(Sz, k1=len(z))
    xp_assert_close(z.astype(np.complex128), z1, err_msg="Roundtrip for complex-valued window failed")
...
def test_stft_padding_roundtrip(window, N: int, nperseg: int, noverlap: int, mfft: int, padding):
    ...
    x1 = SFT.istft(Sx, k1=N)
    xp_assert_close(x1, x.astype(np.complex128), err_msg=f"Failed real roundtrip with '{padding}' padding")
