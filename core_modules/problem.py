...
class Problem:
    ...
    def __init__(
        ...
        if linear.a_ub.shape[1] != n:
            raise ValueError(f"The left-hand side matrices of the linear constraints must " f"have {n} columns.")
    ...
    def best_eval(self, penalty):
        ...
        if np.any(finite_idx):
            ...
            if np.any(feasible_idx) and not np.all(np.isnan(fun_filter[feasible_idx])):
                ...
                merit_filter[finite_idx] = fun_filter[finite_idx] + penalty * maxcv_filter[finite_idx]
