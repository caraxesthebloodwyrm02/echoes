...
def qr_multiply(a, c, mode="right", pivoting=False, conjugate=False, overwrite_a=False, overwrite_c=False):
    ...
    if mode == "left":
        if c.shape[0] != min(M, N + overwrite_c * (M - N)):
            raise ValueError("Array shapes are not compatible for Q @ c" f" operation: {a.shape} vs {c.shape}")
    else:
        if M != c.shape[1]:
            raise ValueError("Array shapes are not compatible for c @ Q" f" operation: {c.shape} vs {a.shape}")
