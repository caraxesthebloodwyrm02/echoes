...
@image_comparison(["constrained_layout10.png"], style="mpl20", tol=0 if platform.machine() == "x86_64" else 0.032)
...
def test_compressed_suptitle():
    fig, (ax0, ax1) = plt.subplots(
        nrows=2, figsize=(4, 10), layout="compressed", gridspec_kw={"height_ratios": (1 / 4, 3 / 4), "hspace": 0}
    )
...
def test_submerged_subfig():
    ...
    fig.draw_without_rendering()
    for ax in axs[1:]:
        assert np.allclose(ax.get_position().bounds[-1], axs[0].get_position().bounds[-1], atol=1e-6)
