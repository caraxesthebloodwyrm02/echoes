...
class burr_gen(rv_continuous):
    ...
    def _logpdf(self, x, c, d):
        output = xpx.apply_where(
            ...
            lambda x_, c_, d_: (np.log(c_) + np.log(d_) + sc.xlogy(-c_ - 1, x_) - sc.xlog1py(d_ + 1, x_ ** (-c_))),
        )
        return output[()] if output.ndim == 0 else output
    ...
    def _stats(self, c, d):
        ...
        g1 = xpx.apply_where(
            c > 3.0,
            (e1, e2, e3, mu2_if_c),
            lambda e1, e2, e3, mu2_if_c: ((e3 - 3 * e2 * e1 + 2 * e1**3) / np.sqrt((mu2_if_c) ** 3)),
            fill_value=np.nan,
        )
        g2 = xpx.apply_where(
            c > 4.0,
            (e1, e2, e3, e4, mu2_if_c),
            lambda e1, e2, e3, e4, mu2_if_c: (((e4 - 4 * e3 * e1 + 6 * e2 * e1**2 - 3 * e1**4) / mu2_if_c**2) - 3),
            fill_value=np.nan,
        )
        if np.ndim(c) == 0:
...
class cosine_gen(rv_continuous):
    ...
    def _logpdf(self, x):
        c = np.cos(x)
        return xpx.apply_where(c != -1, c, lambda c: np.log1p(c) - np.log(2 * np.pi), fill_value=-np.inf)
...
class fatiguelife_gen(rv_continuous):
    ...
    def _logpdf(self, x, c):
        return (
            np.log(x + 1) - (x - 1) ** 2 / (2.0 * x * c**2) - np.log(2 * c) - 0.5 * (np.log(2 * np.pi) + 3 * np.log(x))
        )
...
class f_gen(rv_continuous):
    ...
    def _stats(self, dfn, dfd):
        ...
        g2 = xpx.apply_where(
            v2 > 8, (g1, v2_6, v2_8), lambda g1, v2_6, v2_8: (8 + g1 * g1 * v2_6) / v2_8, fill_value=np.nan
        )
        g2 *= 3.0 / 2.0
...
class genpareto_gen(rv_continuous):
    ...
    def _logpdf(self, x, c):
        return xpx.apply_where((x == x) & (c != 0), (x, c), lambda x, c: -sc.xlog1py(c + 1.0, c * x) / c, fill_value=-x)
    ...
    def _logsf(self, x, c):
        return xpx.apply_where((x == x) & (c != 0), (x, c), lambda x, c: -sc.log1p(c * x) / c, fill_value=-x)
    ...
    def _stats(self, c, moments="mv"):
        ...
        if "v" in moments:
            v = xpx.apply_where(c < 1 / 2, c, lambda xi: 1 / (1 - xi) ** 2 / (1 - 2 * xi), fill_value=np.nan)

        if "s" in moments:
            s = xpx.apply_where(
                c < 1 / 3, c, lambda xi: 2 * (1 + xi) * np.sqrt(1 - 2 * xi) / (1 - 3 * xi), fill_value=np.nan
            )
...
class genextreme_gen(rv_continuous):
    ...
    def _ppf(self, q, c):
        x = -np.log(-np.log(q))
        return xpx.apply_where((x == x) & (c != 0), (x, c), lambda x, c: -sc.expm1(-c * x) / c, fill_value=x)
...
class erlang_gen(gamma_gen):
    ...
    def _argcheck(self, a):
        ...
        if not allint:
            ...
            message = "The shape parameter of the erlang distribution " f"has been given a non-integer value {a!r}."
...
class halflogistic_gen(rv_continuous):
    ...
    def _isf(self, q):
        return xpx.apply_where(q < 0.5, q, lambda q: -sc.logit(0.5 * q), lambda q: 2 * np.arctanh(1 - q))
...
class invgamma_gen(rv_continuous):
    ...
    def _stats(self, a, moments="mvsk"):
        ...
        m2 = xpx.apply_where(a > 2, a, lambda x: 1.0 / (x - 1.0) ** 2 / (x - 2.0), fill_value=np.inf)

        g1, g2 = None, None
        if "s" in moments:
            g1 = xpx.apply_where(a > 3, a, lambda x: 4.0 * np.sqrt(x - 2.0) / (x - 3.0), fill_value=np.nan)
        if "k" in moments:
            g2 = xpx.apply_where(a > 4, a, lambda x: 6.0 * (5.0 * x - 11.0) / (x - 3.0) / (x - 4.0), fill_value=np.nan)
...
class geninvgauss_gen(rv_continuous):
    ...
    def _logquasipdf(self, x, p, b):
        ...
        return xpx.apply_where(
            x > 0, (x, p, b), lambda x, p, b: (p - 1) * np.log(x) - b * (x + 1 / x) / 2, fill_value=-np.inf
        )
...
class loggamma_gen(rv_continuous):
    ...
    def _cdf(self, x, c):
        ...
        #                          = exp(c*x - gammaln(c+1))
        return xpx.apply_where(
            x < _LOGXMIN, (x, c), lambda x, c: np.exp(c * x - sc.gammaln(c + 1)), lambda x, c: sc.gammainc(c, np.exp(x))
        )
    ...
    def _ppf(self, q, c):
        ...
        return xpx.apply_where(
            g < _XMIN, (g, q, c), lambda g, q, c: (np.log(q) + sc.gammaln(c + 1)) / c, lambda g, q, c: np.log(g)
        )
    ...
    def _isf(self, q, c):
        ...
        return xpx.apply_where(
            g < _XMIN, (g, q, c), lambda g, q, c: (np.log1p(-q) + sc.gammaln(c + 1)) / c, lambda g, q, c: np.log(g)
        )
...
class ncx2_gen(rv_continuous):
    ...
    def _logpdf(self, x, df, nc):
        return xpx.apply_where(nc != 0, (x, df, nc), _ncx2_log_pdf, lambda x, df, _: chi2._logpdf(x, df))

    def _pdf(self, x, df, nc):
        with np.errstate(over="ignore"):  # see gh-17432
            return xpx.apply_where(nc != 0, (x, df, nc), scu._ncx2_pdf, lambda x, df, _: chi2._pdf(x, df))

    def _cdf(self, x, df, nc):
        with np.errstate(over="ignore"):  # see gh-17432
            return xpx.apply_where(nc != 0, (x, df, nc), scu._ncx2_cdf, lambda x, df, _: chi2._cdf(x, df))

    def _ppf(self, q, df, nc):
        with np.errstate(over="ignore"):  # see gh-17432
            return xpx.apply_where(nc != 0, (q, df, nc), scu._ncx2_ppf, lambda x, df, _: chi2._ppf(x, df))

    def _sf(self, x, df, nc):
        with np.errstate(over="ignore"):  # see gh-17432
            return xpx.apply_where(nc != 0, (x, df, nc), scu._ncx2_sf, lambda x, df, _: chi2._sf(x, df))

    def _isf(self, x, df, nc):
        with np.errstate(over="ignore"):  # see gh-17432
            return xpx.apply_where(nc != 0, (x, df, nc), scu._ncx2_isf, lambda x, df, _: chi2._isf(x, df))
...
class ncf_gen(rv_continuous):
    ...
    def _stats(self, dfn, dfd, nc, moments="mv"):
        ...
        g2 = scu._ncf_kurtosis_excess(dfn, dfd, nc) - 3 if "k" in moments else None  # isn't really excess kurtosis!
...
class t_gen(rv_continuous):
    ...
    def _pdf(self, x, df):
        return xpx.apply_where(
            df == np.inf, (x, df), lambda x, df: norm._pdf(x), lambda x, df: np.exp(self._logpdf(x, df))
        )
...
class powerlognorm_gen(rv_continuous):
    ...
    def _logpdf(self, x, c, s):
        return (
            np.log(c) - np.log(x) - np.log(s) + _norm_logpdf(np.log(x) / s) + _norm_logcdf(-np.log(x) / s) * (c - 1.0)
        )
...
class skewnorm_gen(rv_continuous):
    ...
    def _pdf(self, x, a):
        return xpx.apply_where(
            a == 0, (x, a), lambda x, a: _norm_pdf(x), lambda x, a: 2.0 * _norm_pdf(x) * _norm_cdf(a * x)
        )

    def _logpdf(self, x, a):
        return xpx.apply_where(
            a == 0, (x, a), lambda x, a: _norm_logpdf(x), lambda x, a: np.log(2) + _norm_logpdf(x) + _norm_logcdf(a * x)
        )
...
class truncnorm_gen(rv_continuous):
    ...
    def _munp(self, n, a, b):
        ...
        return xpx.apply_where(
            (n >= 0) & (a == a) & (b == b), (n, a, b), np.vectorize(n_th_moment, otypes=[np.float64]), fill_value=np.nan
        )
        ...
        def find_kappa(data, loc):
            ...
            if r == 1:
                ...
                if solve_for_kappa(lower_bound) >= 0:
                    ...
                    root_res = root_scalar(solve_for_kappa, method="brentq", bracket=(lower_bound, upper_bound))
...
class crystalball_gen(rv_continuous):
    ...
    def _munp(self, n, beta, m):
        ...
        def n_th_moment(n, beta, m):
            ...
            B = m / beta - beta
            rhs = (
                2 ** ((n - 1) / 2.0) * sc.gamma((n + 1) / 2) * (1.0 + (-1) ** n * sc.gammainc((n + 1) / 2, beta**2 / 2))
            )
        ...
        return N * xpx.apply_where(
            n + 1 < m, (n, beta, m), np.vectorize(n_th_moment, otypes=[np.float64]), fill_value=np.inf
        )
