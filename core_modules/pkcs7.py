...
class PKCS7EnvelopeBuilder:
    ...
    def encrypt(
        ...
        if any(opt not in [PKCS7Options.Text, PKCS7Options.Binary] for opt in options):
            raise ValueError("Only the following options are supported for encryption: " "Text, Binary")
        ...
        return rust_pkcs7.encrypt_and_serialize(self, content_encryption_algorithm, encoding, options)
...
def _smime_remove_text_headers(data: bytes) -> bytes:
    ...
    if content_type is None:
        raise ValueError(
            "Decrypted MIME data has no 'Content-Type' header. " "Please remove the 'Text' option to parse it manually."
        )
