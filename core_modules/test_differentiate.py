...
class TestDerivative:
    ...
    def test_flags(self, xp):
        ...
        res = derivative(f, xp.ones(4, dtype=xp.float64), tolerances=dict(rtol=1e-14), order=2, args=args)

        ref_flags = xp.asarray([eim._ECONVERGED, _EERRORINCREASE, eim._ECONVERR, eim._EVALUEERR], dtype=xp.int32)
    ...
    def test_flags_preserve_shape(self, xp):
        ...
        res = derivative(f, xp.asarray(1, dtype=xp.float64), tolerances=dict(rtol=1e-14), order=2, preserve_shape=True)
    ...
    def test_initial_step(self, xp):
        ...
        for i in range(h0.shape[0]):
            ref = derivative(f, x, initial_step=h0[i, 0], order=2, maxiter=1, step_direction=step_direction)
...
class TestJacobian(JacobianHessianTest):
    ...
    def f3(z, xp):
        r, phi, th = z
        return xp.stack([r * xp.sin(phi) * xp.cos(th), r * xp.sin(phi) * xp.sin(th), r * xp.cos(phi)])
    ...
    def test_attrs(self, xp):
        ...
        for attr in ["success", "status", "df", "nit", "nfev"]:
            ref_attr = xp.asarray(
                [[getattr(res00, attr), getattr(res01, attr)], [getattr(res10, attr), getattr(res11, attr)]]
            )
            ref[attr] = xp.squeeze(ref_attr, axis=tuple(ax for ax, size in enumerate(ref_attr.shape) if size == 1))
    ...
    def test_step_direction_size(self, xp):
        ...
        h0 = [0.25, 0.1, 0.5]
        atol = {"atol": 1e-8}
        res = jacobian(f, xp.asarray(b, dtype=xp.float64), initial_step=h0, step_direction=dir, tolerances=atol)
