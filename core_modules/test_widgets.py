...
def test_rectangle_minspan(ax, spancoords, minspanx, x1, minspany, y1):
    ...
    tool = widgets.RectangleSelector(
        ax, onselect=onselect, interactive=True, spancoords=spancoords, minspanx=minspanx, minspany=minspany
    )
...
def test_rectangle_handles(ax):
    tool = widgets.RectangleSelector(
        ax, grab_range=10, interactive=True, handle_props={"markerfacecolor": "r", "markeredgecolor": "b"}
    )
    tool.extents = (100, 150, 100, 150)
...
def test_rectangle_selector_ignore_outside(ax, ignore_event_outside):
    ...
    tool = widgets.RectangleSelector(ax, onselect=onselect, ignore_event_outside=ignore_event_outside)
    click_and_drag(tool, start=(100, 110), end=(150, 120))
    onselect.assert_called_once()
    assert tool.extents == (100.0, 150.0, 110.0, 120.0)
...
def test_polygon_selector_set_props_handle_props(ax, draw_bounding_box):
    tool = widgets.PolygonSelector(
        ax, props=dict(color="b", alpha=0.2), handle_props=dict(alpha=0.5), draw_bounding_box=draw_bounding_box
    )
