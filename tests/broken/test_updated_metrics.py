...
def test_metrics():
    ...
    for i, messages in enumerate(test_messages, 1):
        try:
            ...
            print(
                f"  Request {i}: {messages[0]['content'][:30]}... | Time: {elapsed:.3f}s"
            )
    ...
    print(
        f"  Avg Response Time: {cache_stats['performance']['avg_response_time']:.3f}s"
    )
    print(
        f"  P90 Response Time: {cache_stats['performance']['p90_response_time']:.3f}s"
    )
    ...
    assert (
        cache_stats["performance"]["avg_response_time"] > 0
    ), "Response time should be positive"
    assert (
        cache_stats["system"]["current_memory_mb"] > 0
