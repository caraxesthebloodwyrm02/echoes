...
from core.ai.intelligent_openai_client import IntelligentOpenAIClient, ChatMessage
...
class TestIntelligentClientSmoke:
    ...
    async def test_provider_registration(self):
        ...
        with patch.dict(
            os.environ,
            {"OPENAI_API_KEY": "test-key", "AZURE_OPENAI_API_KEY": "azure-test-key"},
        ):
            client = IntelligentOpenAIClient(enable_routing=False)
    ...
    async def test_friendly_status_messages(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            assert any(
                keyword in status for keyword in ["providers", "No providers", "✨", "❌"]
            )

    @pytest.mark.asyncio
    async def test_chat_message_conversion(self):
        """Test ChatMessage to dict conversion."""
        msg = ChatMessage(role="user", content="Hello world", name="test_user")
