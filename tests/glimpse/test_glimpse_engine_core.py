...
def test_latency_statuses_and_stale():
    async def run():
        ...
        async def slow_sampler(_d: Draft):
            await asyncio.sleep(6.1)  # > t4 (6000 ms) to force stale
...
def test_clarifier_when_intent_unspecified():
    async def run():
        async def clarifier_sampler(_d: Draft):
            return ("", "", "Clarifier: Could you clarify your goal?", False)

        engine = GlimpseEngine(sampler=clarifier_sampler)
