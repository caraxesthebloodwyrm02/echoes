                    ...
                    for i, line in enumerate(lines, 1):
                        if keyword in line:
                            issues["malicious_keywords"].append(
                                {"line": i, "keyword": keyword, "code": line.strip()}
                            )
    ...
    def _analyze_ast(self, tree: ast.AST, file_path: Path, issues: Dict[str, Any]):
        ...
        class MaliciousNodeVisitor(ast.NodeVisitor):
            ...
            def visit_FunctionDef(self, node):
                ...
                if any(
                    ...
                    for keyword in ["wrapper", "intercept", "detour", "clone", "copy"]
                ):
                    ...
                    if isinstance(child, ast.Call):
                        if isinstance(child.func, ast.Name):
                            ...
                            if any(
                                ...
                                for keyword in ["copy", "clone", "override", "modify"]
                            ):
        ...
        for func in visitor.suspicious_functions:
            if any(
                ...
                for keyword in ["wrapper", "intercept", "modify"]
            ):
        ...
        for func in visitor.suspicious_functions:
            if any(
                keyword in func["name"].lower()
                for keyword in ["clone", "copy", "mirror"]
            ):
    ...
    def generate_removal_script(self, results: Dict[str, Any]) -> str:
        ...
        for file_path, issues in results["issues"].items():
            ...
            if issues.get("malicious_keywords"):
                script_lines.append(
                    "    # TODO: Review and remove malicious keyword usage"
                )
            ...
            if issues.get("suspicious_decorators"):
                ...
                for item in issues["suspicious_decorators"]:
                    script_lines.append(
                        f'    # Line {item["line"]}: {item["decorator"]}'
                    )
                ...
                if issue_type not in ["file", "error"] and items:
                    ...
                    for item in items[:3]:  # Show first 3
                        if isinstance(item, dict) and "line" in item:
                            print(
                                f"      Line {item['line']}: {item.get('keyword', item.get('pattern', item.get('decorator', 'unknown')))}"
                            )
