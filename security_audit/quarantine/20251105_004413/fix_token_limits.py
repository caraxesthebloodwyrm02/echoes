    ...
    try:
        ...
        for test_case in test_cases:
            response = await connection.direct_chat(
                messages=test_case["messages"], max_tokens=test_case["max_tokens"]
            )
            ...
            if actual_tokens <= expected_max:
                print(
                    f"✅ {test_case['name']}: {actual_tokens} tokens <= {expected_max}"
                )
                passed_tests += 1
            else:
...
async def test_no_request_modification():
    ...
    try:
        ...
        checks = {
            ...
            "Token Limit Respected": response.get("usage", {}).get("total_tokens", 0)
            <= 30,
