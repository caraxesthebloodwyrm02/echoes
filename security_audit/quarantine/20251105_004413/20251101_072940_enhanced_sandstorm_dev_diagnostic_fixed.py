...
# Configure enhanced logging for diagnostic reproducibility (UTF-8 compatible)
logging.basicConfig(
    level=logging.INFO,
...
class EnhancedSandstormDevDiagnostic:
    ...
    def _load_config(self, config_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                f"Config file {config_path} not found, using enhanced defaults"
            )
    ...
    def validate_impact_data_source(self, raw_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        if validation_score < min_quality_score:
            logger.warning(
                f"Impact layer quality {validation_score:.3f} below threshold {min_quality_score}"
            )
    ...
    def analyze_atmospheric_extension(
        ...
        logger.info(f"Analyzing enhanced atmospheric extension: {source_name}")
        sig = DiagnosticSignature(
    ...
    def activate_sandstorm_alert_enhanced(self) -> bool:
        ...
        if total_coh >= threshold:
            ...
            logger.warning(
                f"glimpse Alert not activated - {self.current_alert_level.value}"
            )
