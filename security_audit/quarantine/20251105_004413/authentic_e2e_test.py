...
async def test_token_limit_respect():
    ...
    try:
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            print(f"   Test {i}: {completion_tokens} completion tokens <= {test_case['buffer']}? {limit_respected}")
...
async def test_no_echoes_defaults():
    ...
    try:
        ...
        checks = {
            ...
            "Echoes Defaults Bypassed": response.get("echoes_defaults_bypassed") is True,
...
async def test_pure_openai_behavior():
    ...
    try:
        ...
        checks = {
            ...
            "Similar Token Usage": abs(pure_response["usage"]["total_tokens"] - echoes_response["usage"]["total_tokens"]) <= 2,
            "Both Respect Limits": pure_response["usage"]["completion_tokens"] <= 10 and echoes_response["usage"]["completion_tokens"] <= 10,
            "Both Direct": pure_response.get("pure_openai") and echoes_response.get("direct_connection")
        }
        ...
        print(f"      📊 Echoes Direct: {echoes_response['usage']['total_tokens']} tokens")
