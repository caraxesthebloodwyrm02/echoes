...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("root_cleanup.log"), logging.StreamHandler()],
)
    ...
    def organize(self, dry_run: bool = True):
        ...
        for item in self.root_dir.iterdir():
            try:
                ...
                if dry_run:
                    self.logger.info(
                        f"Would move: {item.name} -> {target_path.relative_to(self.root_dir)}"
                    )
                ...
                moved.append(item.name)
                self.logger.info(
                    f"Moved: {item.name} -> {target_path.relative_to(self.root_dir)}"
                )
        ...
        if dry_run:
            ...
            self.logger.info(
                "Run with --apply to perform the actual file organization."
            )
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Organize the root directory by moving non-essential files to appropriate subdirectories."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Actually perform the file operations (default: dry run)",
    )
    parser.add_argument(
        "--root",
        type=str,
