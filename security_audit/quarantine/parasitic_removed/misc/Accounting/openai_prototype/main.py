...
audit_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """You are an expert AI accounting auditor analyzing financial transactions. 
...
class OpenAIAuditEngine:
    ...
    async def audit_transactions(
        ...
        try:
            ...
            flags_raw = await self.audit_chain.ainvoke(
    ...
    def extract_invoice_data(self, image_bytes: bytes) -> dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Vision extraction failed: {str(e)}"
            )
...
async def generate_dataset(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Dataset generation failed: {str(e)}"
        )
...
async def audit_transactions(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Audit processing failed: {str(e)}"
        )
...
async def extract_invoice(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Invoice extraction failed: {str(e)}"
        )
...
async def run_experiment(
    ...
    try:
        ...
        experiment = orchestrator.create_experiment(
            name=name, dataset_size=dataset_size, groups=groups
        )
