    ...
    def __init__(self, api_key: str | None = None):
        ...
        self.audit_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are a forensic accountant AI auditor. Analyze transactions for ALL accounting irregularities.
...
Return ONLY valid JSON array. Be thorough but efficient.""",
        ...
        self.explanation_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are an expert auditor explaining AI findings to human reviewers.
    ...
    def _register_default_policies(self):
        ...
        def check_amount_anomalies(transactions: list[dict]) -> list[dict]:
            ...
            std_amount = (
                sum((x - mean_amount) ** 2 for x in amounts) / len(amounts)
            ) ** 0.5
        ...
        def check_benfords_law(transactions: list[dict]) -> list[dict]:
            ...
            expected_freq = {
                d: (len(leading_digits) * 0.3010 * (1 / d)) for d in range(1, 10)
            }
    ...
    async def audit_with_openai(
        ...
        try:
            ...
            openai_flags = await self.audit_chain.ainvoke(
            ...
            results["summary"] = {
                ...
                "flag_rate": (len(openai_flags) + len(results["policy_flags"]))
                / len(transactions)
                * 100,
                "high_risk_count": sum(
                    1 for flag in openai_flags if flag.get("risk_score", 0) > 80
                ),
            }
