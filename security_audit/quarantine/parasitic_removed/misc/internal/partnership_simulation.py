...
def run_enhanced_sim(path_name, path_config):
    ...
    for _ in range(SIMS):
        responses = sum(
            1
            for _ in range(path_config["outreach"])
            if random.random() < path_config["resp_rate"]
        )
        accepts = sum(
            1 for _ in range(responses) if random.random() < path_config["accept_rate"]
        )
...
for path, result in results.items():
    print(
        f"{path.capitalize():12s} | P(success): {result['success_probability']:.2%} | "
