    ...
    try:
        ...
        try:
            ...
            for token_limit in test_tokens:
                ...
                if completion_tokens <= token_limit + 5:
                    print(
                        f"   ✅ Token limit {token_limit}: {completion_tokens} tokens (respected)"
                    )
                else:
                    print(
                        f"   ❌ Token limit {token_limit}: {completion_tokens} tokens (BLOCKED)"
                    )
                    return False
        ...
        try:
            ...
            for test in extreme_tests:
                response = await connection.direct_chat(
                    messages=[
                        {"role": "user", "content": f"Extreme test: {test['name']}"}
                    ],
                    model="gpt-3.5-turbo",
                ...
                if test["max_tokens"] <= 100:
                    ...
                    if completion_tokens <= test["max_tokens"] + 2:
                        print(
                            f"   ✅ Extreme {test['name']} ({test['max_tokens']}): {completion_tokens} tokens"
                        )
                    else:
                        print(
                            f"   ❌ Extreme {test['name']} ({test['max_tokens']}): {completion_tokens} tokens - BLOCKED"
                        )
                        return False
                else:
        ...
        try:
            ...
            if avg_time < 2.0:  # Should be fast without blocking
                print(
                    f"   ✅ Token iteration speed: {avg_time:.2f}s average (unblocked)"
                )
            else:
                print(
                    f"   ⚠️ Token iteration speed: {avg_time:.2f}s average (potential blocking)"
                )
...
async def generate_token_iteration_report(results: list):
    ...
    test_names = ["Token Iteration Unblocked", "No Default Interference"]
...
async def main():
    ...
    tests = [test_token_iteration_unblocked(), test_no_default_interference()]
