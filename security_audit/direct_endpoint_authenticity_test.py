...
async def test_endpoint_authenticity():
    ...
    try:
        ...
        model_prefix = (
            response["model"].split("-")[0] + "-" + response["model"].split("-")[1]
        )

        if not any(
            model_prefix in authentic_model for authentic_model in authentic_models
        ):
        ...
        missing_usage = [
            field for field in usage_fields if field not in response["usage"]
        ]
        ...
        print(
            f"   ✅ Authentic usage tracking: {response['usage']['total_tokens']} tokens"
        )
    ...
    try:
        ...
        for model in models_to_test:
            try:
                ...
                endpoint_results[model] = {"status": f"❌ ERROR: {e}"}
...
async def test_endpoint_timing():
    ...
    try:
        ...
        for data in timing_data:
            print(
                f"      Request {data['request']}: {data['response_time']:.2f}s ({data['tokens']} tokens)"
            )
