...
class TokenBlockingDetector:
                ...
                for pattern in self.token_blocking_patterns:
                    if re.search(pattern, line, re.IGNORECASE):
                        issues["token_blocking_logic"].append(
                            {"line": i, "pattern": pattern, "code": line.strip()}
                        )
    ...
    def _analyze_token_ast(
        self, tree: ast.AST, file_path: Path, issues: Dict[str, Any]
    ):
        ...
        class TokenBlockingVisitor(ast.NodeVisitor):
            ...
            def visit_Assign(self, node):
                ...
                if (
                    ...
                    and node.value.value == 4000
                ):
                    for target in node.targets:
                        if (
                            isinstance(target, ast.Name)
                            and "token" in target.id.lower()
                        ):
            ...
            def visit_If(self, node):
                ...
                if hasattr(node.test, "left") and hasattr(node.test, "ops"):
                    if (
                        ...
                        and "token" in node.test.left.id.lower()
                    ):
                        self.token_defaults.append(
                            {"line": node.lineno, "condition": "token_if_condition"}
                        )
                    ...
                    if isinstance(items, list):
                        ...
                        for item in items[:3]:  # Show first 3
                            if isinstance(item, dict) and "line" in item:
                                print(
                                    f"      Line {item['line']}: {item.get('code', item.get('pattern', 'suspicious'))}"
                                )
