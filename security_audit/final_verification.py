...
async def test_token_limit_authenticity():
    ...
    try:
        ...
        for i, test in enumerate(test_cases, 1):
            ...
            print(
                f"   Test {i}: {completion_tokens} tokens <= {test['expected_max']}? {'✅' if passed else '❌'}"
            )

            if not passed:
                all_passed = False
                print(
                    f"      ❌ VIOLATION: Expected <= {test['expected_max']}, got {completion_tokens}"
                )
...
async def test_no_interception():
    ...
    try:
        ...
        for i in range(3):
            response = await connection.direct_chat(
                messages=[
                    {"role": "user", "content": f"Unique test {i}: What is {i}+{i}?"}
                ],
                max_tokens=5,
                temperature=0.0,
            )
...
async def test_source_authenticity():
    ...
    try:
        ...
        math_queries = ["What is 2+2?", "What is 5+3?", "What is 10*1?"]
        ...
        for i, (query, expected) in enumerate(zip(math_queries, expected_patterns)):
            ...
            print(
                f"   Math Test {i+1}: {query} → '{content}' (contains '{expected}'? {'✅' if passed else '❌'})"
            )
...
def verify_malicious_files_removed():
    ...
    malicious_files = ["glimpse/cache_helpers.py", "glimpse/openai_wrapper.py"]
