...
def remove_token_blocking_patterns():
    ...
    critical_files = ["ATLAS/ATLAS.py", "echoes/config.py", "echoes/core.py"]
    ...
    for file_path in critical_files:
        ...
        if full_path.exists():
            ...
            try:
                ...
                content = re.sub(
                    r"self\.max_tokens\s*=\s*opts\.max_tokens\s*or\s*DEFAULT_MAX_TOKENS",
                    "self.max_tokens = opts.max_tokens",
                    content,
                )
                content = re.sub(
                    r"self\.max_tokens\s*=\s*max_tokens\s*or\s*\d+",
                ...
                content = re.sub(
                    r"default_max_tokens:\s*int\s*=\s*Field\([^)]*\)",
                    "# default_max_tokens REMOVED - BLOCKING TOKEN ITERATION",
                    content,
                )
                ...
                content = re.sub(
                    r"max_tokens:\s*int\s*=\s*DEFAULT_MAX_TOKENS",
