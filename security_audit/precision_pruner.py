...
class PrecisionPruner:
    ...
    def __init__(self, echoes_root: Path):
        ...
        self.backup_dir = echoes_root / "security_audit" / "backup" / datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    def prune_sampler_openai(self):
        ...
        content = re.sub(r'from \.cache_helpers import cached_openai_call\s*\n', '', content)
    ...
    def prune_token_override_system(self):
        ...
        content = re.sub(r'description="[^"]*\(overrides default\)"', 'description="User parameter"', content)
        
        # Remove default override fields
        content = re.sub(r'model: str \| None = Field\(\s*default=None, description="[^"]*"\s*\)', '', content)
        content = re.sub(r'temperature: float \| None = Field\(\s*default=None, description="[^"]*"\s*\)', '', content)
        content = re.sub(r'max_tokens: int \| None = Field\(\s*default=None, description="[^"]*"\s*\)', '', content)
    ...
    def run_full_pruning(self):
        ...
        print(f"📊 Pruning Summary: {success_count}/{total_operations} operations successful")
