...
def _weak_crypto_key_size_cryptography_io(context, config):
    ...
    if key_type in ["DSA", "RSA"]:
        key_size = (
            context.get_call_arg_value("key_size") or context.get_call_arg_at_position(arg_position[key_type]) or 2048
        )
        ...
        curve = context.get_call_arg_value("curve") or (
            len(context.call_args) > arg_position[key_type] and context.call_args[arg_position[key_type]]
        )
