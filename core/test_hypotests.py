...
class TestMannWhitneyU:
    ...
    def test_exact_distribution(self):
        ...
        for n, table in p_tables.items():
            for m, p in table.items():
                ...
                assert_allclose(_mwu_state.s.cdf(k=u2) + _mwu_state.s.sf(k=u2) - _mwu_state.s.pmf(k=u2), 1)
    ...
    def test_permutation_method(self, alternative):
        ...
        res = stats.mannwhitneyu(x, y, method=stats.PermutationMethod(), alternative=alternative, axis=1)
...
class TestGamesHowell:
    ...
    def test_compare_spss(self, data, res_expect_str):
        ...
        """
        res_expect = np.asarray(res_expect_str.replace(" - ", " ").split()[7:], dtype=float).reshape(-1, 6)
