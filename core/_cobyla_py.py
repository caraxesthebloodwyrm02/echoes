...
def _minimize_cobyla(
    ...
    **unknown_options,
    ...
    if callback is not None:
        ...
        if set(sig.parameters) == {"intermediate_result"}:
            ...
            def wrapped_callback_intermediate(x, f, nf, tr, cstrv, nlconstrlist):
                intermediate_result = OptimizeResult(x=np.copy(x), fun=f, nfev=nf, nit=tr, maxcv=cstrv)
    ...
    result = minimize(
        sf.fun, x0, method="cobyla", bounds=bounds, constraints=constraints, callback=wrapped_callback, options=options
    )
