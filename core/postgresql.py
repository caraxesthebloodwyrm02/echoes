...
class PostgresqlImpl(DefaultImpl):
    ...
    def create_index(self, index: Index, **kw: Any) -> None:
        ...
        for col in postgresql_include:
            if col not in index.table.c:  # type: ignore[union-attr]
                index.table.append_column(Column(col, sqltypes.NullType))  # type: ignore[union-attr]
    ...
    def compare_indexes(
        ...
        for pos, (m_e, r_e) in enumerate(zip(m_exprs, r_exprs), 1):
            ...
            elif m_compile.strip().endswith("_ops") and (" " in m_compile or ")" in m_compile):  # is an expression
