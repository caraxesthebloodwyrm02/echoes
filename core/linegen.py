...
class LineGenerator(Visitor[Line]):
    ...
    def visit_tname(self, node: Node) -> Iterator[Line]:
        ...
        if maybe_make_parens_invisible_in_atom(node.children[2], parent=node, mode=self.mode, features=self.features):
    ...
    def visit_comp_for(self, node: Node) -> Iterator[Line]:
        if Preview.wrap_comprehension_in in self.mode:
            normalize_invisible_parens(node, parens_after={"in"}, mode=self.mode, features=self.features)
        yield from self.visit_default(node)
    ...
    for index, child in enumerate(list(node.children)):
        ...
        if check_lpar:
            if (
                ...
                if maybe_make_parens_invisible_in_atom(child, parent=node, mode=mode, features=features):
                    wrap_in_parentheses(node, child, visible=False)
...
def maybe_make_parens_invisible_in_atom(
    ...
    if (
        ...
        or (is_tuple(node) and parent.type == syms.with_stmt and has_sibling_with_type(node, token.COMMA))
