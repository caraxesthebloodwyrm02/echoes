...
class TestHessianUpdateStrategy(TestCase):
    ...
    def test_hessian_initialization(self):
        ...
        symmetric_matrix = np.array(
            [[43, 24, 33, 34, 49], [24, 36, 44, 15, 44], [33, 44, 37, 1, 30], [34, 15, 1, 5, 46], [49, 44, 30, 46, 22]]
        )
        init_scales = (
            ("auto", np.eye(ndims)),
            (2, np.eye(ndims) * 2),
        ...
        for approx_type in ["hess", "inv_hess"]:
            for init_scale, true_matrix in init_scales:
                ...
                quasi_newton = (
                    BFGS(init_scale=init_scale, min_curvature=1e50, exception_strategy="skip_update"),
    ...
    def test_initialize_catch_illegal(self):
        ...
        # no complex allowed
        inits_msg_errtype = (
            (complex(3.14), r"float\(\) argument must be a string or a " r"(real )?number, not 'complex'", TypeError),
            (
