...
class Fonts(abc.ABC):
    ...
    def render_glyph(
        self, output: Output, ox: float, oy: float, font: str, font_class: str, sym: str, fontsize: float, dpi: float
    ) -> None:
        """
...
class UnicodeFonts(TruetypeFonts):
    ...
    def __init__(self, default_font_prop: FontProperties, load_glyph_flags: LoadFlags):
        ...
        font_cls: type[TruetypeFonts] | None = {"stix": StixFonts, "stixsans": StixSansFonts, "cm": BakomaFonts}.get(
            fallback_rc
        )
    ...
    def _get_glyph(self, fontname: str, font_class: str, sym: str) -> tuple[FT2Font, int, bool]:
        ...
        if not found_symbol:
            if self._fallback_font:
                ...
                _log.warning(
                    "Font %r does not have a glyph for %a [U+%x], " "substituting with a dummy symbol.",
                    new_fontname,
                    sym,
                    uniindex,
                )
...
class Box(Node):
    ...
    def render(self, output: Output, x1: float, y1: float, x2: float, y2: float) -> None:  # type: ignore[override]
        pass
...
class Vlist(List):
    ...
    def __init__(
        self, elements: T.Sequence[Node], h: float = 0.0, m: T.Literal["additional", "exactly"] = "additional"
    ):
        super().__init__(elements)
        self.vpack(h=h, m=m)
...
class Rule(Box):
    ...
    def render(self, output: Output, x: float, y: float, w: float, h: float) -> None:  # type: ignore[override]
...
class Glue(Node):
    ...
    def __init__(
        self,
        glue_type: _GlueSpec | T.Literal["fil", "fill", "filll", "neg_fil", "neg_fill", "neg_filll", "empty", "ss"],
    ):
        super().__init__()
...
class AutoHeightChar(Hlist):
    ...
    def __init__(
        self, c: str, height: float, depth: float, state: ParserState, always: bool = False, factor: float | None = None
    ):
...
class Parser:
    ...
    _small_greek = set(
        [
            ...
            for i in range(ord("\N{GREEK SMALL LETTER ALPHA}"), ord("\N{GREEK SMALL LETTER OMEGA}") + 1)
        ]
    )
    ...
    def __init__(self) -> None:
        ...
        p.substack = cmd(
            r"\substack",
            nested_expr(
                opener="{", closer="}", content=Group(OneOrMore(p.token)) + ZeroOrMore(Literal("\\\\").suppress())
            )("parts"),
        )
    ...
    def symbol(self, s: str, loc: int, toks: ParseResults | dict[str, str]) -> T.Any:
        ...
        if c in self._spaced_symbols:
            ...
            if self._in_subscript_or_superscript or (
                ...
                and (len(s[:loc].split()) == 0 or prev_char in {"{", *self._left_delims, *self._relation_symbols})
            ):
                return [char]
            else:
    ...
    def _genfrac(
        self, ldelim: str, rdelim: str, rule: float | None, style: _MathStyle, num: Hlist, den: Hlist
    ) -> T.Any:
        state = self.get_state()
        thickness = state.get_current_underline_thickness()
        ...
        if ldelim or rdelim:
            ...
            if rdelim == "":
                rdelim = "."
            return self._auto_sized_delimiter(ldelim, T.cast(list[Box | Char | str], result), rdelim)
        return result
    ...
    def _genset(self, s: str, loc: int, toks: ParseResults) -> T.Any:
        ...
        if s[loc + 1] == "u":  # \underset
            vlist = Vlist([centered_body, Vbox(0, vgap), centered_annotation])  # body  # space  # annotation
            ...
            vlist = Vlist([centered_annotation, Vbox(0, vgap), centered_body])  # annotation  # space  # body
