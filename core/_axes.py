...
'polar', 'rectilinear', str}, optional
    ...
    def _parse_bar_color_args(self, kwargs):
        ...
        facecolor = facecolor if facecolor is not None else self._get_patches_for_fill.get_next_color()

        try:
            facecolor = mcolors.to_rgba_array(facecolor)
        except ValueError as err:
            raise ValueError("'facecolor' or 'color' argument must be a valid color or " "sequence of colors.") from err
    ...
    def bar(self, x, height, width=0.8, bottom=None, *, align="center", **kwargs):
        ...
        **kwargs : `.Rectangle` properties
        ...
        patches = []
        args = zip(left, bottom, width, height, facecolor, edgecolor, linewidth, hatch, patch_labels)
        ...
        **kwargs
        ...
        for bar, err, dat, lbl in itertools.zip_longest(bars, errs, datavalues, labels):
            ...
            if label_type == "center":
                xy = (0.5, 0.5)
                kwargs["xycoords"] = (
                    lambda r, b=bar: mtransforms.Bbox.intersection(b.get_window_extent(r), b.get_clip_box())
    ...
    def boxplot(
        ...
        bxpstats = cbook.boxplot_stats(x, whis=whis, bootstrap=bootstrap, labels=tick_labels, autorange=autorange)
        if notch is None:
...
:mpltype:`color`, default: :rc:`scatter.edgecolors`
        ...
        * Fundamentally, scatter works with 1D arrays; *x*, *y*, *s*, and *c*
        ...
        if colors is None:
            if colorizer:
                collection._set_colorizer_check_keywords(colorizer, cmap=cmap, norm=norm, vmin=vmin, vmax=vmax)
            else:
    ...
    def hexbin(
        ...
        **kwargs : `~matplotlib.collections.PolyCollection` properties
        ...
        if cbook._str_equal(bins, "log"):
            if norm is not None:
                _api.warn_external("Only one of 'bins' and 'norm' arguments " f"can be supplied, ignoring {bins=}")
            else:
    ...
    def _fill_between_process_units(self, ind_dir, dep_dir, ind, dep1, dep2, **kwargs):
        ...
        return map(
            np.ma.masked_invalid, self._process_unit_info([(ind_dir, ind), (dep_dir, dep1), (dep_dir, dep2)], kwargs)
        )
    ...
    @_docstring.interpd
    def pcolor(
        self, *args, shading=None, alpha=None, norm=None, cmap=None, vmin=None, vmax=None, colorizer=None, **kwargs
    ):
        r"""
    ...
    @_preprocess_data()
    @_docstring.interpd
    def pcolorfast(self, *args, alpha=None, norm=None, cmap=None, vmin=None, vmax=None, colorizer=None, **kwargs):
        """
        Create a pseudocolor plot with a non-regular rectangular grid.
        ...
        if len(args) == 1:
            ...
            if x.ndim == 1 and y.ndim == 1:
                if x.size == 2 and y.size == 2:
                    ...
                    if x.size != nc + 1:
                        raise ValueError(
                            f"Length of X ({x.size}) must be one larger than the " f"number of columns in C ({nc})"
                        )
                    if y.size != nr + 1:
                        raise ValueError(
                            f"Length of Y ({y.size}) must be one larger than the " f"number of rows in C ({nr})"
                        )
                    dx = np.diff(x)
                    dy = np.diff(y)
        ...
        if style == "quadmesh":
            ...
            if style == "image":
                ...
                im = mimage.PcolorImage(
                    self, x, y, C, cmap=cmap, norm=norm, colorizer=colorizer, alpha=alpha, extent=extent, **kwargs
                )
            self.add_image(im)
            ret = im
...
such objects
    ...
    def violin(
        ...
        line_ends = [[-0.25 if side in ["both", "low"] else 0], [0.25 if side in ["both", "high"] else 0]] * np.array(
            widths
        ) + positions
