...
def print_iteration_header():
    print(f"{'Iteration':^15}{'Max residual':^15}{'Max BC residual':^15}" f"{'Total nodes':^15}{'Nodes added':^15}")


def print_iteration_progress(iteration, residual, bc_residual, total_nodes, nodes_added):
    print(f"{iteration:^15}{residual:^15.2e}{bc_residual:^15.2e}" f"{total_nodes:^15}{nodes_added:^15}")
...
def solve_bvp(
    ...
    if y.shape[1] != x.shape[0]:
        raise ValueError(f"`y` is expected to have {x.shape[0]} columns, but actually " f"has {y.shape[1]}.")
    ...
    if S is not None:
        ...
        if S.shape != (n, n):
            raise ValueError(f"`S` is expected to have shape {(n, n)}, " f"but actually has {S.shape}")
    ...
    f = fun_wrapped(x, y, p)
    if f.shape != y.shape:
        raise ValueError(f"`fun` return is expected to have shape {y.shape}, " f"but actually has {f.shape}.")

    bc_res = bc_wrapped(y[:, 0], y[:, -1], p)
    if bc_res.shape != (n + k,):
        raise ValueError(f"`bc` return is expected to have shape {(n + k,)}, " f"but actually has {bc_res.shape}.")
