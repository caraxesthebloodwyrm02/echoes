...
class Akima1DInterpolator(CubicHermiteSpline):
    ...
    def __init__(self, x, y, axis=0, *, method: Literal["akima", "makima"] = "akima", extrapolate: bool | None = None):
    ...
    def __init__(self, x, y, axis=0, bc_type="not-a-knot", extrapolate=None):
        ...
        if y.size == 0:
            ...
            if n == 3 and bc[0] == "not-a-knot" and bc[1] == "not-a-knot":
                ...
                m = b.shape[0]
                s = solve(A, b.reshape(m, -1), overwrite_a=True, overwrite_b=True, check_finite=False).reshape(b.shape)
                ...
                if bc_start == "periodic":
                    ...
                    m = b1.shape[0]
                    s1 = solve_banded(
                        (1, 1), Ac, b1.reshape(m, -1), overwrite_ab=False, overwrite_b=False, check_finite=False
                    )
                    s1 = s1.reshape(b1.shape)

                    m = b2.shape[0]
                    s2 = solve_banded(
                        (1, 1), Ac, b2.reshape(m, -1), overwrite_ab=False, overwrite_b=False, check_finite=False
                    )
                    ...
                    m = b.shape[0]
                    s = solve_banded(
                        (1, 1), A, b.reshape(m, -1), overwrite_ab=True, overwrite_b=True, check_finite=False
                    )
                    s = s.reshape(b.shape)
        ...
        for bc in bc_type:
            if isinstance(bc, str):
                ...
                if deriv_value.shape != expected_deriv_shape:
                    raise ValueError(
                        f"`deriv_value` shape {deriv_value.shape} is not " f"the expected one {expected_deriv_shape}."
                    )
