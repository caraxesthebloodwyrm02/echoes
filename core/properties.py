...
class ColumnProperty(
    ...
    def columns_to_assign(self) -> List[Tuple[Column[Any], int]]:
        ...
        return [(c, 0) for c in self.columns if isinstance(c, Column) and c.table is None]  # type: ignore
...
class MappedColumn(
    ...
    def declarative_scan(
        ...
        column = self.column = self._adjust_for_existing_column(decl_scan, key, self.column)
    ...
    def declarative_scan_for_composite(
        ...
        self._init_column_for_annotation(cls, decl_scan, key, registry, param_annotation, originating_module)
    ...
    def _init_column_for_annotation(
        ...
        if use_args_from is not None:
            ...
            self.column = use_args_from._adjust_for_existing_column(decl_scan, key, self.column)
        ...
        if sqltype._isnull and not self.column.foreign_keys:
            ...
            for check_type in checks:
                new_sqltype = registry._resolve_type(check_type, _do_fallbacks=check_type is our_type)
                ...
                if isinstance(our_type, TypeEngine) or (
                    ...
                    if len(checks) == 1:
                        ...
                        detail = f"neither '{checks[0]}' nor '{checks[1]}' " "are resolvable by the registry"
                    else:
                        detail = (
                            f"""none of {
