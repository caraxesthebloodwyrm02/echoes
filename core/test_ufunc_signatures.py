...
def test_ufunc_signatures(ufunc):
    ...
    types = set(sig for sig in ufunc.types if not ("l" in sig or "i" in sig or "q" in sig or "p" in sig))
    ...
    expanded_types = set()
    for sig in types:
        expanded_types.update([sig.replace("d", "f").replace("D", "F"), sig.replace("f", "d").replace("F", "D")])
