        ...
        **extraneous,
        ...
        if first_step is None:
            self.h_abs = select_initial_step(
                self.fun, self.t, self.y, t_bound, max_step, self.f, self.direction, 3, self.rtol, self.atol
            )
        else:
    ...
    def _validate_jac(self, jac, sparsity):
        ...
        if jac is None:
            ...
            if J.shape != (self.n, self.n):
                raise ValueError(f"`jac` is expected to have shape {(self.n, self.n)}," f" but actually has {J.shape}.")
        else:
            if issparse(jac):
                J = csc_matrix(jac)
            else:
