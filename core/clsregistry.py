...
def add_class(classname: str, cls: Type[_T], decl_class_registry: _ClsRegistryType) -> None:
    ...
    if classname in decl_class_registry:
        ...
        if not isinstance(existing, _MultipleClassMarker):
            decl_class_registry[classname] = _MultipleClassMarker([cls, cast("Type[Any]", existing)])
    else:
...
class _ModuleMarker(ClsRegistryToken):
    ...
    def add_class(self, name: str, cls: Type[Any]) -> None:
        if name in self.contents:
            ...
            self.contents[name] = _MultipleClassMarker([cls], on_remove=lambda: self._remove_item(name))
