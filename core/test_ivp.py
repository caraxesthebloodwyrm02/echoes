...
def test_duplicate_timestamps():
    ...
    sol = solve_ivp(
        upward_cannon, [0, np.inf], [0, 0.01], max_step=0.05 * 0.001 / 9.80665, events=hit_ground, dense_output=True
    )
...
def test_tbound_respected_larger_interval(method):
    ...
    result = solve_ivp(
        func, (-17, 2), y0=np.array([1, -11]), max_step=0.03, vectorized=False, t_eval=None, atol=1e-8, rtol=1e-5
    )
...
def test_tbound_respected_oscillator(method):
    ...
    def run_sim2(t_end, n_timepoints=10, shortest_delay_line=10000000):
        ...
        t0 = 100.0
        t1 = 200.0
        return solve_ivp(reactions_func, (t0, t1), init_state.copy(), dense_output=True, max_step=t1 - t0)
