...
def getdtype(dtype, a=None, default=None):
    ...
    if newdtype not in supported_dtypes:
        ...
        raise ValueError(
            f"scipy.sparse does not support dtype {newdtype}. " f"The only supported types are: {supported_dtypes_fmt}."
        )
    return newdtype
...
def safely_cast_index_arrays(A, idx_dtype=np.int32, msg=""):
    ...
    if A.format in ("csc", "csr"):
        ...
        raise TypeError(
            f"Format {A.format} is not associated with index arrays. " "DOK and LIL have dict and list, not array."
        )
...
def validateaxis(axis, *, ndim=2) -> tuple[int, ...] | None:
    ...
    if axis == ():
        raise ValueError("sparse does not accept 0D axis (). Either use toarray (for dense) " "or copy (for sparse).")
    ...
    if current_shape is None:
        ...
        if not negative_indexes:
            ...
            if new_size != current_size:
                raise ValueError(f"cannot reshape array of size {current_size}" f" into shape {new_shape}")
            ...
            if remainder != 0:
                ...
                raise ValueError(f"cannot reshape array of size {current_size}" f" into shape {err_shape}")
...
def check_reshape_kwargs(kwargs):
    ...
    if kwargs:  # Some unused kwargs remain
        raise TypeError("reshape() got unexpected keywords arguments: " f"{', '.join(kwargs.keys())}")
    return order, copy
