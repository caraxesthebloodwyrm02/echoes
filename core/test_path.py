...
@pytest.mark.parametrize(
    ...
    [
        (Path([(0.25, 0.25), (0.25, 0.75), (0.75, 0.75), (0.75, 0.25), (0.25, 0.25)], closed=True), True, False),
        (Path([(-0.25, -0.25), (-0.25, 1.75), (1.75, 1.75), (1.75, -0.25), (-0.25, -0.25)], closed=True), False, True),
        (
            Path([(-0.25, -0.25), (-0.25, 1.75), (0.5, 0.5), (1.75, 1.75), (1.75, -0.25), (-0.25, -0.25)], closed=True),
            False,
            False,
        ),
        (Path([(0.25, 0.25), (0.25, 1.25), (1.25, 1.25), (1.25, 0.25), (0.25, 0.25)], closed=True), False, False),
...
def test_nonlinear_containment():
    ...
    assert polygon.get_path().contains_point(ax.transData.transform((5, 0.5)), polygon.get_transform())
    assert not polygon.get_path().contains_point(ax.transData.transform((0.5, 0.5)), polygon.get_transform())
    assert not polygon.get_path().contains_point(ax.transData.transform((50, 0.5)), polygon.get_transform())


@image_comparison(
    ["arrow_contains_point.png"], remove_text=True, style="mpl20", tol=0 if platform.machine() == "x86_64" else 0.027
)
