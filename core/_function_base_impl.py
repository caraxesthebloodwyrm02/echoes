...
def disp(mesg, device=None, linefeed=True):
    ...
    warnings.warn(
        "`disp` is deprecated, " "use your own printing function instead. " "(deprecated in NumPy 2.0)",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    if weights is None:
        ...
        if out is None:
            ...
            if out.shape != r_shape:
                msg = f"Wrong shape of argument 'out', shape={r_shape} is " f"required; got shape={out.shape}."
                raise ValueError(msg)
            result = out
