...
class HBInfo:
    ...
    def from_file(cls, fid):
        ...
        if mxtype.value_type not in ["real", "integer"]:
            raise ValueError("Only real or integer matrices supported for " f"now (detected {mxtype})")
    ...
    def __init__(
        ...
        if isinstance(values_format, ExpFormat):
            if mxtype.value_type not in ["real", "complex"]:
                raise ValueError(f"Inconsistency between matrix type {mxtype} and " f"value type {values_format}")
