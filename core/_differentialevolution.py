...
from scipy._lib._util import check_random_state, MapWrapper, _FunctionWrapper, rng_integers, _transition_to_rng
...
class DifferentialEvolutionSolver:
    ...
    def _mutate_custom(self, candidate):
        ...
        if not len(np.shape(candidate)):
            ...
            S = candidate.shape[0]
            trial = np.array([self.strategy(c, _population, rng=rng) for c in candidate], dtype=float)
