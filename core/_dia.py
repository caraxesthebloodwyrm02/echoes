...
from ._sputils import isdense, isscalarlike, isshape, upcast_char, getdtype, get_sum_dtype, validateaxis, check_shape
...
class _dia_base(_data_matrix):
        ...
        if issparse(arg1):
            ...
            if isshape(arg1):
                ...
                try:
                    ...
                    offsets = np.array(arg1[1], dtype=self._get_index_dtype(maxval=max(shape)), copy=copy)
            ...
            try:
                ...
                raise ValueError("unrecognized form for " f"{self.format}_matrix constructor") from e
    ...
    def count_nonzero(self, axis=None):
        if axis is not None:
            raise NotImplementedError("count_nonzero over an axis is not implemented for DIA format")
    ...
    def _matmul_multivector(self, other):
        ...
        dia_matvecs(*self.shape, *self.data.shape, self.offsets, self.data, other.shape[1], other, res)
        return res
    ...
    def _matmul_sparse(self, other):
        ...
        return self._dia_container((data.reshape(len(offsets), -1), offsets), (self.shape[0], other.shape[1]))
    ...
    def tocoo(self, copy=False):
        ...
        return self._coo_container((data, (row, col)), shape=self.shape, dtype=self.dtype, copy=False)
