    ...
    for i, p in enumerate(points):
        ...
        if not np.all(p[1:] > p[:-1]):
            if np.all(p[1:] < p[:-1]):
                ...
                raise ValueError(f"The points in dimension {i} must be strictly ascending or " f"descending")
...
def _check_dimensionality(points, values):
    if len(points) > values.ndim:
        raise ValueError(f"There are {len(points)} point arrays, but values has " f"{values.ndim} dimensions")
    for i, p in enumerate(points):
        if not np.asarray(p).ndim == 1:
            raise ValueError(f"The points in dimension {i} must be 1-dimensional")
        if not values.shape[i] == len(p):
            raise ValueError(f"There are {len(p)} points and {values.shape[i]} values in " f"dimension {i}")
...
class RegularGridInterpolator:
    ...
    **Interpolate and extrapolate a 2D dataset**
    ...
    def __init__(
        self, points, values, method="linear", bounds_error=True, fill_value=np.nan, *, solver=None, solver_args=None
    ):
    ...
    def _construct_spline(self, method, solver=None, **solver_args):
        if solver is None:
            solver = ssl.gcrotmk
        spl = make_ndbspl(self.grid, self.values, self._SPLINE_DEGREE_MAP[method], solver=solver, **solver_args)
        return spl
    ...
    def __call__(self, xi, method=None, *, nu=None):
        ...
        if nu is not None and method not in self._SPLINE_METHODS_ndbspl:
            raise ValueError(
                f"Can only compute derivatives for methods " f"{self._SPLINE_METHODS_ndbspl}, got {method =}."
            )
