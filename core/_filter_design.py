...
def findfreqs(num, den, N, kind="ba"):
    ...
    lfreq = xp.round(xp.log10(0.1 * xp.min(xp.abs(xp.real(ez + integ)) + 2 * xp.imag(ez))) - 0.5 - fudge)
...
def freqz(b, a=1, worN=512, whole=False, plot=None, fs=2 * pi, include_nyquist=False):
    ...
    if _is_int_type(worN):
        ...
        if (
            xp_size(a) == 1 and (b.ndim == 1 or (b.shape[-1] == 1)) and n_fft >= b.shape[0] and n_fft > 0
    ...
    if h is None:  # still need to compute using freqs w
        zm1 = xp.exp(-1j * w)
        h = _pu.npp_polyval(zm1, b, tensor=False, xp=xp) / _pu.npp_polyval(zm1, a, tensor=False, xp=xp)
...
def iirdesign(wp, ws, gpass, gstop, analog=False, ftype="ellip", output="ba", fs=None):
    ...
    try:
        ...
        raise ValueError(f"{ftype} does not have order selection. " "Use iirfilter function.") from e
    ...
    if wp.shape[0] != ws.shape[0] or wp.shape not in [(1,), (2,)]:
        raise ValueError(
            "wp and ws must have one or two elements each, and " f"the same shape, got {wp.shape} and {ws.shape}"
        )
    ...
    if not analog:
        if fs is None:
            ...
            raise ValueError("Values for wp, ws must be less than fs/2 " f"(fs={fs} -> fs/2={fs/2})")
...
def _find_nat_freq(stopb, passb, gpass, gstop, filter_type, filter_kind):
    if filter_type == 1:  # low
        ...
        wp0 = optimize.fminbound(
            band_stop_obj, passb[0], stopb[0] - 1e-12, args=(0, passb, stopb, gpass, gstop, filter_kind), disp=0
        )
        passb[0] = wp0
        wp1 = optimize.fminbound(
            band_stop_obj, stopb[1] + 1e-12, passb[1], args=(1, passb, stopb, gpass, gstop, filter_kind), disp=0
        )
        passb[1] = wp1
...
def ellipap(N, rp, rs):
    ...
    if N % 2:
        newp = np.compress(abs(p.imag) > EPSILON * np.sqrt(np.sum(p * np.conjugate(p), axis=0).real), p, axis=-1)
...
def gammatone(freq, ftype, order=None, numtaps=None, fs=None):
    ...
    if not 0 < freq < fs / 2:
        raise ValueError(f"The frequency must be between 0 and {fs / 2}" f" (Nyquist), but given {freq}.")
