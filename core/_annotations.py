        ...
        while current := self.fetch_token():
            if current == token.NAME:
                ...
                if tp_kind != Parameter.POSITIONAL_OR_KEYWORD and tp_ann != Parameter.empty:
                    msg = "type parameter bound or constraint is not allowed " f"for {tp_kind.description} parameters"
...
def _parse_type_list(
    ...
    for tp_name, tp_kind, tp_default, tp_ann in parser.type_params:
        ...
        if tp_kind in {Parameter.POSITIONAL_ONLY, Parameter.KEYWORD_ONLY}:
            msg = "positional-only or keyword-only parameters " "are prohibited in type parameter lists"
            raise SyntaxError(msg)
...
def _positional_only_separator() -> addnodes.desc_parameter:
    ...
    positional_only_abbr = nodes.abbreviation("/", "/", explanation=_("Positional-only parameter separator (PEP 570)"))
...
def _keyword_only_separator() -> addnodes.desc_parameter:
    ...
    keyword_only_abbr = nodes.abbreviation("*", "*", explanation=_("Keyword-only parameters separator (PEP 3102)"))
    keyword_only_op = addnodes.desc_sig_operator("*", "", keyword_only_abbr, classes=["keyword-only-separator"])
