...
class HTML5Translator(SphinxTranslator, BaseTranslator):  # type: ignore[misc]
    ...
    def depart_desc_optional(self, node: Element) -> None:
        ...
        if self.multi_line_parameter_list:
            ...
            if level == max_level - 1 and (not is_last_group or level > 0 or self.trailing_comma):
                self.body.append(self.param_separator)
            self.body.append('<span class="optional">]</span>')
    ...
    def visit_admonition(self, node: Element, name: str = "") -> None:
        ...
        self.body.append(self.starttag(node, tag_name, CLASS=f"admonition {name}", **attributes))
        ...
        if (
            ...
            if close_tag.startswith("</h"):
                ...
                self.body.append(
                    ...
                    + 'title="{}">{}'.format(_("Link to this heading"), self.config.html_permalinks_icon)
                )
