    ...
    **Implementation details**
    ...
    def from_power_basis(cls, pp, bc_type="not-a-knot"):
        ...
        if not isinstance(pp, CubicSpline):
            raise NotImplementedError(f"Only CubicSpline objects are accepted " f"for now. Got {type(pp)} instead.")
        x = pp.x
        coef = pp.c
        k = pp.c.shape[0] - 1
        n = x.shape[0]
...
def make_interp_spline(x, y, k=3, t=None, bc_type=None, axis=0, check_finite=True):
    ...
    if nt - n != nleft + nright:
        raise ValueError(
            "The number of derivatives at boundaries does not " f"match: expected {nt-n}, got {nleft}+{nright}"
        )
...
def _compute_optimal_gcv_parameter(X, wE, y, w):
    ...
    if y.ndim == 1:
        ...
        for i in range(y.shape[1]):
            est = minimize_scalar(_gcv, bounds=(0, n), method="Bounded", args=(X, XtWX, wE, XtE, y[:, i]))
            if est.success:
                gcv_est[i] = est.x
            else:
                raise ValueError(f"Unable to find minimum of the GCV " f"function: {gcv_est.message}")
