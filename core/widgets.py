...
class SubplotTool(Widget):
    ...
    def __init__(self, targetfig, toolfig):
        ...
        (self.sliderleft, self.sliderbottom, self.sliderright, self.slidertop, self.sliderwspace, self.sliderhspace) = (
            self._sliders
        )
...
class MultiCursor(Widget):
    ...
    **lineprops
    ...
    def onmove(self, event):
        ...
        xdata, ydata = (
            (event.xdata, event.ydata) if event.inaxes is ax else ax.transData.inverted().transform((event.x, event.y))
        )
...
class _SelectorWidget(AxesWidget):
    ...
    def __init__(
        self, ax, onselect=None, useblit=False, button=None, state_modifier_keys=None, use_data_coordinates=False
    ):
        super().__init__(ax)
