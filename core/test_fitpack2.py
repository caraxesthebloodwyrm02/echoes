...
from scipy._lib._array_api import xp_assert_equal, xp_assert_close, assert_almost_equal, assert_array_almost_equal
...
class TestLSQBivariateSpline:
    ...
    def test_integral(self):
        ...
        assert_almost_equal(np.asarray(lut.integral(tx[0], tx[-1], ty[0], ty[-1])), np.asarray(trpz))
...
class TestSmoothBivariateSpline:
    ...
    def test_integral(self):
        ...
        assert_almost_equal(np.asarray(lut.integral(tx[0], tx[-2], ty[0], ty[-2])), np.asarray(trpz))
...
class TestRectBivariateSpline:
    ...
    @pytest.mark.slow()
    @pytest.mark.skipif(sys.maxsize <= 2**32, reason="Segfaults on 32-bit system " "due to large input data")
...
class TestRectSphereBivariateSpline:
    ...
    def test_derivatives_grid(self):
        ...
        xp_assert_close(lut(x, y, dtheta=1, dphi=1), _numdiff_2d(lut, x, y, dx=1, dy=1, eps=1e-6), rtol=1e-3, atol=1e-3)
        ...
        xp_assert_equal(lut(x, y, dtheta=1, grid=False), lut.partial_derivative(1, 0)(x, y, grid=False))
        xp_assert_equal(lut(x, y, dphi=1, grid=False), lut.partial_derivative(0, 1)(x, y, grid=False))
        xp_assert_equal(lut(x, y, dtheta=1, dphi=1, grid=False), lut.partial_derivative(1, 1)(x, y, grid=False))
