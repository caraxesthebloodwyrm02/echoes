...
def _calc_dual_canonical_window(win: np.ndarray, hop: int) -> np.ndarray:
    ...
    """
    if hop > len(win):
        raise ValueError(f"{hop=} is larger than window length of {len(win)}" + " => STFT not invertible!")
    if issubclass(win.dtype.type, np.integer):
        raise ValueError("Parameter 'win' cannot be of integer type, but " + f"{win.dtype=} => STFT not invertible!")
...
class ShortTimeFFT:
    ...
    _cache_t: tuple[tuple[int, int | None, int | None, int, float], np.ndarray] = (-1, None, None, 0, 0.0), np.ndarray(
        []
    )
    _cache_f: tuple[tuple[FFT_MODE_TYPE, int, float], np.ndarray] = ("onesided", -1, 1.0), np.ndarray([])
    ...
    def from_window(
        ...
        *,
        ...
        return cls(
            win, hop=nperseg - noverlap, fs=fs, fft_mode=fft_mode, mfft=mfft, scale_to=scale_to, phase_shift=phase_shift
        )
    ...
    def from_win_equals_dual(
        ...
        *,
        ...
        if not (1 <= hop <= len(desired_win) and isinstance(hop, int | np.integer)):
            raise ValueError(f"Parameter {hop=} is not an integer between 1 and " + f"{len(desired_win)=}!")
        if scale_to not in ["magnitude", "psd", "unitary", None]:
            raise ValueError(f"Parameter {scale_to=} not in " + "['magnitude', 'psd', 'unitary', None]!")
        ...
        for m in range(hop):
            ...
            if not (a > relative_resolution):
                raise ValueError("Parameter desired_win does not have valid STFT dual " f"window for {hop=}!")
            win[m::hop] /= a
    ...
    def fft_mode(self, t: FFT_MODE_TYPE):
        ...
        if t in {"onesided", "onesided2X"} and np.iscomplexobj(self.win):
            raise ValueError(
                f"One-sided spectra, i.e., fft_mode='{t}', " + "are not allowed for complex-valued windows!"
            )

        if t == "onesided2X" and self.scaling is None:
            raise ValueError(
                f"For scaling is None, fft_mode='{t}' is invalid!" "Do scale_to('psd') or scale_to('magnitude')!"
            )
        self._fft_mode = t
    ...
    def phase_shift(self, v: int | None):
        ...
        if not isinstance(v, int | np.integer):
            raise ValueError(
                f"phase_shift={v} has the unit samples. Hence " + "it needs to be an int or it may be None!"
            )
    ...
    def istft(
        self, S: np.ndarray, k0: int = 0, k1: int | None = None, *, f_axis: int = -2, t_axis: int = -1
    ) -> np.ndarray:
        ...
        if S.shape[f_axis] != self.f_pts:
            raise ValueError(f"{S.shape[f_axis]=} must be equal to " + f"{self.f_pts=} ({S.shape=})!")
        n_min = self.m_num - self.m_num_mid  # minimum signal length
        if not (S.shape[t_axis] >= (q_num := self.p_num(n_min))):
            raise ValueError(f"{S.shape[t_axis]=} needs to have at least " + f"{q_num} slices ({S.shape=})!")
        ...
        if not (num_pts := k1 - k0) >= n_min:
            raise ValueError(
                f"({k1=}) - ({k0=}) = {num_pts} has to be at " + f"least the half the window length {n_min}!"
            )
    ...
    def extent(
        ...
        if self.onesided_fft:
            ...
            raise ValueError(
                f"Attribute fft_mode={self.fft_mode} must be " + "in ['centered', 'onesided', 'onesided2X']"
            )
