        ...
        for field, value in source.items():
            if isinstance(field, str):
                if field[0] not in "_0123456789":
                    ...
                    msg = f"Starting field name with a underscore " f"or a digit ({field}) is ignored"
...
class MatFile5Writer:
    ...
    def write_file_header(self):
        ...
        hdr["description"] = f"MATLAB 5.0 MAT-file Platform: {os.name}, " f"Created on: {time.asctime()}"
        hdr["version"] = 0x0100
