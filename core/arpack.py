...
class IterInv(LinearOperator):
    ...
    def _matvec(self, x):
        ...
        if info != 0:
            raise ValueError(
                f"Error in inverting M: function {self.ifunc.__name__} " f"did not converge (info = {info})."
            )
        return b
...
class IterOpInv(LinearOperator):
    ...
    def _matvec(self, x):
        ...
        if info != 0:
            raise ValueError(
                f"Error in inverting [A-sigma*M]: function {self.ifunc.__name__} " f"did not converge (info = {info})."
            )
        return b
