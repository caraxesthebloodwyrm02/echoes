...
def root(fun, x0, args=(), method="hybr", jac=None, tol=None, callback=None, options=None):
    ...
    **Large problem**
    ...
    if meth == "hybr":
        ...
        _warn_jac_unused(jac, method)
        sol = _root_nonlin_solve(_wrapped_fun, x0, args=args, jac=jac, _method=meth, _callback=callback, **options)
    else:
...
def _root_leastsq(
    ...
    **unknown_options,
    ...
    sol = OptimizeResult(
        x=x, message=msg, status=ier, success=ier in (1, 2, 3, 4), cov_x=cov_x, fun=info.pop("fvec"), method="lm"
    )
    sol.update(info)
    sol.nfev = nfev
    return sol
