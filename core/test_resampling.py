...
def test_bootstrap_batch(method, axis):
    ...
    rng = np.random.RandomState(0)
    res2 = bootstrap((x,), np.mean, batch=10, method=method, axis=axis, n_resamples=100, random_state=rng)
...
def test_bootstrap_vectorized(method, axis, paired):
    ...
    res1 = bootstrap((x, y, z), my_statistic, paired=paired, method=method, rng=0, axis=0, n_resamples=100)
    ...
    res2 = bootstrap((x, y, z), my_statistic, paired=paired, method=method, rng=0, axis=axis, n_resamples=100)
...
def test_bootstrap_vectorized_3samp(method, axis):
    ...
    y = rng.rand(4, 5)
    z = rng.rand(4, 5)
    res1 = bootstrap((x, y, z), statistic, vectorized=True, axis=axis, n_resamples=100, method=method, rng=0)
    res2 = bootstrap((x, y, z), statistic_1d, vectorized=False, axis=axis, n_resamples=100, method=method, rng=0)
    assert_allclose(res1.confidence_interval, res2.confidence_interval)
    assert_allclose(res1.standard_error, res2.standard_error)
...
def test_bootstrap_vectorized_1samp(method, axis):
    ...
    rng = np.random.RandomState(0)
    x = rng.rand(4, 5)
    res1 = bootstrap((x,), statistic, vectorized=True, axis=axis, n_resamples=100, batch=None, method=method, rng=0)
    res2 = bootstrap((x,), statistic_1d, vectorized=False, axis=axis, n_resamples=100, batch=10, method=method, rng=0)
    assert_allclose(res1.confidence_interval, res2.confidence_interval)
    assert_allclose(res1.standard_error, res2.standard_error)
...
def test_bootstrap_gh15678(method):
    ...
    data = (data,)
    res = bootstrap(data, stats.skew, method=method, n_resamples=100, rng=np.random.default_rng(9563))
    # this always worked because np.apply_along_axis returns NumPy data type
    ref = bootstrap(data, stats.skew, method=method, n_resamples=100, rng=np.random.default_rng(9563), vectorized=False)
...
def test_bootstrap_min():
    ...
    res2 = bootstrap(-np.array(data), np.max, method="BCa", n_resamples=100, rng=np.random.default_rng(3942))
...
def test_re_bootstrap(additional_resamples):
    ...
    res = stats.bootstrap((x,), np.mean, n_resamples=n1, rng=rng, confidence_level=0.95, method="percentile")
    res = stats.bootstrap(
        (x,), np.mean, n_resamples=n2, rng=rng, confidence_level=0.90, method="BCa", bootstrap_result=res
    )

    rng = np.random.default_rng(296689032789913033)
    ref = stats.bootstrap((x,), np.mean, n_resamples=n3, rng=rng, confidence_level=0.90, method="BCa")
...
def test_vector_valued_statistic(method):
    ...
    res = bootstrap((sample,), statistic, method=method, axis=-1, n_resamples=9999, batch=200, random_state=rng)
...
class TestPower:
            ...
            for nx_i in nx:
                for alternative_i in kwargs["alternative"]:
                    for popmean_i in popmeans:
                        ...
                        def test2(x, axis=-1):
                            return stats.ttest_1samp(x, popmean_i, axis=axis, alternative=alternative_i)
    ...
    def test_ttest_1samp_power(self):
        ...
        res = stats.power(stats.ttest_1samp, rng.normal, n_observations, significance=significance, kwargs=kwargs)
...
class TestPermutationTest:
    ...
    def test_randomized_test_against_exact_samples(self):
        ...
        kwds = {"vectorized": True, "permutation_type": "samples", "batch": 100, "alternative": alternative, "rng": rng}
        res = permutation_test(data, statistic, n_resamples=permutations, **kwds)
        res2 = permutation_test(data, statistic, n_resamples=np.inf, **kwds)
    ...
    def test_against_ks_2samp(self, alternative):
        ...
        res = permutation_test((x, y), statistic1d, n_resamples=np.inf, alternative="greater", rng=self.rng)
    ...
    def test_against_ansari(self, alternative):
        ...
        res = permutation_test((x, y), statistic1d, n_resamples=np.inf, alternative=alternative, rng=self.rng)
    ...
    def test_against_mannwhitneyu(self, alternative):
        ...
        res = permutation_test(
            (x, y), statistic, vectorized=True, n_resamples=np.inf, alternative=alternative, axis=1, rng=self.rng
        )
    ...
    def test_against_kendalltau(self):
        ...
        res = permutation_test((x,), statistic1d, permutation_type="pairings", n_resamples=np.inf, rng=self.rng)
    ...
    def test_against_fisher_exact(self, alternative):
        ...
        res = permutation_test(
            (x,), statistic, permutation_type="pairings", n_resamples=np.inf, alternative=alternative, rng=rng
        )
    ...
    def test_vectorized_nsamp_ptype_samples(self, axis):
        ...
        kwds = {"vectorized": False, "axis": axis, "alternative": "greater", "permutation_type": "pairings", "rng": 0}
        res = permutation_test(data, statistic1d, n_resamples=np.inf, **kwds)
