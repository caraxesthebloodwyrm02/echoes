...
from scipy.sparse import issparse, SparseEfficiencyWarning, csr_array, csc_array, eye_array, diags_array
...
def spsolve(A, b, permc_spec=None, use_umfpack=True):
    ...
    if not (issparse(A) and A.format in ("csc", "csr")):
        A = csc_array(A)
        warn("spsolve requires A be CSC or CSR matrix format", SparseEfficiencyWarning, stacklevel=2)
    ...
    if b_is_vector and use_umfpack:
        ...
        if A.dtype.char not in "dD":
            raise ValueError(
                "convert matrix data to double, please, using" " .astype(), or set linsolve.useUmfpack.u = False"
            )
...
def splu(A, permc_spec=None, diag_pivot_thresh=None, relax=None, panel_size=None, options=None):
    ...
    return _superlu.gstrf(
        N, A.nnz, A.data, indices, indptr, csc_construct_func=csc_construct_func, ilu=False, options=_options
    )
...
def spilu(
    ...
    return _superlu.gstrf(
        N, A.nnz, A.data, indices, indptr, csc_construct_func=csc_construct_func, ilu=True, options=_options
    )
...
def spsolve_triangular(A, b, lower=True, overwrite_A=False, overwrite_b=False, unit_diagonal=False):
    ...
    if not (issparse(A) and A.format == "csc"):
        warn("CSC or CSR matrix format is required. Converting to CSC matrix.", SparseEfficiencyWarning, stacklevel=2)
        A = csc_array(A)
    ...
    x, info = _superlu.gstrs(trans, N, L.nnz, L.data, L.indices, L.indptr, N, U.nnz, U.data, U.indices, U.indptr, b)
    if info:
...
def is_sptriangular(A):
    ...
    if not (issparse(A) and A.format in ("csc", "csr", "coo", "dia", "dok", "lil")):
        warn(
            "is_sptriangular needs sparse and not BSR format. Converting to CSR.", SparseEfficiencyWarning, stacklevel=2
        )
        A = csr_array(A)
...
def spbandwidth(A):
    ...
    if not (issparse(A) and A.format in ("csc", "csr", "coo", "dia", "dok")):
        warn(
            "spbandwidth needs sparse format not LIL and BSR. Converting to CSR.", SparseEfficiencyWarning, stacklevel=2
        )
        A = csr_array(A)
