...
class Rectangle(Patch):
    ...
    def rotation_point(self, value):
        if value in ["center", "xy"] or (
            isinstance(value, tuple) and len(value) == 2 and isinstance(value[0], Real) and isinstance(value[1], Real)
        ):
...
class Arc(Ellipse):
    ...
    def draw(self, renderer):
        ...
        # ellipse.
        box_path_transform = (
            transforms.BboxTransformTo((self.axes or self.get_figure(root=False)).bbox) - self.get_transform()
        )
...
class ConnectionPatch(FancyArrowPatch):
    ...
    def _get_xy(self, xy, s, axes=None):
        ...
        if s == "data":
            ...
            return (
                self._get_xy(self.xy, self.xycoords) + xy * self.get_figure(root=True).dpi / 72  # converted data point
            )  # converted offset
        elif s == "polar":
            theta, r = x, y
            x = r * np.cos(theta)
