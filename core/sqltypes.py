...
class String(Concatenable, TypeEngine[str]):
    ...
    def result_processor(self, dialect: Dialect, coltype: object) -> Optional[_ResultProcessorType[str]]:
        return None
...
class SchemaType(SchemaEventTarget, TypeEngineMixin):
    ...
    def _on_metadata_create(self, target: Any, bind: _CreateDropBind, **kw: Any) -> None:
        if not self._is_impl_for_variant(bind.dialect, kw):
            return
    ...
    def _is_impl_for_variant(self, dialect: Dialect, kw: Dict[str, Any]) -> Optional[bool]:
        ...
        def _we_are_the_impl(typ: SchemaType) -> bool:
            return typ is self or isinstance(typ, ARRAY) and typ.item_type is self  # type: ignore[comparison-overlap]
...
class Enum(String, SchemaType, Emulated, TypeEngine[Union[str, enum.Enum]]):
    ...
    def _parse_into_values(self, enums: _EnumTupleArg, kw: Any) -> Tuple[Sequence[str], _EnumTupleArg]:
...
class JSON(Indexable, TypeEngine[Any]):
    ...
    **Customizing the JSON Serializer**
    ...
    class JSONElementType(TypeEngine[Any]):
        ...
        def string_bind_processor(self, dialect: Dialect) -> Optional[_BindProcessorType[str]]:
            return self._string._cached_bind_processor(dialect)

        def string_literal_processor(self, dialect: Dialect) -> Optional[_LiteralProcessorType[str]]:
...
class ARRAY(SchemaEventTarget, Indexable, Concatenable, TypeEngine[Sequence[_T]]):
    ...
    * When used with the Python ``getitem`` accessor, the number of dimensions
    ...
    class Comparator(
        ...
        @overload
        def _setup_getitem(self, index: int) -> Tuple[OperatorType, int, TypeEngine[Any]]: ...

        @overload
        def _setup_getitem(self, index: slice) -> Tuple[OperatorType, Slice, TypeEngine[Any]]: ...
        ...
        def any(self, other: Any, operator: Optional[OperatorType] = None) -> ColumnElement[bool]:
        ...
        @util.preload_module("sqlalchemy.sql.elements")
        def all(self, other: Any, operator: Optional[OperatorType] = None) -> ColumnElement[bool]:
...
class TupleType(TypeEngine[Tuple[Any, ...]]):
    ...
    def coerce_compared_value(self, op: Optional[OperatorType], value: Any) -> TypeEngine[Any]:
        if value is type_api._NO_VALUE_IN_LIST:
            ...
            return TupleType(*[typ.coerce_compared_value(op, elem) for typ, elem in zip(self.types, value)])
    ...
    def _resolve_values_to_types(self, value: Any) -> TupleType:
        if self._fully_typed:
            ...
            return TupleType(
                *[_resolve_value_to_type(elem) if typ is NULLTYPE else typ for typ, elem in zip(self.types, value)]
            )
...
class Uuid(Emulated, TypeEngine[_UUID_RETURN]):
    ...
    **Python uuid objects**, from the Python
    ...
    def bind_processor(self, dialect: Dialect) -> Optional[_BindProcessorType[_UUID_RETURN]]:
