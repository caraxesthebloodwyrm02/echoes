...
class ScalarFunction:
    ...
    def __init__(
        ...
        if isinstance(hess, HessianUpdateStrategy):
            ...
            if callable(hess):
                self._wrapped_hess = _ScalarHessWrapper(hess, x0=x0, args=args, finite_diff_options=finite_diff_options)
                self.H = self._wrapped_hess.H
                self.H_updated = True
            elif hess in FD_METHODS:
                self._wrapped_hess = _ScalarHessWrapper(
                    hess, x0=x0, args=args, grad=self._wrapped_grad, finite_diff_options=finite_diff_options
                )
                self._update_grad()
...
class _VectorHessWrapper:
    ...
    def _fd_hess(self, x, v, J0=None):
        ...
        H = approx_derivative(self.jac_dot_v, x, f0=J0.T.dot(v), args=(v,), **self.finite_diff_options)
        return H
...
class VectorFunction:
    ...
    def __init__(
        ...
        if not (callable(hess) or hess in FD_METHODS or isinstance(hess, HessianUpdateStrategy)):
            raise ValueError("`hess` must be either callable," f"HessianUpdateStrategy or one of {FD_METHODS}.")
        ...
        if callable(jac):
            ...
            self.J, dct = approx_derivative(self.fun_wrapped, self.x, f0=self.f, **finite_diff_options)
            self.J_updated = True
            self._nfev += dct["nfev"]
        ...
        self.jac_wrapped = _VectorJacWrapper(
            jac, fun=self.fun_wrapped, finite_diff_options=finite_diff_options, sparse_jacobian=self.sparse_jacobian
        )

        self.hess_wrapped = _VectorHessWrapper(hess, jac=self.jac_wrapped, finite_diff_options=finite_diff_options)
