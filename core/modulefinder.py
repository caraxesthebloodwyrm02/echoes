...
class FindModuleCache:
    ...
    def find_module_via_source_set(self, id: str) -> ModuleSearchResult | None:
        ...
        if p and self.fscache.isfile(p):
            ...
            for _ in range(id.count(".")):
                if not any(self.fscache.isfile(os_path_join(d, "__init__" + x)) for x in PYTHON_EXTENSIONS):
                    return None
                d = os.path.dirname(d)
            return p
    ...
    def _find_module_non_stub_helper(self, id: str, pkg_dir: str) -> OnePackageDir | ModuleNotFoundReason:
...
def verify_module(fscache: FileSystemCache, id: str, path: str, prefix: str) -> bool:
    ...
    for i in range(id.count(".")):
        path = os.path.dirname(path)
        if not any(
            fscache.isfile_case(os_path_join(path, f"__init__{extension}"), prefix) for extension in PYTHON_EXTENSIONS
        ):
            return False
    return True
...
def default_lib_path(data_dir: str, pyversion: tuple[int, int], custom_typeshed_dir: str | None) -> list[str]:
    ...
    if custom_typeshed_dir:
        ...
        if not os.path.isdir(typeshed_dir) or not os.path.isfile(versions_file):
            print(
                "error: --custom-typeshed-dir does not point to a valid typeshed ({})".format(custom_typeshed_dir),
                file=sys.stderr,
            )
            sys.exit(2)
    else:
