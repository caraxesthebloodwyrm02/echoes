    ...
    if isinstance(declared, UnionType):
        ...
        return make_simplified_union(
            [
                ...
                # y: int | None
                # x = y
                if (is_overlapping_types(d, n, ignore_promotions=True) or is_subtype(n, d, ignore_promotions=False))
            ]
        )
...
def is_enum_overlapping_union(x: ProperType, y: ProperType) -> bool:
    ...
    return (
        ...
        and any(
            isinstance(p := get_proper_type(z), LiteralType) and x.type == p.fallback.type for z in y.relevant_items()
        )
    )
...
def are_related_types(left: Type, right: Type, *, proper_subtype: bool, ignore_promotions: bool) -> bool:
    if proper_subtype:
        return is_proper_subtype(left, right, ignore_promotions=ignore_promotions) or is_proper_subtype(
...
def is_overlapping_types(
    ...
    if are_related_types(left, right, proper_subtype=overlap_for_overloads, ignore_promotions=ignore_promotions):
        return True
    ...
    if isinstance(left, Instance) and isinstance(right, Instance):
        ...
        if right.type.has_type_var_tuple_type:
            ...
            left_prefix, left_middle, left_suffix = split_with_prefix_and_suffix(left.args, prefix, suffix)
        ...
        if len(left_args) == len(right_args):
            ...
            if all(_is_overlapping_types(left_arg, right_arg) for left_arg, right_arg in zip(left_args, right_args)):
                return True
...
def are_tuples_overlapping(left: Type, right: Type, is_overlapping: Callable[[Type, Type], bool]) -> bool:
...
def typed_dict_mapping_overlap(left: Type, right: Type, overlapping: Callable[[Type, Type], bool]) -> bool:
    ...
    * A TypedDict with at least one ReadOnly[] key does not overlap
    ...
    mutable_mapping = next((base for base in other.type.mro if base.fullname == "typing.MutableMapping"), None)
