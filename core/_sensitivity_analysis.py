...
class SobolResult:
    ...
    def bootstrap(
        ...
        first_order = BootstrapResult(
            confidence_interval=ConfidenceInterval(res.confidence_interval.low[0], res.confidence_interval.high[0]),
            bootstrap_distribution=res.bootstrap_distribution[0],
            standard_error=res.standard_error[0],
        )
        total_order = BootstrapResult(
            confidence_interval=ConfidenceInterval(res.confidence_interval.low[1], res.confidence_interval.high[1]),
...
def sobol_indices(*, func, n, dists=None, method="saltelli_2010", rng=None):
    ...
    if not callable(method):
        ...
        try:
            ...
            message = (
                f"{method!r} is not a valid 'method'. It must be one of" f" {set(indices_methods)!r} or a callable."
            )
            raise ValueError(message) from exc
    else:
    ...
    if callable(func):
        ...
        try:
            f_A, f_B, f_AB = map(lambda arr: arr.copy(), np.atleast_2d(func["f_A"], func["f_B"], func["f_AB"]))
        except KeyError as exc:
            raise ValueError(message) from exc
    ...
    res = dict(
        first_order=first_order, total_order=total_order, _indices_method=indices_method, _f_A=f_A, _f_B=f_B, _f_AB=f_AB
    )
