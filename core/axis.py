...
class Tick(martist.Artist):
    ...
    def _apply_params(self, **kwargs):
        ...
        label_kw = {k[5:]: v for k, v in kwargs.items() if k in ["labelsize", "labelcolor", "labelfontfamily"]}
...
class Axis(martist.Artist):
    ...
    _tick_class = None
    converter = _api.deprecate_privatize_attribute("3.10", alternative="get_converter and set_converter methods")
    ...
    def set_label(self, s):
        ...
        raise RuntimeError(
            "A legend label cannot be assigned to an Axis. Did you mean to " "set the axis label via set_label_text()?"
        )
...
class XAxis(Axis):
    ...
    def get_tick_space(self):
        ends = mtransforms.Bbox.unit().transformed(self.axes.transAxes - self.get_figure(root=False).dpi_scale_trans)
...
class YAxis(Axis):
    ...
    def _update_offset_text_position(self, bboxes, bboxes2):
        ...
        if "outline" in self.axes.spines:
            ...
            bbox = self.axes.bbox
        top = bbox.ymax
        self.offsetText.set_position((x, top + self.OFFSETTEXTPAD * self.get_figure(root=True).dpi / 72))
