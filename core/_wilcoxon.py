...
class WilcoxonDistribution:
    ...
    def cdf(self, k):
        ...
        return xpx.apply_where(k <= mn, (k, self.n), self._cdf, lambda k, n: 1 - self._sf(k + 1, n))[()]

    def sf(self, k):
        k, mn, out = self._prep(k)
        return xpx.apply_where(k <= mn, (k, self.n), self._sf, lambda k, n: 1 - self._cdf(k - 1, n))[()]
...
def _wilcoxon_iv(x, y, zero_method, correction, alternative, method, axis):
    ...
    if not isinstance(method, stats.PermutationMethod):
        ...
        message = f"`method` must be one of {methods} or " "an instance of `stats.PermutationMethod`."
...
def _wilcoxon_nd(x, y=None, zero_method="wilcox", correction=True, alternative="two-sided", method="auto", axis=0):
