    ...
    *,
    ...
    if driver:
        if b is None and (driver in ["gv", "gvd", "gvx"]):
            raise ValueError(f"{driver} requires input b array to be supplied " "for generalized eigenvalue problems.")
        if (b is not None) and (driver in ["ev", "evd", "evr", "evx"]):
            raise ValueError(f'"{driver}" does not accept input b array ' "for standard eigenvalue problems.")
    ...
    if info == 0:
        ...
        if info < -1:
            raise LinAlgError(f"Illegal value in argument {-info} of internal " f"{drv.typecode + pfx + driver}")
        elif info > n:
            raise LinAlgError(
