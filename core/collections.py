...
class Collection(mcolorizer.ColorizingArtist):
    ...
    def get_datalim(self, transData):
        ...
        if not (isinstance(offset_trf, transforms.IdentityTransform) or offset_trf.contains_branch(transData)):
    ...
    def draw(self, renderer):
        ...
        if do_single_path_optimization:
            ...
            renderer.draw_markers(
                gc, paths[0], combined_transform.frozen(), mpath.Path(offsets), offset_trf, tuple(facecolors[0])
            )
        else:
    ...
    def contains(self, mouseevent):
        ...
        pickradius = (
            float(self._picker)
            if isinstance(self._picker, Number) and self._picker is not True  # the bool, not just nonzero or 1
    ...
    def legend_elements(self, prop="colors", num="auto", fmt=None, func=lambda x: x, **kwargs):
        ...
        **kwargs
        ...
        if prop == "colors":
            ...
            raise ValueError("Valid values for `prop` are 'colors' or " f"'sizes'. You supplied '{prop}' instead.")
...
class FillBetweenPolyCollection(PolyCollection):
    ...
    def get_datalim(self, transData):
        ...
        datalim.update_from_data_xy(
            (self.get_transform() - transData).transform(np.concatenate([self._bbox, [self._bbox.minpos]]))
        )
        return datalim
    ...
    def _make_verts(self, t, f1, f2, where):
        ...
        self._bbox.update_from_data_xy(
            self._fix_pts_xy_order(np.concatenate([np.stack((t[where], f[where]), axis=-1) for f in (f1, f2)]))
        )
...
class LineCollection(Collection):
    ...
    def set_segments(self, segments):
        ...
        self._paths = [
            mpath.Path(seg) if isinstance(seg, np.ma.MaskedArray) else mpath.Path(np.asarray(seg, float))
            for seg in segments
        ]
        self.stale = True
...
class EventCollection(LineCollection):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        super().__init__(
            [], linewidths=linewidth, linestyles=linestyle, colors=color, antialiaseds=antialiased, **kwargs
        )
...
class _MeshData:
    ...
    def _convert_mesh_to_paths(coordinates):
        ...
        if isinstance(coordinates, np.ma.MaskedArray):
            ...
            c = coordinates
        points = np.concatenate([c[:-1, :-1], c[:-1, 1:], c[1:, 1:], c[1:, :-1], c[:-1, :-1]], axis=2).reshape(
            (-1, 5, 2)
        )
