...
class UnaryExpression(ColumnElement[_T]):
    ...
    def _wraps_unnamed_column(self):
        ...
        return not isinstance(ungrouped, NamedColumn) or ungrouped._non_anon_label is None
...
class Over(ColumnElement[_T]):
    ...
    def __init__(
        ...
        if sum(bool(item) for item in (range_, rows, groups)) > 1:
            raise exc.ArgumentError("only one of 'rows', 'range_', or 'groups' may be provided")
        else:
