...
def _continued_fraction_iv(a, b, args, tolerances, maxiter, log):
    ...
    message = "`eps` and `tiny` must be (or represent the logarithm of) " "finite, positive, real scalars."
    tols = np.asarray([eps if eps is not None else 1, tiny if tiny is not None else 1])
    not_real = not np.issubdtype(tols.dtype, np.number) or np.issubdtype(tols.dtype, np.complexfloating)
...
def _continued_fraction(a, b, *, args=(), tolerances=None, maxiter=100, log=False):
    ...
    work = _RichResult(
        n=0, fn=fn, Cnm1=Cnm1, Dnm1=Dnm1, CnDn=CnDn, eps=eps, tiny=tiny, nit=nit, nfev=nfev, status=status
    )
