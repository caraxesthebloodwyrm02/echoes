...
@image_comparison(["legend_labels_first.png"], remove_text=True, tol=0 if platform.machine() == "x86_64" else 0.013)
...
@image_comparison(["legend_multiple_keys.png"], remove_text=True, tol=0 if platform.machine() == "x86_64" else 0.013)
...
@image_comparison(
    ["shadow_argument_types.png"], remove_text=True, style="mpl20", tol=0 if platform.machine() == "x86_64" else 0.028
)
...
def test_warn_big_data_best_loc(monkeypatch):
    ...
    with rc_context({"legend.loc": "best"}):
        legend = ax.legend()
    with pytest.warns(
        UserWarning, match='Creating legend with loc="best" can be slow with large ' "amounts of data."
    ) as records:
...
def test_plot_single_input_multiple_label(label_array):
    ...
    y = [2, 5, 6]
    fig, ax = plt.subplots()
    with pytest.warns(mpl.MatplotlibDeprecationWarning, match="Passing label as a length 2 sequence"):
        ax.plot(x, y, label=label_array)
    leg = ax.legend()
...
def test_loc_invalid_tuple_exception():
    ...
    fig, ax = plt.subplots()
    with pytest.raises(
        ValueError, match=("loc must be string, coordinate " "tuple, or an integer 0-10, not \\(1.1,\\)")
    ):
        ax.legend(loc=(1.1,), labels=["mock data"])

    with pytest.raises(
        ValueError,
        match=("loc must be string, coordinate " "tuple, or an integer 0-10, not \\(0.481, 0.4227, 0.4523\\)"),
    ):
        ax.legend(loc=(0.481, 0.4227, 0.4523), labels=["mock data"])

    with pytest.raises(
        ValueError, match=("loc must be string, coordinate " "tuple, or an integer 0-10, not \\(0.481, 'go blue'\\)")
    ):
...
def test_loc_invalid_list_exception():
    fig, ax = plt.subplots()
    with pytest.raises(
        ValueError, match=("loc must be string, coordinate " "tuple, or an integer 0-10, not \\[1.1, 2.2, 3.3\\]")
    ):
...
def test_loc_invalid_type():
    fig, ax = plt.subplots()
    with pytest.raises(
        ValueError, match=("loc must be string, coordinate " "tuple, or an integer 0-10, not {'not': True}")
    ):
