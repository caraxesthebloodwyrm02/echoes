...
def instances(cursor: CursorResult[Any], context: QueryContext) -> Result[Any]:
    ...
    try:
        (process, labels, extra) = list(
            zip(*[query_entity.row_processor(context, cursor) for query_entity in context.compile_state._entities])
        )
    ...
    *,
    ...
    if primary_key_identity is not None:
        ...
        if None in primary_key_identity:
            nones = {
                _get_params[col].key for col, value in zip(mapper.primary_key, primary_key_identity) if value is None
            }
    ...
    def do_load(context, path, states, load_only, effective_entity):
        ...
        while states:
            ...
            context.session.execute(
                q2,
                dict(primary_keys=[state.key[1][0] if zero_idx else state.key[1] for state, load_attrs in chunk]),
            ).unique().scalars().all()
