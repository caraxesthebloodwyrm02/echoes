...
def _check_broadcast_up_to(arr_from, shape_to, name):
    ...
    raise ValueError(f"{name} argument must be able to broadcast up " f"to shape {shape_to} but had shape {shape_from}")
...
class PPoly(_PPolyBase):
    ...
    def from_bernstein_basis(cls, bp, extrapolate=None):
        ...
        if not isinstance(bp, BPoly):
            raise TypeError(f".from_bernstein_basis only accepts BPoly instances. " f"Got {type(bp)} instead.")
...
class BPoly(_PPolyBase):
    ...
    def from_power_basis(cls, pp, extrapolate=None):
        ...
        if not isinstance(pp, PPoly):
            raise TypeError(f".from_power_basis only accepts PPoly instances. " f"Got {type(pp)} instead.")
