...
class QAPCommonTests:
    ...
    def test_accuracy_1(self):
        ...
        res = quadratic_assignment(A, B, method=self.method, options={"rng": rng, "maximize": False})
    ...
    def test_accuracy_3(self):
        ...
        res = quadratic_assignment(A, B, method=self.method, options={"partial_match": seed, "rng": rng})
        assert_(11156 <= res.fun < 21000)
...
class Test2opt(QAPCommonTests):
    ...
    def test_partial_guess(self):
        ...
        res2 = quadratic_assignment(A, B, method=self.method, options={"rng": rng, "partial_guess": guess})
        fix = [2, 4]
        match = np.array([np.arange(5)[fix], res1.col_ind[fix]]).T
        res3 = quadratic_assignment(
            A, B, method=self.method, options={"rng": rng, "partial_guess": guess, "partial_match": match}
        )
