...
def datsp_math_dtypes(dat1d):
    ...
    return {spcreator: [(dtype, dat, spcreator(dat)) for dtype, dat in dat_dtypes.items()] for spcreator in spcreators}
...
@pytest.mark.parametrize("spcreator", [csr_array, dok_array, bsr_array, csc_array, dia_array, lil_array])
