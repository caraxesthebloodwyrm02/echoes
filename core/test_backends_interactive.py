...
def _get_available_interactive_backends():
    _is_linux_and_display_invalid = sys.platform == "linux" and not _c_internal_utils.display_is_valid()
    _is_linux_and_xdisplay_invalid = sys.platform == "linux" and not _c_internal_utils.xdisplay_is_valid()
    envs = []
    for deps, env in [
...
def test_interactive_backend(env, toolbar):
    ...
    if env["MPLBACKEND"] == "wxagg" and toolbar == "toolmanager":
        pytest.skip("Temporarily deactivated: show() changes figure height " "and thus fails the test")
    try:
        proc = _run_helper(
