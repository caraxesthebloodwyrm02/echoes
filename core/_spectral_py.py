...
def lombscargle(
    ...
    *,
    ...
    if not (np.all(weights >= 0) and np.sum(weights) > 0):
        raise ValueError("Parameter weights must have only non-negative entries " "which sum to a positive value!")
    ...
    if normalize not in ["power", "normalize", "amplitude"]:
        raise ValueError("Normalize must be: False (or 'power'), True (or 'normalize'), " "or 'amplitude'.")
    ...
    * The `csd` function calculates ``np.conj(Sx[q,p]) * Sy[q,p]``, whereas
    ...
    if isinstance(window, str) or isinstance(window, tuple):
        ...
        if nperseg < 1:
            ...
            warnings.warn(
                f"{nperseg=} is greater than signal length max(len(x), " + f"len(y)) = {n}, using nperseg = {n}",
                stacklevel=3,
            )
            nperseg = n
    ...
    SFT = ShortTimeFFT(
        win, nperseg - noverlap, fs, fft_mode=fft_mode, mfft=nfft, scale_to=scales[scaling], phase_shift=None
    )
...
def _spectral_helper(
    ...
    if boundary not in boundary_funcs:
        raise ValueError(f"Unknown boundary option '{boundary}', " f"must be one of: {list(boundary_funcs.keys())}")
