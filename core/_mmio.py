...
class MMFile:
    ...
    def _validate_symmetry(self, symmetry):
        if symmetry not in self.SYMMETRY_VALUES:
            raise ValueError(f"unknown symmetry type {symmetry}, " f"must be one of {self.SYMMETRY_VALUES}")
    ...
    def _init_attrs(self, **kwargs):
        ...
        if invalid_keys:
            raise ValueError(
                f"found {tuple(invalid_keys)} invalid keyword " f"arguments, please only use {public_attrs}"
            )
