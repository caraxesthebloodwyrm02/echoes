...
def test_min_weight_fraction_leaf_with_min_samples_leaf_on_sparse_input(name, csc_container):
    check_min_weight_fraction_leaf_with_min_samples_leaf(name, "multilabel", sparse_container=csc_container)
...
def test_sample_weight_invalid():
    ...
    expected_err = re.escape(
        ("Input should have at least 1 dimension i.e. satisfy " "`len(x.shape) > 0`, got scalar `array(0.)` instead.")
    )
...
def test_missing_values_is_resilience(make_data, Tree, sample_weight_train, global_random_seed, tolerance):
    ...
    tree_with_imputer = make_pipeline(SimpleImputer(), Tree(max_depth=max_depth, random_state=global_random_seed))
    ...
    assert score_native_tree + tolerance > score_tree_with_imputer, (
        f"{score_native_tree=} + {tolerance} should be strictly greater than" f" {score_tree_with_imputer}"
    )
...
def test_missing_value_is_predictive(Tree, expected_score, global_random_seed):
    ...
    assert tree_cv_score >= expected_score, f"Expected CV score: {expected_score} but got {tree_cv_score}"
