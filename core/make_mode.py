...
class Make:
        ...
        try:
            with chdir(self.build_dir_join("latex")):
                if "-Q" in self.opts:
                    ...
                    if returncode:
                        print("Latex error: check %s" % self.build_dir_join("latex", "__LATEXSTDOUT__"))
                elif "-q" in self.opts:
                    returncode = subprocess.call(
                        [
                            makecmd,
                    ...
                    if returncode:
                        print("Latex error: check .log file in %s" % self.build_dir_join("latex"))
                else:
    ...
    def run_generic_build(self, builder: str, doctreedir: str | os.PathLike[str] | None = None) -> int:
