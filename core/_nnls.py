...
def nnls(A, b, *, maxiter=None, atol=_NoValue):
    ...
    if (b.ndim > 2) or ((b.ndim == 2) and (b.shape[1] != 1)):
        raise ValueError("Expected a 1D array,(or 2D with one column), but the," f" shape of b is {b.shape}")
    ...
    if m != b.shape[0]:
        raise ValueError(
            "Incompatible dimensions. The first dimension of " + f"A is {m}, while the shape of b is {(b.shape[0], )}"
        )
