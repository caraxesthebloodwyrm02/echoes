        ...
        if check:
            ...
            if dml.isupdate(compile_state):
                tables_mentioned = set(
                    c.table for c, v in stmt_parameter_tuples if isinstance(c, ColumnClause) and c.table is not None
                ).difference([compile_state.dml_table])

                multi_not_in_from = tables_mentioned.difference(compile_state._extra_froms)

                if tables_mentioned and (
                    not compile_state.is_multitable or not compiler.render_table_with_column_in_update_from
                ):
                    if not compiler.render_table_with_column_in_update_from:
                        preamble = "Backend does not support additional " "tables in the SET clause"
                    else:
...
def _key_getters_for_crud_column(compiler: SQLCompiler, stmt: ValuesBase, compile_state: DMLState) -> Tuple[
...
def _get_update_multitable_params(
    ...
    for t in compile_state._extra_froms:
        ...
        # supported
        we_shouldnt_be_here_if_columns_found = not include_table and not compile_state.dml_table.is_derived_from(t)
