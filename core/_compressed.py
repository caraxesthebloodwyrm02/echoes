...
class _cs_matrix(_data_matrix, _minmax_mixin, IndexMixin):
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False, *, maxprint=None):
        ...
        if issparse(arg1):
            ...
            self.indptr, self.indices, self.data, self._shape = (arg1.indptr, arg1.indices, arg1.data, arg1._shape)
            ...
            if isshape(arg1, allow_nd=self._allow_nd):
                ...
                if len(arg1) == 2:
                    ...
                    raise ValueError(f"unrecognized {self.__class__.__name__} " f"constructor input: {arg1}")
            ...
            try:
                ...
                raise ValueError(f"unrecognized {self.__class__.__name__} " f"constructor input: {arg1}") from e
    ...
    def _multiply_2d_with_broadcasting(self, other):
        ...
        if self.shape[0] == 1 or self.ndim == 1:
            ...
            return self._coo_container(
                (data.view(np.ndarray).ravel(), (row, col)), shape=(other2d.shape[0], self.shape[-1]), copy=False
            )
        ...
        if self.shape[1] == 1:
            ...
            return self._coo_container(
                (data.view(np.ndarray).ravel(), (row, col)), shape=(self.shape[0], other2d.shape[1]), copy=False
            )
    ...
    def _matmul_sparse(self, other):
        ...
        csr_matmat(M, N, s_indptr, s_indices, s.data, o_indptr, o_indices, o.data, indptr, indices, data)
    ...
    def _insert_many(self, i, j, x):
        ...
        idx_dtype = self._get_index_dtype((self.indices, self.indptr), maxval=(self.indptr[-1] + x.size))
        self.indptr = np.asarray(self.indptr, dtype=idx_dtype)
        self.indices = np.asarray(self.indices, dtype=idx_dtype)
    ...
    def resize(self, *shape):
        ...
        if hasattr(self, "blocksize"):
            ...
            if rm or rn:
                raise ValueError(f"shape must be divisible into {self.blocksize}" f" blocks. Got {shape}")
