...
class NdBSpline:
    ...
    def __call__(self, xi, *, nu=None, extrapolate=None):
        ...
        if nu is None:
            ...
            if nu.ndim != 1 or nu.shape[0] != ndim:
                raise ValueError(f"invalid number of derivative orders {nu = } for " f"ndim = {len(self.t)}.")
            if any(nu < 0):
    ...
    def design_matrix(cls, xvals, t, k, extrapolate=True):
        ...
        ndim = xvals.shape[-1]
        if len(t) != ndim:
            raise ValueError(f"Data and knots are inconsistent: len(t) = {len(t)} for " f" {ndim = }.")
...
def _preprocess_inputs(k, t_tpl):
    ...
    for d in range(ndim):
        ...
        if n < kd + 1:
            raise ValueError(f"Need at least {2*kd + 2} knots for degree" f" {kd} in dimension {d}.")
        if (np.diff(td) < 0).any():
