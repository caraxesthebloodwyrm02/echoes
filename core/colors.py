...
class MultivarColormap:
    ...
    def __call__(self, X, alpha=None, bytes=False, clip=True):
        ...
        if len(X) != len(self):
            raise ValueError(
                f"For the selected colormap the data must have a first dimension " f"{len(self)}, not {len(X)}"
            )
        ...
        *over* modifies the attributes of the copied component colormaps.
        ...
        if under is not None:
            if not np.iterable(under) or len(under) != len(new_cm):
                raise ValueError(
                    "*under* must contain a color for each scalar colormap" f" i.e. be of length {len(new_cm)}."
                )
            else:
        ...
        if over is not None:
            if not np.iterable(over) or len(over) != len(new_cm):
                raise ValueError(
                    "*over* must contain a color for each scalar colormap" f" i.e. be of length {len(new_cm)}."
                )
            else:
...
class BivarColormap:
    ...
    def __call__(self, X, alpha=None, bytes=False):
        ...
        if len(X) != 2:
            raise ValueError(f"For a `BivarColormap` the data must have a first dimension " f"2, not {len(X)}")
    ...
    def resampled(self, lutshape, transposed=False):
        ...
        if transposed:
            ...
            new_cmap = BivarColormapFromImage(new_lut, name=self.name, shape=shape_memory, origin=self.origin[::-1])
        else:
            new_lut = self((x_0, x_1))
            new_cmap = BivarColormapFromImage(new_lut, name=self.name, shape=shape_memory, origin=self.origin)
    ...
    def _clip(self, X):
        ...
        if self.shape == "square":
            ...
            for X_part in X:
                if X_part.dtype.kind != "f":
                    raise NotImplementedError(
                        "Circular bivariate colormaps are only" " implemented for use with with floats"
                    )
...
class BivarColormapFromImage(BivarColormap):
    ...
    def __init__(self, lut, shape="square", origin=(0, 0), name="from image"):
        ...
        if lut.ndim != 3 or lut.shape[2] not in (3, 4):
            raise ValueError(
                "The lut must be an array of shape (n, m, 3) or (n, m, 4)", " or a PIL.image encoded as RGB or RGBA"
            )
...
def rgb_to_hsv(arr):
    ...
    if np.any(arr_max > 1):
        raise ValueError("Input array must be in the range [0, 1]. " f"Found a maximum value of {arr_max.max()}")

    if arr.min() < 0:
        raise ValueError("Input array must be in the range [0, 1]. " f"Found a minimum value of {arr.min()}")
        ...
        **kwargs
        ...
        if blend_mode in lookup:
            ...
            try:
                ...
                raise ValueError('"blend_mode" must be callable or one of ' f"{lookup.keys}") from err
