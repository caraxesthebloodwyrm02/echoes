...
class CallbackRegistry:
    ...
    def __setstate__(self, state):
        ...
        self.callbacks = {
            s: {cid: _weak_or_strong_ref(func, functools.partial(self._remove_proxy, s)) for cid, func in d.items()}
            for s, d in self.callbacks.items()
        }
        self._func_cid_map = _UnhashDict(
...
def _g_sig_digits(value, delta):
    ...
    # decimal point.
    return max(0, (math.floor(math.log10(abs(value))) + 1 if value else 1) - math.floor(math.log10(delta)))
...
def _is_torch_array(x):
    ...
    return isinstance(tp, type) and isinstance(x, tp)  # Just in case it's a very nonstandard module.
