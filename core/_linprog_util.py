...
def _clean_inputs(lp):
    ...
    if len(bsh) > 2:
        ...
        raise ValueError("Invalid input for linprog: provide a 2-D array for bounds, " f"not a {len(bsh):d}-D array.")
        ...
        raise ValueError(
            f"Invalid input for linprog: provide a {n_x:d} x 2 array for bounds, " f"not a 2 x {n_x:d} array."
        )
    else:
        raise ValueError("Invalid input for linprog: unable to interpret bounds with this " f"dimension tuple: {bsh}.")
