...
class NpzFile(Mapping):
    ...
    def __init__(self, fid, own_fid=False, allow_pickle=False, pickle_kwargs=None, *, max_header_size=_MAX_HEADER_SIZE):
...
@set_module("numpy")
def load(
    file, mmap_mode=None, allow_pickle=False, fix_imports=True, encoding="ASCII", *, max_header_size=_MAX_HEADER_SIZE
):
    """
...
def _read(
    ...
    *,
    ...
    if dtype.kind in "SUM" and dtype in {np.dtype("S0"), np.dtype("U0"), np.dtype("M8"), np.dtype("m8")}:
...
def recfromtxt(fname, **kwargs):
    ...
    warnings.warn(
        "`recfromtxt` is deprecated, " "use `numpy.genfromtxt` instead." "(deprecated in NumPy 2.0)",
        DeprecationWarning,
        stacklevel=2,
    )
