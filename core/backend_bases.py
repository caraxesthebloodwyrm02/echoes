...
class LocationEvent(Event):
    ...
    def __init__(self, name, canvas, x, y, guiEvent=None, *, modifiers=None):
        ...
        self._set_inaxes(
            self.canvas.inaxes((x, y)) if self.canvas.mouse_grabber is None else self.canvas.mouse_grabber, (x, y)
        )
            ...
            if last_axes is not None:
                ...
                try:
                    ...
                    leave_event = LocationEvent(
                        "axes_leave_event", canvas, event.x, event.y, event.guiEvent, modifiers=event.modifiers
                    )
    ...
    **Other operations**
    ...
    def show(self):
        ...
        if sys.platform == "linux" and not os.environ.get("DISPLAY"):
            ...
            return
        raise NonGuiException(f"{type(self.canvas).__name__} is non-interactive, and thus cannot be " f"shown")
    ...
    def _get_image_filename(self, tool):
        ...
        if os.path.isabs(tool.image):
            ...
            if "image" in getattr(tool, "__dict__", {}):
                raise ValueError("If 'tool.image' is an instance variable, " "it must be an absolute path")
            for cls in type(tool).__mro__:
                if "image" in vars(cls):
                    try:
                        ...
                        raise ValueError("Failed to locate source file " "where 'tool.image' is defined") from None
            else:
