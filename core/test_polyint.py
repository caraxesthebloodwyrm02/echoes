...
from scipy._lib._array_api import xp_assert_equal, xp_assert_close, assert_array_almost_equal, assert_almost_equal
...
class TestKrogh:
    ...
    def test_ndim_derivatives(self):
        ...
        for i in range(D.shape[0]):
            xp_assert_close(
                D[i],
                np.stack(
                    (poly1.deriv(i)(self.test_xs), poly2.deriv(i)(self.test_xs), poly3.deriv(i)(self.test_xs)), axis=-1
                ),
            )
    ...
    def test_int_inputs(self):
        ...
        xp_assert_close(abs((f(x) - offset_cdf) / f.derivative(x, 1)), np.zeros_like(offset_cdf), atol=1e-10)
...
class TestBarycentric:
    ...
    def test_wrapper(self):
        ...
        xp_assert_close(P.derivative(self.test_xs, 2), bi(self.xs, self.ys, self.test_xs, der=2, rng=1))
        xp_assert_close(P.derivatives(self.test_xs, 2), bi(self.xs, self.ys, self.test_xs, der=[0, 1], rng=1))
...
class TestPCHIP:
    ...
    def test_pchip_interpolate(self):
        assert_array_almost_equal(pchip_interpolate([1, 2, 3], [4, 5, 6], [0.5], der=1), np.asarray([1.0]))

        assert_array_almost_equal(pchip_interpolate([1, 2, 3], [4, 5, 6], [0.5], der=0), np.asarray([3.5]))

        assert_array_almost_equal(
            np.asarray(pchip_interpolate([1, 2, 3], [4, 5, 6], [0.5], der=[0, 1])), np.asarray([[3.5], [1]])
        )
...
class TestCubicSpline:
    ...
    def check_correctness(S, bc_start="not-a-knot", bc_end="not-a-knot", tol=1e-14):
        ...
        xp_assert_close(c[2, 1:], 3 * c[0, :-1] * dxi**2 + 2 * c[1, :-1] * dxi + c[2, :-1], rtol=tol, atol=tol)
