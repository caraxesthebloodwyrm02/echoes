...
def cossin(X, p=None, q=None, separate=False, swap_sign=False, compute_u=True, compute_vh=True):
    ...
    if p or q:
        ...
        if not np.equal(*X.shape[-2:]):
            raise ValueError("Cosine Sine decomposition only supports square" f" matrices, got {X.shape[-2:]}")
        m = X.shape[-2]
        if p >= m or p <= 0:
        ...
        if len(X) != 4:
            raise ValueError("When p and q are None, exactly four arrays" f" should be in X, got {len(X)}")
        x11, x12, x21, x22 = (np.atleast_2d(x) for x in X)

    return _cossin(
        x11, x12, x21, x22, separate=separate, swap_sign=swap_sign, compute_u=compute_u, compute_vh=compute_vh
    )
