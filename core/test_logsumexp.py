...
from scipy._lib._array_api import is_array_api_strict, make_xp_test_case, xp_default_dtype, xp_device
...
class TestLogSumExp:
    ...
    def test_dtypes_ab(self, dtype_a, dtype_b, xp):
        ...
        if is_array_api_strict(xp):
            ...
            xp_float_dtypes = [dtype for dtype in [xp_dtype_a, xp_dtype_b] if not xp.isdtype(dtype, "integral")]
...
class TestSoftmax:
    def test_softmax_fixtures(self, xp):
        xp_assert_close(softmax(xp.asarray([1000.0, 0.0, 0.0, 0.0])), xp.asarray([1.0, 0.0, 0.0, 0.0]), rtol=1e-13)
        xp_assert_close(softmax(xp.asarray([1.0, 1.0])), xp.asarray([0.5, 0.5]), rtol=1e-13)
        xp_assert_close(softmax(xp.asarray([0.0, 1.0])), xp.asarray([1.0, np.e]) / (1 + np.e), rtol=1e-13)

        # Expected value computed using mpmath (with mpmath.mp.dps = 200) and then
        # converted to float.
        x = xp.arange(4, dtype=xp.float64)
        expected = xp.asarray(
            [0.03205860328008499, 0.08714431874203256, 0.23688281808991013, 0.6439142598879722], dtype=xp.float64
        )
    ...
    def test_softmax_int_array(self, xp):
        xp_assert_close(softmax(xp.asarray([1000, 0, 0, 0])), xp.asarray([1.0, 0.0, 0.0, 0.0]), rtol=1e-13)
...
class TestLogSoftmax:
    def test_log_softmax_basic(self, xp):
        xp_assert_close(log_softmax(xp.asarray([1000.0, 1.0])), xp.asarray([0.0, -999.0]), rtol=1e-13)
    ...
    def data_1d(xp):
        ...
        expect = [-3.4401896985611953, -2.4401896985611953, -1.4401896985611953, -0.44018969856119533]
    ...
    def test_axes(self, axis_2d, expected_2d, xp):
        ...
        xp_assert_close(log_softmax(x, axis=axis_2d), xp.asarray(expected_2d, dtype=x.dtype), rtol=1e-13)
