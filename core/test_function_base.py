...
class TestAverage:
    ...
    def test_weight_and_input_dims_different(self):
        ...
        with pytest.raises(
            ValueError, match="Shape of weights must be consistent with " "shape of a along specified axis"
        ):
    ...
    def test_basic(self, cumprod):
        ...
        for ctype in [np.int16, np.uint16, np.int32, np.uint32, np.float32, np.float64, np.complex64, np.complex128]:
            ...
            if ctype in ["1", "b"]:
                ...
                assert_array_equal(cumprod(a, axis=-1), np.array([1, 2, 20, 220, 1320, 6600, 26400], ctype))
                assert_array_equal(
                    cumprod(a2, axis=0), np.array([[1, 2, 3, 4], [5, 12, 21, 36], [50, 36, 84, 180]], ctype)
                )
                assert_array_equal(
                    cumprod(a2, axis=-1), np.array([[1, 2, 6, 24], [5, 30, 210, 1890], [10, 30, 120, 600]], ctype)
                )
...
class TestVectorize:
    ...
    def test_datetime_conversion(self):
        ...
        assert_array_equal(np.vectorize(lambda x: x, signature="(i)->(j)", otypes=[otype])(arr), arr)
...
class TestLeaks:
    ...
    @pytest.mark.skipif(
        NOGIL_BUILD, reason=("Functions are immortalized if a thread is " "launched, making this test flaky")
    )
    @pytest.mark.parametrize(
        "name, incr",
        [
...
class TestSinc:
    ...
    @pytest.mark.parametrize("dtype", [np.float16, np.float32, np.longdouble, np.complex64, np.complex128])
...
class TestPercentile:
    ...
    def test_linear_interpolation(self, function, quantile, method, weighted, expected, input_dtype, expected_dtype):
...
class TestQuantile:
    ...
    def test_quantile_identification_equation(self, weights, method, alpha):
        ...
        if method in ("higher",):
            ...
            # "sample precision".
            assert_allclose(np.average(self.V(x, y, alpha), weights=w), 0, atol=1 / n / np.amin([alpha, 1 - alpha]))
