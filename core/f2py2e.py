...
def run_main(comline_list):
    ...
    if args.module_name:
        if "-h" in comline_list:
            ...
            modname = validate_modulename(pyf_files, args.module_name)  # Validate modname when -h is not present
    ...
    for plist in postlist:
        ...
        if plist["block"] == "python module" and "__user__" in module_name:
            if module_name in isusedby:
                ...
                outmess(f'Skipping Makefile build for module "{module_name}" ' f"which is used by {usedby}\n")
    if "signsfile" in options:
