...
def visit_figmpl_html(self, node):
    ...
    # </figcaption>
    # </figure>
    self.body.append(self.starttag(node, "figure", CLASS=f'align-{node["align"]}' if node["align"] else "align-center"))
    self.body.append(
...
def figurempl_addnode(app):
    app.add_node(
        figmplnode, html=(visit_figmpl_html, depart_figmpl_html), latex=(visit_figmpl_latex, depart_figmpl_latex)
    )
