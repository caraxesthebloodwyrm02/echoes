...
from ._stats_mstats_common import _find_repeats, theilslopes as stats_theilslopes, siegelslopes as stats_siegelslopes
...
def trimmed_stde(a, limits=(0.1, 0.1), inclusive=(1, 1), axis=None):
    ...
    if axis is None:
        ...
        if a.ndim > 2:
            raise ValueError(f"Array 'a' must be at most two dimensional, " f"but got a.ndim = {a.ndim}")
...
def stde_median(data, axis=None):
    ...
    if axis is None:
        ...
        if data.ndim > 2:
            raise ValueError(f"Array 'data' must be at most two dimensional, " f"but got data.ndim = {data.ndim}")
...
def skewtest(a, axis=0, alternative="two-sided"):
    ...
    n = a.count(axis)
    if np.min(n) < 8:
        raise ValueError(f"skewtest is not valid with less than 8 samples; " f"{np.min(n)} samples were given.")
...
def kurtosistest(a, axis=0, alternative="two-sided"):
    ...
    n = a.count(axis=axis)
    if np.min(n) < 5:
        raise ValueError(f"kurtosistest requires at least 5 observations; " f"{np.min(n)} observations were given.")
    if np.min(n) < 20:
        warnings.warn(f"kurtosistest only valid for n>=20 ... continuing " f"anyway, n={np.min(n)}", stacklevel=2)
