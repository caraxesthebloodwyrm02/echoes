    ...
    *p*-value produced using the normal approximation.
    ...
    if method == "exact":
        ...
        def statistic(x, y, axis):
            return mannwhitneyu(
                x, y, use_continuity=use_continuity, alternative=alternative, axis=axis, method="asymptotic"
            ).statistic

        res = stats.permutation_test((x, y), statistic, axis=axis, **method._asdict(), alternative=alternative)
        p = res.pvalue
        f = 1
