...
axis_nan_policy_cases = [
    ...
    (stats.cramervonmises_2samp, tuple(), dict(), 2, 2, False, lambda res: (res.statistic, res.pvalue)),
    ...
    (stats.pointbiserialr, tuple(), dict(), 2, 3, True, lambda res: (res.statistic, res.pvalue, res.correlation)),
    (stats.kendalltau, tuple(), dict(), 2, 3, True, lambda res: (res.statistic, res.pvalue, res.correlation)),
    (stats.weightedtau, tuple(), dict(), 2, 3, True, lambda res: (res.statistic, res.pvalue, res.correlation)),
    (weightedtau_weighted, tuple(), dict(), 3, 3, True, lambda res: (res.statistic, res.pvalue, res.correlation)),
    (stats.linregress, tuple(), dict(), 2, 6, True, lambda res: tuple(res) + (res.intercept_stderr,)),
...
def _axis_nan_policy_test(
    ...
    for i, _ in np.ndenumerate(np.zeros(output_shape)):
        ...
        if nan_policy == "omit" and data_generator == "all_nans" and hypotest not in too_small_special_case_funcs:
        ...
        elif nan_policy == "omit" and data_generator == "mixed" and hypotest not in too_small_special_case_funcs:
...
def test_keepdims(hypotest, args, kwds, n_samples, n_outputs, paired, unpacker, sample_shape, axis_cases, nan_policy):
    small_sample_raises = {stats.skewtest, stats.kurtosistest, stats.normaltest, stats.differential_entropy}
...
@pytest.mark.parametrize(
    ("hypotest", "args", "kwds", "n_samples", "n_outputs", "paired", "unpacker"), paired_non_broadcastable_cases()
)
