...
def test_b_orthonormalize(n, m, Vdtype, Bdtype, BVdtype):
    ...
    is_all_complex = np.issubdtype(Vdtype, np.complexfloating) and np.issubdtype(BVdtype, np.complexfloating)
    is_all_notcomplex = (
...
def test_hermitian():
    ...
    for s, k, gen, dh, dx, db in itertools.product(sizes, ks, gens, gens, gens, gens):
        ...
        for wx, vx in zip(w, v.T):
            # Check eigenvector
            assert_allclose(
                np.linalg.norm(H.dot(vx) - B.dot(vx) * wx) / np.linalg.norm(H.dot(vx)), 0, atol=5e-2, rtol=0
            )
...
def test_MikotaPair(n):
    ...
    a_l = LinearOperator((n, n), matvec=a, matmat=a, dtype="float64")
    ea, _ = eigsh(B, k=m, M=A, Minv=a_l, which="LA", tol=1e-4, maxiter=50, v0=rng.normal(size=(n, 1)))
    accuracy = max(abs(ee - np.sort(1.0 / ea)) / ee)
    assert_allclose(accuracy, 0.0, atol=tol)
