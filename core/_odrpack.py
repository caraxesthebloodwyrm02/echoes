...
class Data:
    ...
    def __init__(self, x, y=None, we=None, wd=None, fix=None, meta=None):
        ...
        if not isinstance(self.x, np.ndarray):
            raise ValueError(
                "Expected an 'ndarray' of data for 'x', " f"but instead got data of type '{type(self.x).__name__}'"
            )
...
class ODR:
    ...
    def _check(self):
        ...
        if self.data.x.size == 0:
            warn(
                "Empty data detected for ODR instance. " "Do not expect any fitting to occur", OdrWarning, stacklevel=3
            )
    ...
    def _gen_work(self):
        ...
        if isinstance(self.work, np.ndarray) and self.work.shape == (lwork,) and self.work.dtype.str.endswith("f8"):
