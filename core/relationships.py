...
def remote(expr: _CEA) -> _CEA:
    ...
    """
    return _annotate_columns(coercions.expect(roles.ColumnArgumentRole, expr), {"remote": True})  # type: ignore
...
def foreign(expr: _CEA) -> _CEA:
    ...
    return _annotate_columns(coercions.expect(roles.ColumnArgumentRole, expr), {"foreign": True})  # type: ignore
...
class RelationshipProperty(_IntrospectsAnnotations, StrategizedProperty[_T], log.Identified):
    ...
    def _post_init(self) -> None:
        ...
        if not self.viewonly:
            self._dependency_processor = (dependency.DependencyProcessor.from_relationship)(self)  # type: ignore
