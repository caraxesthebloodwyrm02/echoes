...
class PolarTransform(mtransforms.Transform):
    ...
    def __init__(self, axis=None, use_rmin=True, *, apply_theta_transforms=True, scale_transform=None):
        """
        Parameters
        ----------
...
class ThetaTick(maxis.XTick):
    ...
    def __init__(self, axes, *args, **kwargs):
        self._text1_translate = mtransforms.ScaledTranslation(0, 0, axes.get_figure(root=False).dpi_scale_trans)
        self._text2_translate = mtransforms.ScaledTranslation(0, 0, axes.get_figure(root=False).dpi_scale_trans)
...
class PolarAxes(Axes):
    ...
    def _set_lim_and_transforms(self):
        ...
        self.transProjection = self.PolarTransform(self, apply_theta_transforms=False, scale_transform=self.transScale)
    ...
    def draw(self, renderer):
        ...
        rmin, rmax = (
            rscale_tr.transform(self._realViewLim.intervaly) - rscale_tr.transform(self.get_rorigin())
        ) * self.get_rsign()
