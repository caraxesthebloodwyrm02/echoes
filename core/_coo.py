...
class _coo_base(_data_matrix, _minmax_mixin):
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False, *, maxprint=None):
        ...
        if isinstance(arg1, tuple):
            if isshape(arg1, allow_nd=self._allow_nd):
                ...
                idx_dtype = self._get_index_dtype(coords, maxval=max(self.shape), check_contents=True)
    ...
    def _getnnz(self, axis=None):
        ...
        return np.bincount(downcast_intp_index(self.coords[1 - axis]), minlength=self.shape[1 - axis])
    ...
    def _check(self):
        ...
        if self.ndim != len(self.coords):
            raise ValueError(
                "mismatching number of index arrays for shape; " f"got {len(self.coords)}, expected {self.ndim}"
            )
        ...
        if self.nnz > 0:
            for i, idx in enumerate(self.coords):
                if idx.max() >= self.shape[i]:
                    raise ValueError(f"axis {i} index {idx.max()} exceeds " f"matrix dimension {self.shape[i]}")
                if idx.min() < 0:
    ...
    def toarray(self, order=None, out=None):
        ...
        # original shape.
        if self.ndim == 1:
            coo_todense_nd(np.array([1]), self.nnz, self.ndim, self.coords[0], self.data, B.ravel("A"), fortran)
        elif self.ndim == 2:
            M, N = self.shape
    ...
    def _setdiag(self, values, k):
        ...
        self.coords = (np.concatenate((self.row[keep], new_row)), np.concatenate((self.col[keep], new_col)))
    ...
    def _add_dense(self, other):
        ...
        if self.ndim == 1:
            coo_todense_nd(np.array([1]), self.nnz, self.ndim, self.coords[0], self.data, result.ravel("A"), fortran)
        elif self.ndim == 2:
            ...
            if fortran:
                ...
                strides = np.append(np.cumprod(self.shape[1:][::-1])[::-1], 1)
            coords = np.concatenate(self.coords)
            coo_todense_nd(strides, self.nnz, self.ndim, coords, self.data, result.ravel("A"), fortran)
    ...
    def _matmul_vector(self, other):
        if self.ndim > 2:
            result = np.zeros(math.prod(self.shape[:-1]), dtype=upcast_char(self.dtype.char, other.dtype.char))
    ...
    def dot(self, other):
        ...
        if self.shape[-1] != other.shape[-2:][0]:
            raise ValueError(f"shapes {self.shape} and {other.shape}" " are not aligned for n-D dot")
...
def _process_axes(ndim_a, ndim_b, axes):
    if isinstance(axes, int):
        ...
        if any(ax >= ndim_a or ax < -ndim_a for ax in axes_a) or any(bx >= ndim_b or bx < -ndim_b for bx in axes_b):
