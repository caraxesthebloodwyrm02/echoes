...
from scipy._lib._array_api import array_namespace, is_torch, is_numpy, xp_copy, xp_size, xp_default_dtype
...
def correlate(in1, in2, mode="full", method="auto"):
    ...
    if method in ("fft", "auto"):
        ...
        swapped_inputs = (
            (mode == "full") and (xp_size(in2) > xp_size(in1)) or _inputs_swap_needed(mode, in1.shape, in2.shape)
        )
...
def _conv_ops(x_shape, h_shape, mode):
    ...
    if mode == "full":
        ...
        raise ValueError("Acceptable mode flags are 'valid'," f" 'same', or 'full', not mode={mode}")
            ...
            if zi.shape != expected_shape:
                ...
                for k in range(zi.ndim):
                    if k == axis and zi.shape[k] == expected_shape[k]:
                        ...
                        raise ValueError("Unexpected shape for zi: expected " f"{expected_shape}, found {zi.shape}.")
...
def envelope(
    ...
    * The frequency spacing ``1 / (n*T)`` for corner frequencies of the bandpass filter
    ...
    bp = slice(bp_in[0] if bp_in[0] is not None else -(n // 2), bp_in[1] if bp_in[1] is not None else (n + 1) // 2)
    if not (-n // 2 <= bp.start < bp.stop <= (n + 1) // 2):
        raise ValueError(
            "`-n//2 <= bp_in[0] < bp_in[1] <= (n+1)//2` does not hold " + f"for n={z.shape[axis]=} and {bp_in=}!"
        )
...
def resample(x, num, t=None, axis=0, window=None, domain="time"):
    ...
    if window is None:  # Determine spectral windowing function:
        ...
        if window.shape != (n_x,):
            raise ValueError(
                f"{window.shape=} != ({n_x},), i.e., window length " + "is not equal to number of frequency bins!"
            )
...
def _validate_pad(padtype, padlen, x, axis, ntaps):
    ...
    if padtype not in ["even", "odd", "constant", None]:
        raise ValueError(
            f"Unknown value '{padtype}' given to padtype. " "padtype must be 'even', 'odd', 'constant', or None."
        )
    ...
    if x.shape[axis] <= edge:
        raise ValueError(f"The length of the input vector x must be greater than padlen, " f"which is {edge}.")
...
def decimate(x, q, n=None, ftype="iir", axis=-1, zero_phase=True):
    ...
    if ftype == "fir":
        ...
        if system.poles.shape[0] == 0:
            ...
            ftype = "fir"
        elif any(np.iscomplex(system.poles)) or any(np.iscomplex(system.poles)) or np.iscomplex(system.gain):
