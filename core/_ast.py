            ...
            for i in range(len(names)):
                ...
                if txt_nne:
                    if nne.templateArgs and i_template_params < len(template_params):
                        ...
                        i_template_params += 1
                    nne.describe_signature(dest, "markType", env, template_params_prefix + prefix, symbol)
                prefix += txt_nne
            if mode == "lastIsName":
                if len(self.names) > 1:
                    dest += addnodes.desc_sig_punctuation("::", "::")
...
class ASTDeclaration(ASTBase):
    ...
    def clone(self) -> ASTDeclaration:
        ...
        trailing_requires_clasue_clone = self.trailingRequiresClause.clone() if self.trailingRequiresClause else None
    ...
    def describe_signature(
        ...
        if self.trailingRequiresClause:
            ...
            self.trailingRequiresClause.describe_signature(trailing_req_node, "markType", env, self.symbol)
        if self.semicolon:
