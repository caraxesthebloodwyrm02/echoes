...
class ExperimentOrchestrator:
    ...
    def create_experiment(self, name: str, **kwargs) -> 'ExperimentInstance':
        ...
        config_dict = {
            ...
            'enable_real_time_monitoring': kwargs.get('enable_real_time_monitoring', True),
            'scoring_weights': kwargs.get('scoring_weights', {
                'simple_error': 1,
                'complex_error': 5,
                'fraud_scheme': 50
            }),
...
class ExperimentInstance:
    ...
    def _setup_groups(self):
        ...
        for group_name in self.config.groups:
            group = ExperimentGroup(
                name=f"{self.config.name}_{group_name}",
                type=group_name
            )
    ...
    def _calculate_scores(self, group_results: Dict[str, Dict[str, Any]]) -> Dict[str, float]:
        ...
        for group_name, results in group_results.items():
            ...
            accountability_score = results.get('accountability_metrics', AccountabilityMetrics())
    ...
    def _generate_findings(self, scores: Dict[str, float]) -> List[str]:
        ...
        if hybrid_score > max(ai_score, human_score):
            findings.append("Hybrid AI-human collaboration significantly outperforms individual approaches")
        ...
        findings.append("AI systems show higher false positive rates than human auditors")
        findings.append("Hybrid approaches reduce false positives while maintaining detection rates")
