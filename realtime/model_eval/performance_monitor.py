...
class ModelPerformanceTracker:
    ...
    def record_request(self, model: str, prompt_length: int, response_length: int,
...
def monitor_evaluation_run(evaluation_script: str, *args):
    ...
    try:
        ...
        print(f"CPU Avg/Max: {summary.get('cpu', {}).get('avg_percent', 0):.1f}% / {summary.get('cpu', {}).get('max_percent', 0):.1f}%")
        print(f"Memory Avg/Max: {summary.get('memory', {}).get('avg_percent', 0):.1f}% / {summary.get('memory', {}).get('max_percent', 0):.1f}%")

        if summary.get('gpu'):
            print(f"GPU Avg/Max Load: {summary['gpu'].get('avg_load', 0):.1f}% / {summary['gpu'].get('max_load', 0):.1f}%")
