...
from app.tools.business_analysis import BusinessAnalyzer, MarketOpportunity, RevenueStream
...
Examples:
  ...
  python business_cli.py project --streams '{"name":"Consulting","base_revenue":5000,"growth_rate":0.1}'
        ...
        opp_parser = subparsers.add_parser('opportunities', help='Find market opportunities')
        opp_parser.add_argument('--industry', type=str, default='AI/ML', help='Target industry')
        opp_parser.add_argument('--skills', type=str, required=True, 
                              help='Comma-separated list of skills')
        opp_parser.add_argument('--investment', type=str, default='1000-5000',
                              help='Investment range (e.g., 1000-5000)')
        opp_parser.add_argument('--months', type=int, default=6,
                              help='Time horizon in months (default: 6)')
        
        # Revenue streams command
        rev_parser = subparsers.add_parser('revenue', help='Identify revenue streams')
        rev_parser.add_argument('--skills', type=str, required=True,
                              help='Comma-separated list of skills')
        rev_parser.add_argument('--hours', type=int, default=20,
                              help='Weekly hours available (default: 20)')
        rev_parser.add_argument('--risk', type=str, choices=['low', 'medium', 'high'],
                              default='medium', help='Risk tolerance (default: medium)')
        
        # Projection command
        proj_parser = subparsers.add_parser('project', help='Project revenue')
        proj_parser.add_argument('--streams', type=str, required=True,
                               help='JSON array of stream configurations')
        proj_parser.add_argument('--months', type=int, default=12,
                               help='Number of months to project (default: 12)')
        proj_parser.add_argument('--growth', type=float, default=0.05,
                               help='Monthly growth rate (default: 0.05)')
        
        # Quick wins command
        quick_parser = subparsers.add_parser('quick-wins', help='Find quick wins')
        quick_parser.add_argument('--skills', type=str, required=True,
                                help='Comma-separated list of skills')
        quick_parser.add_argument('--hours', type=int, default=10,
                                help='Weekly hours available (default: 10)')
        quick_parser.add_argument('--weeks', type=int, default=2,
                                help='Maximum preparation time in weeks (default: 2)')
        
        # Metrics command
        metrics_parser = subparsers.add_parser('metrics', help='Calculate business metrics')
        metrics_parser.add_argument('--revenue', type=float, required=True,
                                  help='Monthly recurring revenue (MRR)')
        metrics_parser.add_argument('--expenses', type=float, required=True,
                                  help='Monthly operating expenses')
        metrics_parser.add_argument('--churn', type=float, default=0.05,
                                  help='Monthly churn rate (default: 0.05)')
        metrics_parser.add_argument('--cac', type=float, default=100.0,
                                  help='Customer acquisition cost (default: 100.0)')
        metrics_parser.add_argument('--acv', type=float, default=500.0,
    ...
    def handle_opportunities(self, args):
        ...
        self.print_markdown("🔍 Market Opportunity Analysis", {
        ...
        self.print_markdown("📊 Opportunities", result["opportunities"] or "No matching opportunities found.")
    ...
    def handle_revenue(self, args):
        ...
        self.print_markdown("💰 Revenue Stream Analysis", {
            "Skills": skills,
        ...
        self.print_markdown("📈 Potential Revenue Streams", result["revenue_streams"] or "No matching revenue streams found.")
    ...
    def handle_projections(self, args):
        ...
        self.print_markdown("📊 Revenue Projections", {
    ...
    def handle_quick_wins(self, args):
        ...
        self.print_markdown("⚡ Quick Wins Analysis", {
            "Skills": skills,
            "Weekly Hours": args.hours,
        ...
        for i, win in enumerate(wins, 1):
            self.print_markdown(f"🏆 {i}. {win['name']}", {
    ...
    def handle_metrics(self, args):
        ...
        self.print_markdown("📊 Business Metrics", {
        ...
        formatted_metrics = {
            ...
            "📈 Health Indicators": {
                "Healthy LTV:CAC Ratio (≥3:1)": "✅ Yes" if metrics['key_metrics']['healthy_ltv_cac_ratio'] else "❌ No",
                "Reasonable CAC Payback (≤12 months)": "✅ Yes" if metrics['key_metrics']['healthy_months_to_recover_cac'] else "❌ No",
                "Sustainable Churn Rate (≤10%)": "✅ Yes" if metrics['key_metrics']['sustainable_churn_rate'] else "❌ No",
                "Profitable": "✅ Yes" if metrics['key_metrics']['profitable'] else "❌ No"
            }
        }
