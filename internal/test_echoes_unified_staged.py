...
def test_stage_5_business_analysis(assistant):
    ...
    for query in queries:
        try:
            ...
            if len(response) > 100 and any(keyword in response.lower() 
    ...
    for query in queries:
        try:
            ...
            if len(response) > 150:
                ...
                print(f"❌ Complex query: {query[:40]}... -> Too short: {len(response)} chars")
        except Exception as e:
...
def test_stage_7_unanswered_questions(assistant):
    ...
    for query in queries:
        try:
            ...
            if has_response:
                print(f"✅ Unanswered: {query[:40]}... -> Response: {len(response)} chars, RAG: {len(rag_results)}")
                success_count += 1
            else:
...
def test_stage_8_performance(assistant):
    ...
    try:
        ...
        print(f"✅ Knowledge retrieval: {retrieval_time:.3f}s for {len(results)} results")
