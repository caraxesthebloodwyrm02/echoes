...
def update_imports_in_file(filepath: Path, rename_map: Dict[str, str]) -> bool:
    ...
    for old_name, new_name in rename_map.items():
        ...
        if num_subs > 0:
            updated = new_content
            changes_made = True
            print(f"  - Replaced 'from {old_name} import' with 'from {new_name} import'")
...
def main():
    ...
    for source_dir in SOURCE_DIRS:
        ...
        for py_file in source_path.rglob("*.py"):
            ...
            if (any(skip in py_file.parts for skip in SKIP_DIRS) or 
                py_file.name in SKIP_FILES or
                py_file.name.startswith("agent_") or
                py_file.suffix != '.py'):
                continue
