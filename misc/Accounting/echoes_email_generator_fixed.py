...
def load_email_config(yaml_path: str) -> EmailConfig:
    ...
    try:
        ...
        config.decision_deadline = data.get(
            "decision_deadline", config.decision_deadline
        )
        config.contract_duration = data.get(
...
def interactive_setup() -> EmailConfig:
    ...
    recipient_input = input(
        "Email recipients (comma-separated) [CFO@bank.com, CTO@bank.com, Chief.Compliance@bank.com]: "
    ).strip()
    if recipient_input:
        config.email_to = [email.strip() for email in recipient_input.split(",")]

    config.email_from = (
        input("Email From [Sarah Chen, Echoes AI <sarah.chen@echoes.ai>]: ").strip()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Echoes AI - Email ROI Stakeholder Generator (Production)"
    )
