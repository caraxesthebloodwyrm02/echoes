...
def generate_person_name():
    first_names = ['John', 'Jane', 'Mike', 'Sarah', 'David', 'Lisa', 'Robert', 'Emily', 'James', 'Maria']
...
class InnovateIncGenerator:
    ...
    def __init__(self, seed: int = 42):
        ...
        self.departments = [
            ...
            'Operations', 'Legal', 'IT', 'Research', 'Customer Service'
        ]

        self.vendors = [
            'Office Depot', 'Microsoft', 'AWS', 'Google Cloud', 'Adobe',
            'Deloitte', 'PwC', 'Accenture', 'IBM', 'Oracle',
            'Local Catering Co', 'Travel Agency', 'Printer Supplies Inc'
        ]

        # Transaction categories and types
        self.expense_categories = [
            'Office Supplies', 'Software Licenses', 'Cloud Services',
            'Professional Services', 'Travel', 'Marketing', 'Equipment',
            'Rent', 'Utilities', 'Insurance', 'Payroll', 'Taxes'
        ]

        self.income_categories = [
            'Product Sales', 'Service Revenue', 'Consulting', 'Licensing',
            'Subscriptions', 'Grants', 'Investments'
        ]
    ...
    def generate_company_data(
        ...
        """
        print(f"🏭 Generating Innovate Inc. dataset ({years} years, {complexity_level} complexity)...")
    ...
    def _generate_transfer_transaction(self, index: int, date: datetime) -> Transaction:
        ...
        to_account = random.choice(
            self.accounts['checking'] + self.accounts['savings']
        )
    ...
    def _plant_accounting_errors(self, dataset: Dataset, complexity: str):
        ...
        if complexity == 'simple':
            ...
            error_types = ['duplicate', 'rounding', 'timing', 'classification', 'amount']
        ...
        for i in range(num_errors):
            ...
            if error_type == 'duplicate':
                ...
                wrong_category = random.choice([
                    cat for cat in self.expense_categories + self.income_categories
                ...
                if random.random() < 0.5:
                    ...
                    wrong_amount = original.amount * (10 if random.random() < 0.5 else 0.1)
    ...
    def _plant_fraud_scheme(self, dataset: Dataset):
        ...
        dataset.planted_errors.append({
            ...
            'involved_transactions': [f"FRD-{i:03d}" for i in range(len(fraud_amounts))],
