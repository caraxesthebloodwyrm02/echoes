...
def load_email_config(yaml_path: str) -> EmailConfig:
    ...
    config.monthly_investment = data.get(
        "monthly_investment", config.monthly_investment
    )
    config.net_monthly_benefit = data.get(
    ...
    # Financial breakdown
    config.labor_efficiency_savings = data.get(
        "labor_efficiency_savings", config.labor_efficiency_savings
    )
    config.error_reduction_savings = data.get(
        "error_reduction_savings", config.error_reduction_savings
    )
    config.audit_preparation_savings = data.get(
        "audit_preparation_savings", config.audit_preparation_savings
    )
...
def generate_executive_email(config: EmailConfig) -> str:
    ...
    total_monthly_savings = (
        config.labor_efficiency_savings
        + config.error_reduction_savings
        + config.audit_preparation_savings
    )
...
def export_email_files(
    config: EmailConfig, base_filename: str = "stakeholder_email"
) -> None:
    ...
    with open(f"{base_filename}_metrics.csv", "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow(
            ["Annual Net Benefit", f"{config.net_monthly_benefit * 12:.2f}"]
        )
...
def convert_to_html(email_text: str) -> str:
    ...
    for line in lines:
        ...
        if not line:
            ...
            html_lines.append(
                f'<h2 style="color: #2c5aa0; margin-bottom: 10px;">{line}</h2>'
            )
        elif line.startswith("•"):
            html_lines.append(
                f'<li style="margin-bottom: 5px;">{line[1:].strip()}</li>'
            )
            ...
            if len(cells) >= 2:
                html_lines.append(
                    f'<tr><td style="padding: 8px; border: 1px solid #ccc;">{cells[0]}</td><td style="padding: 8px; border: 1px solid #ccc; text-align: right;">{cells[1]}</td></tr>'
                )
        elif line.startswith("✓"):
            html_lines.append(
                f'<p style="color: #28a745; margin-bottom: 8px;">{line}</p>'
            )
        else:
...
def interactive_email_setup() -> EmailConfig:
    ...
    config.institution_name = (
        input("Institution Name [Pilot A Bank]: ") or "Pilot A Bank"
    )
    config.email_to = (
        input("Email To (recipients) [CFO, CTO, Chief Compliance Officer]: ")
        or "CFO, CTO, Chief Compliance Officer"
    )
    config.email_from = (
        input("Email From [Sarah Chen, Echoes AI <sarah.chen@echoes.ai>]: ")
        or "Sarah Chen, Echoes AI <sarah.chen@echoes.ai>"
    )

    # ROI metrics
    config.team_size = int(input("Team Size (FTEs) [5]: ") or "5")
    config.monthly_savings = float(input("Monthly Savings ($) [71822]: ") or "71822")
    config.monthly_investment = float(
        input("Monthly Investment ($) [12000]: ") or "12000"
    )
    ...
    config.roi_percentage = (
        config.net_monthly_benefit / config.monthly_investment
    ) * 100

    # Financial breakdown
    config.labor_efficiency_savings = float(
        input("Labor Efficiency Savings ($) [14722]: ") or "14722"
    )
    config.error_reduction_savings = float(
        input("Error Reduction Savings ($) [56250]: ") or "56250"
    )
    config.audit_preparation_savings = float(
        input("Audit Preparation Savings ($) [850]: ") or "850"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Echoes AI - Email ROI Stakeholder Generator"
    )
    parser.add_argument("--config", help="YAML configuration file")
    parser.add_argument(
        "--interactive", action="store_true", help="Interactive email setup"
    )
    parser.add_argument(
        "--output", default="stakeholder_email", help="Output filename base"
    )
