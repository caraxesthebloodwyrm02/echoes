...
from .unified_comprehension import UnifiedComprehensionSystem, UnifiedInput, InputModality, ProcessingContext
...
class UnifiedComprehensionIntegration:
    ...
    async def process_multimodal_input(self,
        ...
        for modality_name, input_obj in inputs_to_process:
            try:
                ...
                logger.info(f"Successfully processed {modality_name} with {result.fidelity_level.value} fidelity")
        ...
        if len(results) > 1:
            try:
                multimodal_result = await self._create_multimodal_fusion(results, context, user_preferences)
    ...
    async def _create_multimodal_fusion(self, individual_results: Dict[str, Any],
    ...
    def _update_stats(self, result):
        ...
        if result.cross_modal_alignments:
            avg_alignment = np.mean([a.get('strength', 0) for a in result.cross_modal_alignments])
            current_avg = self.integration_stats['alignment_strength']
            self.integration_stats['alignment_strength'] = (
                current_avg * (self.integration_stats['total_processed'] - 1) + avg_alignment
            ) / self.integration_stats['total_processed']

    def get_semantic_paths(self, concept1: str, concept2: str, max_hops: int = 3) -> List[List[str]]:
        """
...
class EnhancedEchoesAssistant:
    ...
    async def enhanced_chat(self,
        ...
        comprehension_results = await self.unified_comprehension.process_multimodal_input(
            text=message,
        ...
        enhanced_response = {
            ...
            'multimodal_insights': self._extract_multimodal_insights(comprehension_results),
    ...
    def _extract_multimodal_insights(self, comprehension_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for modality, result in results.items():
            if isinstance(result, dict) and 'error' not in result:
                insights['fidelity_levels'][modality] = result.get('fidelity_level', 'unknown')
                insights['confidence_scores'][modality] = result.get('confidence', 0.0)
                insights['semantic_alignments'] += result.get('cross_modal_alignments', 0)
    ...
    async def analyze_semantic_relationships(self, concepts: List[str]) -> Dict[str, Any]:
        """
        ...
        for i, concept1 in enumerate(concepts):
            for concept2 in concepts[i+1:]:
                paths = self.unified_comprehension.get_semantic_paths(concept1, concept2)
                if paths:
...
async def demo_unified_comprehension():
    ...
    text_result = await integration.comprehension_system.process_unified_input(text_input)
    ...
    sensor_result = await integration.comprehension_system.process_unified_input(sensor_input)
    ...
    multimodal_result = await integration.comprehension_system.process_unified_input(multimodal_input)
    print(f"Multimodal fusion completed with {multimodal_result.fidelity_level.value} fidelity")
    ...
    print(f"- Registered sensors: {len(status['sensor_fusion_engine']['active_sensors'])}")
