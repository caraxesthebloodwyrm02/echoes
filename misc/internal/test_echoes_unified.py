...
class EchoesUnifiedTester:
    ...
    def __init__(self):
        ...
        self.assistant = EchoesAssistantV2(
            enable_rag=True,
            rag_preset="openai-balanced"
        )
        self.test_results = []
        self.start_time = time.time()
        
    def log_test(self, category: str, query: str, result: Any, success: bool, notes: str = ""):
        """Log test result."""
...
"""
        ...
        try:
            ...
            print(f"✅ Knowledge base loaded: {result['total_chunks']} chunks from {result['total_documents']} documents")
            return True
    ...
    def test_rag_knowledge_retrieval(self):
        ...
        for query in queries:
            try:
                ...
                notes = f"Found {len(results)} results, top score: {results[0].get('score', 0):.3f}" if results else "No results"
    ...
    def test_tool_calling(self):
        ...
        for query, expected_tool in tool_queries:
            try:
                ...
                success = expected_tool.lower() in response.lower() or "error" not in response.lower()
                notes = f"Tool used: {expected_tool}" if success else "Tool may not have been used"
    ...
    def test_business_analysis(self):
        ...
        for query in business_queries:
            try:
                response = self.assistant.process_message(query)
                success = len(response) > 50 and any(keyword in response.lower() for keyword in 
                    ["revenue", "market", "metrics", "income", "opportunity"])
    ...
    def test_agent_workflows(self):
        ...
        for query in workflow_queries:
            try:
                response = self.assistant.process_message(query)
                success = len(response) > 100 and any(keyword in response.lower() for keyword in 
                    ["workflow", "steps", "plan", "analysis", "recommend"])
    ...
    def test_complex_multimodal_queries(self):
        ...
        for query in complex_queries:
            try:
                response = self.assistant.process_message(query)
                success = len(response) > 150 and any(keyword in response.lower() for keyword in 
                    ["architecture", "revenue", "market", "workflow", "technical", "business"])
    ...
    def test_unanswered_questions(self):
        ...
        for query in unanswered_queries:
            try:
                ...
                notes = f"RAG: {len(rag_results)} results, Response: {len(response)} chars"
    ...
    def test_performance_metrics(self):
        ...
        performance_tests = [
            ("Knowledge retrieval speed", lambda: self.assistant._retrieve_context("Echoes architecture", top_k=5)),
            ("Simple query response", lambda: self.assistant.process_message("What is Echoes?")),
            ("Complex query response", lambda: self.assistant.process_message("Analyze business opportunities for AI consulting")),
            ("Batch knowledge addition", lambda: self.assistant.add_knowledge([
    ...
    def generate_report(self):
        ...
        if failed_tests:
            ...
            for test in failed_tests[:5]:  # Show first 5
                print(f"  - [{test['category']}] {test['query'][:60]}... ({test['notes']})")
...
def main():
    ...
    if success_rate >= 80:
        print(f"🎉 EXCELLENT: System is highly functional ({success_rate:.1f}% success)")
