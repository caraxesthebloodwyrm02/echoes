...
def find_shadowed_modules(project_path):
    ...
    for root, dirs, files in os.walk(project_path):
        ...
        if any(skip in root for skip in ['.venv', 'venv', '__pycache__', '.git', '.github']):
            continue
...
def generate_report(shadowed_modules, output_file='shadowed_modules_report.txt'):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"Found {len(shadowed_modules)} modules that shadow Python standard library names:\n\n")
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as mod_file:
                    ...
                    if 'import ' in content or 'from ' in content:
                        f.write("  Note: This file contains imports that may need updating\n")
