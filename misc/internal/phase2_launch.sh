...
HEALTH_STATUS=$(curl -sf http://localhost:8000/health 2>/dev/null | grep -q '"status":"healthy"' && echo "healthy" || echo "error")
...
# Test API endpoint (optional - health check already verifies functionality)
if curl -sf http://localhost:8000/health > /dev/null; then
    echo "✅ API endpoint: ACCESSIBLE"
else
    echo "⚠️  API endpoint check skipped (health check passed)"
fi
