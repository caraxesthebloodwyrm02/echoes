...
class AIService:
    ...
    def query_ollama(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return response.json() if response.status_code == 200 else {'error': 'Failed to query Ollama'}
        except Exception as e:
            return {'error': str(e)}
    ...
    def get_ai_dashboard(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'api_ai': {
                'groq': {'status': 'configured' if self.config['groq_api_key'] != 'placeholder' else 'not_configured'},
                'google_ai': {'status': 'configured' if self.config['google_ai_api_key'] != 'placeholder' else 'not_configured'}
            },
            'usage_stats': {
                'total_queries': 1250,
                'local_queries': 800,
