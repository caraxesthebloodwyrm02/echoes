...
class SimpleDataHub:
    ...
    async def fetch_all_ecosystems(self) -> Dict[str, Any]:
        ...
        tasks = [self.fetch_microsoft_data(), self.fetch_google_data(), self.fetch_x_data()]

        results = await asyncio.gather(*tasks)

        ecosystem_data = {"microsoft": results[0], "google": results[1], "x": results[2]}
...
async def main():
    ...
    print(
        f"   X/Twitter: {len(data['ecosystems']['x'].get('trending_topics', {}).get('trending_topics', []))} trending topics"
    )
