...
def run_insights():
    ...
    try:
        ...
        print(f"✅ Social Insights: {data['engagement_summary']['total_interactions']:,} interactions")
        return True
    except ImportError as e:
...
def main():
    ...
    parser.add_argument('command', choices=['research', 'entertainment', 'finance', 'insights', 'master', 'weather', 'all'],
                       help='Module to run')
    parser.add_argument('--nudge', choices=['direction', 'motivation', 'reflection', 'celebration'],
    ...
    if args.nudge and success:
        try:
            ...
            print(f"🎵 Music Nudge: {nudge_result['song']['title']} by {nudge_result['song']['artist']}")
