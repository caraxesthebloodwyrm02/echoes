...
class MicrosoftFetcher:
        ...
        try:
            async with self.session.get(self.base_urls["azure_status"]) as response:
                ...
                return {
                    ...
                    if all(s["status"] == "status-green" for s in services)
                    else "issues",
    ...
    async def _fetch_microsoft_news(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"total_articles": len(articles), "latest_articles": articles}
        ...
        try:
            ...
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    return {"total_repos": len(popular_repos), "popular_repos": popular_repos}
                else:
        ...
        try:
            ...
            async with self.session.get(self.base_urls["office365_status"]) as response:
                if response.status == 200:
                            ...
                            services.append(
                                {
                                    ...
                                    "features": [
                                        f.get("DisplayName") for f in service.get("Features", [])
                                    ],
                                }
                            )
                    ...
                    return {
                        "overall_status": "healthy"
                        if all(s["status"] == "ServiceOperational" for s in services)
                        else "issues",
                        "services": services,
                    }
                else:
    ...
    async def _fetch_microsoft_releases(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.get(windows_url) as response:
                ...
                return {"windows_releases": releases, "last_updated": datetime.now().isoformat()}
