...
class PersonalizedFeed:
    ...
    resonance_patterns: Dict[str, float] = field(
        default_factory=dict
    )  # Category -> resonance score
...
class ResonanceEngine:
    ...
    def aggregate_financial_content(
        self, categories: List[ContentCategory] = None, hours_back: int = 24
        ...
        logger.info(
            f"Aggregating financial content for categories: {[c.value for c in categories]}"
        )
    ...
    def create_personalized_feed(
        ...
        emotional_preferences: List[EmotionalTone],
    ) -> PersonalizedFeed:
    ...
    def deliver_tailored_content(
        self, user_id: str, feed_id: Optional[str] = None, content_limit: int = 10
    ) -> List[TailoredContent]:
        ...
        if user_id not in self.user_feeds:
            # Create default feed
            self.create_personalized_feed(
                user_id, "Default Feed", [ContentCategory.FINANCIAL_NEWS], [EmotionalTone.NEUTRAL]
            )
    ...
    def analyze_content_resonance(
        self, content_id: str, user_id: str, engagement_data: Dict[str, Any]
    ) -> ContentResonance:
        ...
        if user_id in self.user_feeds:
            ...
            if content_category and hasattr(content_category, "category"):
                ...
                new_score = (
                    ...
                    + (
                        1.0
                        if resonance_level in [ResonanceLevel.HIGH, ResonanceLevel.MEDIUM]
                        else 0.0
                    )
                ) / 2
    ...
    def generate_content_insights(
        self, category: ContentCategory, time_period: str = "24h"
    ) -> Dict[str, Any]:
        ...
        logger.info(
            f"Content insights generated for {category.value}: {insights['content_volume']} items analyzed"
        )

        return insights

    def _fetch_raw_content(
        self, categories: List[ContentCategory], hours_back: int
    ) -> List[Dict[str, Any]]:
    ...
    def _process_and_tailor_content(self, raw_item: Dict[str, Any]) -> Optional[TailoredContent]:
        ...
        try:
            ...
            emotional_tone = self._analyze_emotional_tone(
                raw_item["title"] + " " + raw_item["summary"]
            )
            ...
            tailored_content = TailoredContent(
                ...
                target_audience=self._determine_target_audience(
                    raw_item["category"], emotional_tone
                ),
    ...
    def _determine_target_audience(
        self, category: ContentCategory, tone: EmotionalTone
    ) -> List[str]:
    ...
    def _rank_content_by_resonance(
        self, content_list: List[TailoredContent], user_feed: PersonalizedFeed
    ) -> List[TailoredContent]:
    ...
    def _calculate_resonance_level(
        self, content_id: str, user_id: str, engagement_data: Dict[str, Any]
    ) -> ResonanceLevel:
    ...
    def _analyze_sentiment_distribution(
        self, content_list: List[TailoredContent]
    ) -> Dict[str, float]:
...
def aggregate_financial_content(
    categories: List[ContentCategory] = None, hours_back: int = 24
) -> List[TailoredContent]:
...
def create_personalized_feed(
    ...
    return resonance_engine.create_personalized_feed(
        user_id, feed_name, preferred_categories, emotional_preferences
    )


def deliver_tailored_content(
    user_id: str, feed_id: Optional[str] = None, content_limit: int = 10
) -> List[TailoredContent]:
...
def analyze_content_resonance(
    content_id: str, user_id: str, engagement_data: Dict[str, Any]
) -> ContentResonance:
...
def generate_content_insights(
    category: ContentCategory, time_period: str = "24h"
) -> Dict[str, Any]:
