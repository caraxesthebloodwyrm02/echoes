...
def check_bandit_scan():
    ...
    print("đŸ”  Running Bandit security scan...")
    ...
    if not success:
        print(f"â Œ Bandit scan failed: {stderr}")
        return False
    ...
    try:
        ...
        print(
            f"đŸ“Š Bandit Results: {len(high_severity)} HIGH, {len(medium_severity)} MEDIUM issues"
        )

        if high_severity:
            print("đŸš HIGH SEVERITY ISSUES FOUND:")
        ...
        print(f"â Œ Error parsing Bandit results: {e}")
        return False
...
def check_dependency_vulnerabilities():
    ...
    print("âœ… Dependency vulnerability check completed")
    return True
...
def generate_security_report():
    ...
    print(
        f'đŸ“Š Security Score: {report["security_score"]:.1f}% ({passed_checks}/{total_checks} checks passed)'
    )
...
def main():
    ...
    try:
        ...
        print("\n=" * 50)
        if overall_secure:
            print("âœ… SECURITY MONITORING COMPLETE - System appears secure")
            return 0
        else:
