...
class EvaluationMetrics:
    ...
    def evaluate_results(self, game_results: Dict[str, Any], interp_results: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _evaluate_game_performance(self, game_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        avg_team_efficiency = np.mean([
            perf.get('efficiency', 0) for perf in team_perf.values()
        ]) if team_perf else 0
    ...
    def _evaluate_interpretability(self, interp_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        feature_score = len([f for f in features if f.get('causal_importance', 0) > 0.5]) / max(1, len(features))

        # Intervention success rate
        intervention_success = len([i for i in interventions if i.get('causal_evidence', False)]) / max(1, len(interventions))

        # Pattern detection effectiveness
        pattern_score = min(1.0, len(suspicious_patterns) / 3)  # Cap at 3 patterns for perfect score
    ...
    def _combine_scores(self, game_score: Dict[str, Any], interp_score: Dict[str, Any]) -> Dict[str, Any]:
        """Combine game and interpretability scores."""
        game_weight = 0.4
        interp_weight = 0.6

        combined = {
            'weighted_score': (game_score.get('overall_game_score', 0) * game_weight +
                             interp_score.get('overall_interp_score', 0) * interp_weight),
            'game_contribution': game_weight,
            'interp_contribution': interp_weight,
            'confidence': min(game_score.get('overall_game_score', 0),
                            interp_score.get('overall_interp_score', 0))  # Minimum of both
        }
