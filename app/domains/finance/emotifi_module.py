...
async def create_market_stress_notification(notification: MarketStressNotification):
    ...
    """
    logger.info(
        f"Market stress notification: {notification.user_id} - {notification.stress_trigger}"
    )
...
def _get_portfolio_adjustments(self, profile: EmotionalRiskProfile) -> List[str]:
    ...
    return (
        adjustments
        if adjustments
        else ["Maintain diversified, balanced approach with emotional monitoring"]
    )
...
def _get_financial_reframing(self, stress_trigger: str) -> str:
    ...
    return reframes.get(
        stress_trigger, "Markets are cyclical. Focus on your investment philosophy."
    )
