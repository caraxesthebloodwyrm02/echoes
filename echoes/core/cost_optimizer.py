...
class CostOptimizer:
    ...
    def select_optimal_model(self, task_description: str, context_length: int = 0,
                           requires_vision: bool = False, requires_tools: bool = False) -> str:
        """
    ...
    def _analyze_task_complexity(self, task_description: str) -> str:
        ...
        # Complex tasks
        complex_keywords = ['analyze', 'design', 'architecture', 'complex', 'research',
                          'implement', 'code', 'debug', 'optimize', 'refactor']
    ...
    def check_cost_alerts(self) -> List[str]:
        ...
        monthly_cost = sum(day_stats.get('cost', 0.0)
                          for day_key, day_stats in self.usage_stats['daily_stats'].items()
                          if day_key.startswith(current_month))

        days_in_month = 30  # Approximate
        projected_monthly = (monthly_cost / max(1, len([k for k in self.usage_stats['daily_stats'].keys()
                                                       if k.startswith(current_month)]))) * days_in_month

        if projected_monthly >= self.cost_alerts['monthly_limit'] * self.cost_alerts['alert_threshold']:
            alerts.append(f"⚠️ Monthly cost projection: ~${projected_monthly:.2f} of ${self.cost_alerts['monthly_limit']:.2f} limit")
    ...
    def track_multimodal_usage(self, modality: str, duration_seconds: float = 0, 
                              tokens_used: int = 0, image_count: int = 0, 
                              detail_level: str = 'low') -> float:
        """
    ...
    def estimate_multimodal_cost(self, modality: str, duration_seconds: float = 0, 
                                estimated_tokens: int = 1000, image_count: int = 1, 
                                detail_level: str = 'low') -> float:
        """
