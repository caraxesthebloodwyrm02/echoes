...
class WebhookRegistration(BaseModel):
    ...
    events: List[str] = Field(..., description="Events to trigger webhook: ['image_processed', 'audio_transcribed', 'media_processed']")
...
async def process_media(
    ...
    try:
        ...
        async def process_media_file():
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(None, lambda: assistant.analyze_media_file(media_path, req.analysis_type, req.custom_prompt))
            # Clean up temp file
            try:
