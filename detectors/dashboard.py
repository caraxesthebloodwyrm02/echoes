...
def create_dashboard():
    ...
    tier_fig = go.Figure(data=[
        go.Bar(x=list(tier_data.keys()), y=list(tier_data.values()),
               marker_color=['blue', 'orange', 'red'])
    ])
    tier_fig.update_layout(
        title="Detections by Tier",
        xaxis_title="Tier",
        yaxis_title="Count"
    )
    ...
    if audit_data:
        ...
        if 'timestamp' in df.columns:
            ...
            timeline_fig = px.histogram(df, x='timestamp', color='tier',
    ...
    confusion_fig = go.Figure(data=[
        go.Bar(x=list(fp_fn_data.keys()), y=list(fp_fn_data.values()),
    ...
    app.layout = html.Div([
        ...
        html.Div([
            html.Div([
                html.H3(f"{total_detections}"),
                html.P("Total Detections")
            ], className="summary-card"),

            html.Div([
                html.H3(f"{total_actions}"),
                html.P("Actions Taken")
            ], className="summary-card"),

            html.Div([
                html.H3(f"{len(metrics)}"),
                html.P("Active Detectors")
            ], className="summary-card"),
